...
| **4** | **B, abstract** | say what the P3109 cross-walk maps — **layout**, not values | The abstract says it "maps each pack to its **corresponding** standards-track configured format". Measured against the working group's own tables, P3109 `binary8p4` and OCP `E4M3FN` share a field layout and differ in bias by one, so **every finite value differs by exactly a factor of 2** (996 of 1000 codes, one distinct ratio). Mapping layout is worth publishing; the sentence just needs to say so. | one word |
