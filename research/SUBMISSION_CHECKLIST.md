...
| **4** | **B, related work** | add the four-paragraph subsection | Positions the corpus against *published vector sets*, measured from five projects. Currently the paper positions against format families only (Paper A does this well) and against no vector corpus at all. | one subsection |
...
| IEEE P3109 **v3.2.0** | Not publicly verifiable — no public repo, no release feed. Either cite a retrievable artefact or say "draft, version on file". | — |
