...
| **3a** | **both** | work from **`BIBLIOGRAPHY_FIXES.md`** | One table, both papers, **18 entries**: 7 in A and 11 in B, each with what it currently says, what the identifier actually resolves to, and the defect class. Regenerated from the live arXiv API rather than transcribed. Paste-ready LaTeX for all of them is in `CORRECTED_BIBITEMS.tex`. | — |
