...
physical principle; it is an empirically-located design heuristic. The robustness
...
Seven findings, restated without superlatives:
...
\item \textbf{GF16 robustness.} GF16 \texttt{[1|6|9]} is the minimum-width
  IEEE-style format achieving full workload robustness (7/7 across the four ML
  workloads and the three hold-out workloads); FP16 and BF16 each fail one.

\item \textbf{BF16 gradient loss.} BF16's 7-bit mantissa loses 92.7\% of gradient
  updates to quantization, versus 36.1\% for GF16---an $8.7\times$ edge that
  arises purely from mantissa width.

\item \textbf{MLP training.} In end-to-end 2-layer MLP training with per-step
  re-quantization, GF16 reaches $\sim$4$\times$ lower final loss than BF16
  (0.0019 vs 0.0074) and $14\times$ lower weight MSE.

\item \textbf{LUT information floor.} Floating-point LUT cost on Artix-7
  (zero-DSP) follows $\text{LUT}_{\text{ADD}} = 1.55W^{2}$ (R$^{2}{=}0.88$) and
  $\text{LUT}_{\text{MUL}} = 2.06W^{2}$ (R$^{2}{=}0.97$) across four orders of
  magnitude in width.

\item \textbf{Encoding invariance.} GF16 MUL and a native takum16 MUL both cost
  505 LUT in the zero-DSP regime: $\text{GF16} \equiv \text{takum16}$. The
  encoding buys dynamic range, not area.
\end{enumerate}
