...
gHashTag/t27 repository: 83 formats in 13 families, each with a bit-exact
conformance pack~\cite{vasilev2026catalog}. The catalog is a registry-filling
...
\item \textbf{Truncated-multiply}---Mitchinson--Smith sticky-OR truncation that
  ...
  bits routes and is strictly more correct (2 fails vs 5 on a 4\,848-vector
  stress set, zero regressions).
\end{enumerate}
...
\item \textbf{Structural formats:} 15 of 83 are structural-by-design
  (parametric, block-scaled, non-S:E:M). They are reported honestly as such,
  not forced into bit-exact boxes.
\end{itemize}
...
remaining 15 formats are structural (no decode law---unreachable) or
transcendental-decode research-level work (takum32/64: routing unlocked,
residual 1-ULP Taylor misses).
...
Takum64 decode, \emph{truncated} (94 + 72-bit, sticky-OR) & 94/72-bit & $\checkmark$ & below the openXC7 routing ceiling; strictly more correct (2 fails vs 5) \\
\bottomrule
\end{tabular}
\end{table}
...
\item \textbf{Structural formats.} 15 of 83 are structural-by-design
  ...
  than forced into bit-exact boxes. E8M0 is the shared-exponent component of
  Microscaling, not a standalone catalog row---the canonical count is
  83~\cite{vasilev2026catalog}.
