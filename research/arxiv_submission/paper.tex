...
bytes, not parameter count), narrower formats allow larger models. A controlled
sweep ($W \in \{12,14,16,32\}$, 2-layer transformer, 300 steps, synthetic
Markov text) reveals that GF14 ($E{=}5, M{=}8$) achieves the lowest BPB at
matched budget: 0.295 vs.\ FP32's 0.346 (14.6\% better) and GF16's 0.336
(12.2\% better). The mechanism is straightforward: at 1.75~B/param vs.\ 2.0,
GF14 fits $\sim$18\% more parameters in the same byte budget, and the precision
loss from $M{=}8$ vs.\ $M{=}9$ is insufficient to negate the gains from
additional model capacity. Stochastic rounding \emph{hurts} at this scale
(GF14+SR: 0.304 $>$ GF14: 0.295), consistent with the model being
underfit rather than overfit. This finding is preliminary (single seed,
synthetic data, CPU-scale model) but suggests that the $\varphi$-rule's
value extends beyond robustness: at matched budget, the optimal operating
point may be narrower than GF16.
...
\item \textbf{LUT information floor (Vasilev Floor).} By the Vasilev Floor
  ...
  follows $\text{LUT}_{\text{ADD}} = 1.55W^{2}$ (R$^{2}{=}0.88$) and
  $\text{LUT}_{\text{MUL}} = 2.06W^{2}$ (R$^{2}{=}0.97$) across four
