...
evidence chain. We further demonstrate, via a training-noise-floor
simulation, that GF16 preserves $8.7\times$ more gradient updates than BF16,
explaining its suitability for low-precision training.
\end{abstract}
...
\item \textbf{BF16} ($E{=}8$, $M{=}7$) \textbf{fails matmul}: its 7-bit mantissa
  produces $10\times$ worse max error on the matrix-multiply workload. The 7-bit
  mantissa is the bottleneck---too few mantissa bits.
\end{itemize}
...
\paragraph{Connection to the IGLA RACE training pipeline.} The
trios-trainer-igla project~\cite{vasilev2026igla} trains a small language model
with weights quantized to GF16 and tracks bits-per-byte (BPB). The two tables
above explain the format choice: GF16 is the minimum-width IEEE-style format
where training converges \emph{without loss scaling or stochastic rounding}---at
$63.9\%$ update survival and $2.2\%$ accumulation error, the gradient signal
remains informative, whereas BF16's $7.3\%$ survival and $50\%$ accumulation
error effectively freeze the weights. This is consistent with the robustness
result of \S\ref{sec:robustness}: the $E/M$ balance that passes all four ML
workloads is the same balance that keeps the training-noise floor low.
...
GF14 and above pass all four tests; FP16 fails dynamic range; BF16 fails matmul.}
