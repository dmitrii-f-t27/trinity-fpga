...
\title{Ternary Network Floats \\
  \large A datapath with no multiplier anywhere: two formats close the two halves
  of a ternary node, one sign-select primitive builds both a radio and a
  transformer at zero DSP, and no uniform format escapes the accumulator ladder}
...
The ladder is a frontier. A width-$N$ format of effective mantissa $m$ and range
$b$ binades is dominated exactly when $m+\log_3(b+1)\le N-1$, while any uniform
format obeys $m+\log_2(b+1)\le N-1$: the same books in two currencies. Every
uniform binary format is dominated with slack $0.3691E$ positions; of 28
catalogued formats from 8 to 128 bits none escapes, and Ternary27, the only
ternary-radix competitor, falls with five positions to spare.
...
\begin{table}[t]\centering
\caption{One ternary neuron, synthesised as a single design on XC7A200T with no
DSP. The two variants differ in exactly one block: posit decodes its regime before
the add, TNF reads its fields directly.}
\label{tab:tnet}
\begin{tabular}{lrrr}
\toprule
format & LUTs & $F_{\max}$ & throughput per LUT \\
\midrule
\textbf{TNF} & $\mathbf{440}$ & $\mathbf{80.73}$\,MHz & $\mathbf{0.184}$\,MHz/LUT \\
...
\paragraph{The standard falls out.} Applied to the block axis this stops being
...
\begin{table}[t]\centering
\caption{Throughput per area among formats that carry a range. One ternary-neuron
datapath, 21 formats, XC7A200T, no DSP.}
\label{tab:carryrange}
\begin{tabular}{rlrrr}
\toprule
& format & storage & MHz/LUT & vs.\ first \\
\midrule
1 & \textbf{GFTernary} (ours) & 2 bits & \textbf{0.177} & $1.00\times$ \\
2 & \textbf{TNF4} (ours) & 4 bits & \textbf{0.167} & $1.06\times$ \\
3 & binary32 & 32 bits & 0.163 & $1.09\times$ \\
4 & TNF32 (ours) & 32 bits & 0.151 & $1.17\times$ \\
$\vdots$ & & & & \\
20 & posit16 & 16 bits & 0.047 & $3.77\times$ \\
21 & posit32 & 32 bits & 0.030 & $5.90\times$ \\
\bottomrule
\end{tabular}
\end{table}
...
\paragraph{}
An \texttt{int8} datapath scores $0.189$\,MHz/LUT in the same harness, which is
$7\%$ above GFTernary. It is excluded, and the reason is structural rather than
