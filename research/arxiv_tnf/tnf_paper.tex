...
flow the pair costs zero DSP: a matched filter on real air at
$78$\,MHz, every transformer block, a systolic GEMM at
...
per area, $0.177$ and $0.167$\,MHz/LUT against binary32's $0.163$.
...
$+0.5$. Five retractions are marked in place below; more were withdrawn during the
...
\begin{theorem}[Three digits are forced]\label{thm:cardinal}
Parallel addition in base $\varphi$ --- a digit-set conversion in which each
output digit depends on a bounded window of input digits --- requires an
alphabet of three digits and admits one~\cite{frougny2011}. The minimality is of
\emph{cardinality} and not of the particular set: $\{-1,0,1\}$ is one such
alphabet and $\{0,1,2\}$ is another, verified by construction. No two-digit
alphabet suffices at any window size.
\end{theorem}

Ternary is therefore forced and the particular letters are not, which is the
strongest form the statement takes and weaker than the one we first wrote down.
Two further limits belong with it. The window is not small: the published
algorithm on $\{-1,0,1\}$ is $21$-local, with memory $10$ and anticipation $10$,
so ``constant time'' read as ``cheap'' would be wrong by an order of magnitude.
And the theorem concerns the digit alphabet of a positional representation,
while our accumulator is a pair of integers never reduced to a canonical digit
string --- so what transfers is the impossibility for two letters, and not any
claim that our alphabet is unique.
