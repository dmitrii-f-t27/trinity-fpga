...
ternary gate on a binary-optimised process, the $5.3\%$ slot advantage is consumed
several times over. Nothing in this paper contradicts that. The $5.3\%$ is quoted
...
budget at which binary packing is free, so the ``$5.3\%$ more economical'' of
...
mantissa bit of silicon at every rung above TNF8, and posit's costs more than one
...
rather than lumping them. Against posit the silicon term alone is $9.95$ bits at
TNF16's $M = 9$ --- more than TNF16's entire $9$-bit mantissa, so the area a
...
class. Against takum the silicon term at the same $M$ is $0.91$ bits, which is small. The
...
bits as the class widens: for the unary regime $9.95$ bits at TNF16's $M = 9$
becomes $4.26$ at TNF32's $M = 21$. The fixed-field area
...
\[
  ...
  \end{cases}
\]
Both branches are non-negative as written: over-sizing spends the surplus exponent
trits that the smaller range would not have needed, so the first branch subtracts
the required width from the chosen one and not the reverse. The penalty is
asymmetric: over-sizing costs precision logarithmically in the range, under-sizing
costs a share of the visited span that is not logarithmic in it, so when the
visited range is uncertain the rule should round up.
\end{theorem}
...
\log_3 2 = 0.3313$ positions of accuracy net of the range it returns, where
$\kappa(r) = (r-1)^2/(r \ln r)$. The near-coincidence of $0.3691$ and $0.3313$ is
numerical and carries no content: TNF fixes the scale radix at $2$
...
\caption{Canon plate. The silicon table separates nominal format width from stored binary words and documents the corrected modules with their corresponding conformance coverage.}
...
\subsection{The post-route FPGA table, evaluated on an audited harness}
\begin{figure}[htbp]\centering
...
arithmetic continues independently of the P3109 track~\cite{mohanty2025}. Against
...
first is what a format can supply. The first layer is also acquiring load-bearing
...
$b = 2$, $2^b - 2 = 2 = 3^1 - 1$, equality. For $t \ge 2$ the gap is strict and
grows: $L_t = 2^b - 3^t$ is the packing loss of
Theorem~\ref{thm:packing-density}, which is positive and odd for every $t$.
\end{proof}
...
\section{Limitations}\label{sec:limitations-anchor}
