...
$+0.5$. Five retractions of our own claims are marked in place below; further claims
were withdrawn during the campaign and removed rather than marked.
\end{abstract}
...
We also report what the campaign cost us. Five of our own claims were falsified by
...
\subsection{Which rung, and why the answer moves}\label{sec:ladderlaw}

Table~\ref{tab:ladder} quantises every $2$-D linear weight of SmolLM2-135M to
...
\subsection{The four-bit transposition, and an explanation that failed}
\label{sec:salience}
...
effect and not noise. We do not have a mechanism for it, and record the
candidate we tested and rejected because the rejection is informative.

Mean squared error treats every weight alike, while the output error of a linear
layer is $\sum_j \delta w_{ij}x_j$, so a weight multiplying a large input
channel should cost more. The channels are far from equal: measuring the RMS of
every input channel of every linear projection, the first attention projection
spans $28\times$ between its largest channel and its median on SmolLM2-135M and
$22.4\times$ on Qwen2.5-0.5B. This is the observation AWQ~\cite{awq} is built
on --- salience follows the activation distribution rather than the weight
distribution --- and it predicts that weighting the error by $a_j^{2}$ should
move the choice toward the coarser ladder, which is the direction the
transposition needs.

It does so on one model and not the other.

\begin{table}[t]\centering
\caption{Ranking by $\sum_j a_j^2\delta w_{ij}^2$ against ranking by
$\sum \delta w^2$. The weighting reverses the four-bit pair on SmolLM2, matching
perplexity, and leaves it unchanged on Qwen, where it does not.}
\label{tab:salience}
\begin{tabular}{llll}
\toprule
model, bits & by MSE & by weighted error & by perplexity \\
\midrule
SmolLM2, 4 & supergolden & \textbf{$\varphi$} & \textbf{$\varphi$} \\
SmolLM2, 5 & plastic & plastic & plastic \\
Qwen, 4    & supergolden & supergolden & \textbf{$\varphi$} \\
Qwen, 5    & plastic & plastic & plastic \\
\bottomrule
\end{tabular}
\end{table}

The reason the correction fails on the second model is visible in the numbers
rather than hidden in them. On SmolLM2 the weighting scales the four candidates
by factors between $0.12$ and $0.24$, so it reorders them; on Qwen it scales all
four by very nearly $1.09$, so it cannot. Channel salience is therefore
correlated with the quantisation error pattern in one model and close to
independent of it in the other, and an explanation that requires the correlation
is not general.

We leave the transposition open. What survives it is narrower and still useful:
at four bits the two ladders are within $2.7\%$ on the smaller model and
$10.7\%$ on the larger, both in $\varphi$'s favour on the measurement that
matters, and the unweighted form remains the best available predictor at three
and five bits.
...
$r^{*}=2.054,\,1.4115,\,1.1999,\,1.1050,\,1.0576,\,1.0319$ at $3$ to $8$ bits,
...
\begin{theorem}[The normalisation floor]\label{thm:floor}
