...
window instead of off the count. Every application of the rule in this paper
carries a per-block or per-channel scale, which is precisely the centring
hypothesis, so no figure reported elsewhere moves; the correction bites on raw,
unscaled workloads, and those are exactly the ones a reader is most likely to try
first.
