...
\emph{MXFP4's scale field is four bits wider than it needs to be.} $2^{k}$ at
four bits reproduces E8M0's perplexity \emph{exactly} --- $22.4998$ and
...
\emph{And the upper frontier is not ours.} E4M3 --- a scale carrying a
mantissa, which is what NVFP4~\cite{nvfp4} uses --- reaches $19.8628$ and
$13.7636$ at the same $4.25$ bits per weight where MXFP4 sits. No geometric
grid, however fine, matches a scale with significand bits. Below $4.25$ bits per
weight the frontier is ours and above it is theirs.
