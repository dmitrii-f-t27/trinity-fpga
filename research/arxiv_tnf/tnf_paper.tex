...
\caption{Canon plate. Of 882 distinctive literals, 880 trace to the tree that produced them; the two that do not are retained and marked, and five document-to-code pairs still have no gate.}
...
Re-run on this revision over $6{,}108$ data files, the check finds $882$
distinctive literals in the paper of which \textbf{two} trace to no file. They are
...
those now trace, and the count is restated rather than carried forward.}
\label{tab:untraced}
\begin{tabular}{ll}
\toprule
...
value survives. Both of the two are achieved frequencies, the quantity with the
widest seed spread in this paper, so neither should be quoted without a
re-measurement; that is the pattern this check exists to surface.
