# Ternary Floats (GF-T) — paper, figures, provenance

`gft_paper.tex` builds with `tectonic gft_paper.tex`. `pdflatex` is not installed
...
| GF-T16 is 2.83× / 5.46× more accurate than takum16 | round trip, 6000 values, **inside GF-T's range only** | §Accuracy |

## Bounds that must travel with the claims

- **The accuracy result is range-bounded.** Theorem 12 forbids beating takum
  beyond GF-T's own range; past `|e| = 40` GF-T16 has no values and takum16 does.
- **The area argument separates the competitors.** Decisive against posit
  ...
  every rung above GF-T8). Do not average the two into one number.
...
- **The regime codecs are structural models**, not reproductions of the published
  ...
  length-prefixed one.
- **GF-T128 is not claimed.** M=119 does not converge in routing on XC7A200T.
...
1. *The GF-T rungs do not fit their widths.* They do — the width counts positions,
...
4. *GF-T64 will cost 4,762 LUTs.* It costs 7,479 — the single power law was low by
   36%. The two-term structural model is right to 4.4%.

## Rebuilding

```
tectonic gft_paper.tex
