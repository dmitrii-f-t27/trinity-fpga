...
\paragraph{takum16 and tekum16 are indistinguishable here.} The two produce
identical figures on every bin, which says the reconstruction our tekum oracle
implements does not capture whatever separates tekum from its parent. The
comparison in Section~\ref{sec:accuracy} should therefore be read as against
takum16 as reconstructed; we say this rather than let the reader discover it.
...
tekum itself, and that model behaves as takum.} Table~\ref{tab:field} shows the
two producing identical figures on every bin. tekum~\cite{tekum} is a descendant of takum~\cite{takum} adapted for balanced
ternary. Our
...
\item \textbf{Four of nine rungs are measured.} GF-T4 through GF-T32 fit the
