...
(GF-T), a ladder built for that role: the exponent is a balanced-ternary integer of
...
The law's consequence is that a format's costs become commensurable, and this is
the paper's main result. A mantissa bit has a marginal area $\lambda$, so a
variable-length exponent field costs a measurable number of mantissa bits of
silicon (Theorem~\ref{thm:convert}). Synthesising the regime codecs of both
tapered families under one harness on an XC7A200T, net of harness: a unary regime
costs $438$ LUTs and a length-prefixed one $40$, against $0$ for a fixed field.
At the 16-bit class, where $\lambda = 48.8$ LUTs per bit, those are $8.98$ and
$0.82$ mantissa bits. The first exceeds GF-T16's entire mantissa; the second is
small. The area argument for fixed fields is therefore decisive against posit and
weak against takum, and we say so rather than averaging the two.
...
regime is asymptotically optimal and nothing, GF-T included, beats it beyond its
own range. Inside GF-T16's range it is $2.83\times$ and $5.46\times$ more accurate
at mid and far magnitudes.
...
LUTs at $161.1$, $153.2$, $131.7$, $83.3$ and $48.2$\,MHz for GF-T4 through
GF-T64, one cycle of latency, no DSP blocks.
...
area for GF-T64 that came in $1.57\times$ low, and each is retracted in place.
...
$0.247$, against the $2^{-es} = 0.25$ that Theorem~\ref{thm:posit} requires;
...
GF-T16, a fixed-field format, as geometrically tapered. Measuring each format's
...
argument for GF-T's binary scale, alongside the $0.331$ positions of
Theorem~\ref{thm:scaleradix}: a radix-3 scale would wobble by $1.585$ bits
instead of $1$.
...
rungs of the machine-readable specification obey it at GF-T4, GF-T8 and GF-T32;
...
GF-T16   & spec & $4$  & $9$    & $14$   & $2$ & $\mathbf{11}$ \\
GF-T32   & spec & $6$  & $25$   & $32$   & $0$ & $25$ \\
GF-T32   & note & $5$  & $21$   & $27$   & $5$ & --- \\
GF-T64   & note & $7$  & $52$   & $60$   & $4$ & $\mathbf{56}$ \\
GF-T128  & note & $8$  & $115$  & $124$  & $4$ & $\mathbf{119}$ \\
GF-T256  & note & $9$  & $242$  & $252$  & $4$ & $\mathbf{246}$ \\
GF-T512  & note & $10$ & $497$  & $508$  & $4$ & $\mathbf{501}$ \\
GF-T1024 & note & $11$ & $1006$ & $1018$ & $6$ & $\mathbf{1012}$ \\
\bottomrule
\end{tabular}
\end{table}

No single alternative convention explains the upper rungs either: reading the
exponent as $\lfloor E_t\log_2 3\rfloor$ bits fits GF-T16, GF-T64, GF-T128 and
GF-T1024 exactly but overshoots GF-T256 and GF-T512 by one and misses GF-T32
...
workload built in exact arithmetic --- necessary above GF-T32, where a
...
find, no one has stated: GF-T encodes its exponent in radix 3 but scales by
...
\begin{corollary}[What GF-T is, precisely]
GF-T is not a ternary float in the sense of Ternary27; it is a binary-radix float
...
--- GF-T included --- can beat it over unbounded range. What GF-T does instead is
...
advantages \emph{inside GF-T's range}, which is where the taper has already cost
...
\midrule
fixed field (GF, GF-T)                     & $0$   & ---          & $0$   & ---          & $\mathbf{0}$ \\
\bottomrule
\end{tabular}
\end{table}
...
predicted from the $[7,25]$ fit that the reconciled GF-T64, at $M = 56$, would
...
regime's silicon exceeds an entire GF-T16 mantissa; the length-prefixed regime is
...
GF-T8, and posit's costs more than one at every rung below GF-T256.
\end{corollary}

Corollary~\ref{cor:breakeven} is the exact form of what
Corollary~\ref{cor:narrow} could only say qualitatively, and it is the sharpest
...
the 16-bit class --- more than GF-T16's entire $9$-bit mantissa, so the area a
...
Theorem~\ref{thm:floor} requires to GF-T's own range.
...
\midrule
\texttt{posit32\_decode} & $480$ & $49.0$\,MHz & leading-zero count, barrel shift \\
\texttt{takum32\_decode} & $76{,}821$ & --- & a $65536 \times 48$-bit table for $2^{f}$ \\
GF-T32, exponent path    & $0$   & ---         & the field is read directly \\
\bottomrule
\end{tabular}
\end{table}

The takum figure needs stating carefully. Its decoder evaluates
$2^{f_{\mathrm{hi}}/2^{16}}$ from a $65536 \times 48$-bit table, then applies a
Taylor correction through a $108$-bit and an $80$-bit multiply. In this flow the
table did not infer as block RAM --- the module lacks the
\texttt{ram\_style="block"} attribute that the same suite's 16-bit decoder
carries --- so it was built from logic, and $76{,}821$ LUTs is what the design as
written costs here rather than a property of takum. Inferred correctly the table
is $3.15$\,Mbit, which is $85$ RAMB36 tiles, $23\%$ of this device's entire block
memory. Either way the cost is not the regime field.
...
tapered and logarithmic; LNS is untapered and logarithmic; GF, GF-T and IEEE are
...
\begin{corollary}[What GF-T's exponent path costs]
\label{cor:zeropath}
GF-T's value law is linear and its exponent field is fixed, so its exponent path
