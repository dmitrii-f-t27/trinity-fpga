...
So the gap is not empty because its occupants are dominated; they are not. We
conjecture it is empty because of decode cost. Both occupied classes decode with
the same piece of hardware, a leading-zero counter: unary reads the run directly,
and a length-prefixed regime reads the run as a length. An intermediate growth
rate requires an integer square root or equivalent, which is a multi-cycle
structure or a table, and no accuracy gain in Table~\ref{tab:codes} is large
enough to pay for one. We have not built the hardware to test this, and mark it as
a conjecture rather than a finding.
