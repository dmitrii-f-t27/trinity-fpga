...
\paragraph{The declared rungs do not fit their declared widths.} Storing the
exponent as a plain binary field of $\lceil E_t\log_2 3\rceil$ bits, the layout
$1 + \lceil E_t\log_2 3\rceil + M$ comes to 17 bits for GF-T16, 36 for GF-T32, 65
for GF-T64 and 129 for GF-T128. The reference oracle stores it exactly that way,
so either the format packs trits more densely than the oracle models --- five
trits into eight bits reaches $94.9\%$ against the $63.3\%$ of a four-trit field
--- or the rung names are approximate. This has to be settled before any
design-space conclusion can be drawn, because it decides how many bits the
exponent actually costs.

\paragraph{The current budgets are the least efficient available.} GF-T16 uses
$E_t = 4$ at $63.3\%$ and GF-T64 uses $E_t = 7$ at $53.4\%$, against $94.9\%$ at
$E_t = 5$ and $84.4\%$ at $E_t = 3$. That is worth stating as a fact about the
layout; we deliberately do not turn it into a recommendation, because which
budget is best depends entirely on the range a workload actually visits. Searching
for the lowest error at a fixed width on our $\pm25$-binade sweep returns
$E_t = 4$ at every width --- not because four trits are optimal, but because on a
workload that narrow, range is never worth a mantissa bit. A recommendation would
require stating the workload first, and that is the point of the corollary in
Section~\ref{sec:law}: the pair $(M_{\mathrm{eff}}, \text{binades})$ is the answer,
not either half of it.

\paragraph{The waste is the price of the feature, and should be named as such.}
At $E_t = 4$ the field holds 81 of its 128 codes, discarding $0.66$ bits; at
$E_t = 7$ it discards $0.91$. Those are not losses to be recovered --- they are
exactly what is paid so the exponent remains a balanced-ternary number that a
ternary ALU adds without conversion. Stated that way the trade is legible: GF-T16
buys native ternary exponent arithmetic for two-thirds of a bit of precision. A
reader can then decide whether their fabric makes that worth it.

\paragraph{What would follow, once the packing question is settled.} If the
exponent is stored as a binary field, budgets of $E_t \in \{3, 5, 8, 10\}$ pay
$84.4$--$94.9\%$ against the $53.4$--$63.3\%$ of the present choices, and a
re-laddered family on those budgets would spend less of each word on being
ternary. If instead the trits are densely packed, the premium is already close to
the $\log_2 3 = 1.585$ bits-per-trit floor and there is little to recover. We
report the fork rather than pick a side: the measurement that decides it is
whether the shipped encoder packs, and that is a question about the specification
