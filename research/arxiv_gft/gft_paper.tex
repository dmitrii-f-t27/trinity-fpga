...
\begin{theorem}[The ladder's area exponent rises toward two]
\label{thm:alpha}
A rung's area is a fixed overhead --- registers, the exponent adder, the
renormalisation --- plus a significand multiplier whose cost is asymptotically
quadratic in $M$. Hence the effective exponent
$\alpha(M) = \mathrm{d}\ln A/\mathrm{d}\ln M$ increases monotonically toward $2$,
and any single-power fit is valid for interpolation within its range and not for
extrapolation beyond it.
\end{theorem}

This sharpens rather than weakens Corollary~\ref{cor:narrow}. Since $\lambda$
grows faster than a fixed $\alpha$ would suggest, a regime codec of roughly fixed
cost converts to \emph{even fewer} mantissa bits as the class widens than
Table~\ref{tab:convert} shows, and the fixed-field area advantage is
correspondingly more concentrated at narrow widths --- the widths at which
quantised inference on FPGAs actually operates~\cite{mlformats}.
