...
$0.375 \cdot 2^{-(M+1)}$, with the constant holding to $\pm 1.5\%$ over eight
rungs and 302 orders of magnitude --- the precision is a closed form in the
mantissa width alone. And at each of those rungs the error varies by at most
...
In silicon, on an entirely open flow: 12, 50, 212 and 1{,}477 LUTs at 161.1,
153.2, 131.7 and 83.3\,MHz for GF-T4/8/16/32, one cycle of latency, no DSP
blocks. Three defects found while measuring are reported rather than quietly
...
\paragraph{The precision is a closed form.} The measured error is
$0.375 \cdot 2^{-(M+1)}$ at every rung, the constant varying by $\pm1.5\%$ across
eight rungs that span 302 orders of magnitude. Nothing was fitted: $M$ comes from
the format definition and the constant falls out of round-to-nearest on a uniform
mantissa. A user of the ladder can state the error of a rung that has never been
built.

\paragraph{The error does not move with magnitude.} Flatness stays between
