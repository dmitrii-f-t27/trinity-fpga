...
\begin{theorem}[Posit taper]\label{thm:posit}
For a posit of $n$ bits with exponent field $es$, the regime is a run of $k$ like
bits carrying a scale of $\mathrm{useed}^k = 2^{2^{es}k}$, so $k = |e|/2^{es}$, and
the regime consumes $k+1$ of the $n$ bits. Hence
\[
  M(e) \;=\; n - 2 - es - \frac{|e|}{2^{es}},
  \qquad \frac{\mathrm{d}M}{\mathrm{d}|e|} \;=\; -2^{-es}\ \text{exactly.}
\]
\end{theorem}

The Posit Standard (2022) fixes $es = 2$ at every precision, so the rate is
$-0.25$ for all widths --- which is why it appeared as a family constant.
Measured: $-0.2497$ at posit16 and $-0.2505$ at posit32 against a predicted
$-0.2500$, with a linear fit at $R^2 = 0.98$ against a logarithmic fit at $0.84$.

\begin{theorem}[Takum taper]\label{thm:takum}
Takum spends a fixed 3-bit regime field holding $r$, after which the
characteristic occupies $r$ bits. The reachable exponent therefore grows like
$2^r$ while the significand loses $r$, giving
\[
  M(e) \;=\; M_0 - c\log_2|e| ,
\]
a logarithmic taper with no constant rate.
\end{theorem}

Measured: $c = 0.751$ at takum16 and $0.747$ at takum32, logarithmic fit
$R^2 = 0.92$ against a linear fit at $0.79$ --- the reverse of posit, and the same
$c$ at both widths.

\begin{corollary}[A rate is not always the right summary]
A taper can be summarised by a rate only when it is linear. Quoting
``bits per binade'' for a logarithmic taper yields a number that depends on the
measurement window: an earlier draft of this paper reported $-0.117$ for takum16
over $|e| \in [0,30)$, and the same format measured over $[0,60)$ would give a
smaller figure for no reason but the window. The functional form has to be
identified before a summary statistic is chosen.
\end{corollary}
...
This also gives a quantitative reading of takum's stated claim to improve on
posits~\cite{takum}: at $|e| = 30$ a posit has shed $7.5$ bits of significand and
a takum $3.7$, and the gap widens without bound because one taper is linear and
the other is not. What the literature describes qualitatively --- more precision
near unity, less at the extremes~\cite{gustafson} --- has a form and a coefficient,
and both are recoverable from round-trip error alone.
