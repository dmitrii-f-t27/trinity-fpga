...
the exponential. GF-T's exponent path is wires. Theorem~\ref{thm:valuelaw} makes
...
GF-T32, exponent path             & $0$        & $0$  & ---         & the field is read directly \\
\bottomrule
\end{tabular}
\end{table}
...
Evaluating $2^{f}$ to $P$ bits with a $2^{k}$-entry table and a degree-$d$
polynomial correction requires $k \approx P/(d+1)$, so the table is
$P \cdot 2^{P/(d+1)}$ bits: exponential in the target precision and reducible only
by raising the polynomial degree, which trades memory for multipliers. The width
of the source format does not enter.
\end{theorem}
...
\begin{corollary}[Where the logarithmic law becomes unaffordable]
Decoding into binary32 needs $P = 48$, so $k = 16$ and $3.1$\,Mbit --- large but
buildable, as measured. Into binary64 it needs $P = 105$, so $k = 35$ and roughly
$3.6$\,Tbit at $d = 2$, which is not a design; the degree must rise instead, and
the cost reappears as multipliers. A linear value law needs no polynomial at any
target precision, because the significand is already in the datapath's
representation.
\end{corollary}
