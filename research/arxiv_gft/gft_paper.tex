...
\title{GF-T16: a fixed-field float with a balanced-ternary exponent, \\
       measured against tekum16 and on open-source silicon}
...
tekum~\cite{tekum} buy
dynamic range with a variable-length regime field. The field costs a
barrel-shift decode on any fabric, and precision tapers towards the ends of the
range. We describe \textbf{GF-T16}, a 16-bit float that keeps its fields fixed
and places the exponent in balanced ternary: four trits give $3^4=81$ exponent
steps and, on a ternary fabric, the exponent add is a native operation rather
than a base conversion. The mantissa stays 9 bits at every magnitude.
...
\item \textbf{The accuracy comparison is against a structural model of tekum, not
tekum itself.} tekum~\cite{tekum} is a descendant of takum~\cite{takum} adapted for balanced
ternary. Our
...
\bibitem{goldenfloat}
D.~Vasilev.
\newblock GoldenFloat: $\varphi$-structured floating-point arithmetic.
\newblock arXiv:2606.05017.

\bibitem{catalogue}
D.~Vasilev.
\newblock A bit-exact conformance catalogue for 83 numeric formats.
\newblock arXiv:2606.09686.
