#!/usr/bin/env python3
"""Five invariants across every GF-T rung, asserted rather than printed.
...
Run: python3 conformance/gft_ladder_invariants_test.py
"""
import random
import sys
...
def check_rung(N, Et, M, rng):
    ...
    for v in vals:
        ...
        assert isinstance(d, F) and d != 0, f"GF-T{N}: {v} decoded to {d!r}"
        assert (d < 0) == (v < 0), f"GF-T{N}: sign flipped on {v}"
