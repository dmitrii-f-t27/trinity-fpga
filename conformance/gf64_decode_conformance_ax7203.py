#!/usr/bin/env python3
"""gf64 decode conformance — GF(64,24,39) BIAS=8388607 → FP32. 8-byte frame."""
import serial, struct, time, random, sys, argparse
N,E,M,BIAS = 64,24,39,8388607
import os as _os, sys as _sys
_sys.path.insert(0, _os.path.dirname(_os.path.abspath(__file__)))
from gf_decode_golden import decode_to_fp32
EM = (1<<E)-1
def decode(raw):
    # Exact golden -- conformance/gf_decode_golden.py. This used to compute
    #     v = (1 + m/float(1<<M)) * (2.0 ** (e - BIAS))
    # which rounds twice (format -> double -> fp32) and, for this format's own
    # top-of-range exponents, raised OverflowError before it could round at all.
    # It also treated exp=all-ones as Inf/NaN, which only gf16 does: the .t27
    # spec, conformance/gf_ref.py and fpga/openxc7-synth/gf_adder_param.v all
    # agree that for every other GF width it is a FINITE maximum.
    return decode_to_fp32(raw, N, E, M, BIAS, "gf64")
def main():
    ap=argparse.ArgumentParser()
    ap.add_argument("--port",default="/dev/cu.usbserial-1120")
    ap.add_argument("--baud",type=int,default=160000)
    args=ap.parse.parse_args() if False else ap.parse_args()
    codes=set()
    MMAX=(1<<M)-1
    for s in (0,1):
        codes.add(s<<(N-1)); codes.add((s<<(N-1))|(EM<<M))
    codes.add((EM<<M)|1); codes.add((EM<<M)|MMAX)
    for s in (0,1):
        for mv in [1,MMAX,max(1,MMAX//2)]:
            codes.add((s<<(N-1))|mv)
    for e in [1,2,max(2,BIAS & EM) if BIAS < EM else 1,EM-1]:
        if 1<=e<EM:
            for mv in [0,MMAX,max(0,MMAX//2)]:
                for s in (0,1): codes.add((s<<(N-1))|(e<<M)|mv)
    rng=random.Random(64)
    NMAX=(1<<N)-1
    for _ in range(min(2000,NMAX)): codes.add(rng.randrange(NMAX+1))
    codes=sorted(codes)
    port=serial.Serial(args.port,args.baud,timeout=3)
    ok=0; fails=[]
    nbytes=8
    for raw in codes:
        g=decode(raw)
        b=[(raw>>(i*8))&0xFF for i in range(nbytes)]
        port.write(bytes([0xAA,0x55,0]+b+[0]))
        time.sleep(0.005)
        r=port.read(5)
        if len(r)>=5 and r[0]==0xA5:
            d=r[1]|(r[2]<<8)|(r[3]<<16)|(r[4]<<24)
            gn=(g>>23&0xFF)==0xFF and g&0x7FFFFF
            dn=(d>>23&0xFF)==0xFF and d&0x7FFFFF
            if gn and dn or d==g: ok+=1
            else:
                if len(fails)<10: fails.append(f"raw={raw:#x} g={g:#010x} d={d:#010x}")
        else:
            if len(fails)<10: fails.append(f"raw={raw:#x} noresp")
    print(f"HW RESULT: {ok}/{len(codes)} bit-exact (fails={len(codes)-ok})")
    for f in fails: print(f"  {f}",file=sys.stderr)
    port.close()
if __name__=="__main__": main()
