#!/usr/bin/env python3
"""Five invariants across every TEF rung, asserted rather than printed.
...
Run: python3 conformance/tef_ladder_invariants_test.py
"""
import random
import sys
...
def check_rung(N, Et, M, rng):
    ...
    for v in vals:
        ...
        assert isinstance(d, F) and d != 0, f"TEF{N}: {v} decoded to {d!r}"
        assert (d < 0) == (v < 0), f"TEF{N}: sign flipped on {v}"
