#!/usr/bin/env python3
"""gf512 STRICT bit-exact oracle -- GF(512, E=195, M=316, BIAS=2^194-1) -> IEEE-754 binary64.

Continuation of the gf48/gf96/gf128/gf256 strict SW-bitexact line (horizon-A).
Widest format: M=316 -> 316-52 = 264 mantissa bits rounded RNE per normal decode.
BIAS = 2^194-1; exponent range +-2^194 vs binary64 +-1023/1074 -> almost all codes
map to +/-inf or +/-0; only e ~= BIAS yields finite nonzero output.

Three independent witnesses (A==B==C over representative+boundary+random sweep):
  (A) exact mpmath mpf (dps=400 >> 316 bits) -> binary64 via frexp+scaling+RNE.
  (B) pure integer field-construct, guard/sticky for 316->52.
  (C) RTL gf512_decode_fp64.v via iverilog (fixed-width, catches width bugs).

Theoretical-only format (GF512 ~401% of XC7A200T, COMPLETE_LUT_TABLE): the
SW-bitexact decode is provable but it can never be Tier-E (silicon).
...
mpmath.mp.dps = 400   # >> 316 mantissa bits -> every dyadic input value is exact

N, E, M, BIAS = 512, 195, 316, (1 << 194) - 1     # BIAS = 2^194 - 1
EMASK = (1 << E) - 1
MMASK = (1 << M) - 1
EMAX = EMASK
...
def witness_B(raw):
    ...
    if sh > sigbits:                                # sh >= 318 -> underflow to 0
        return s << 63
    mant = full_sig >> sh
...
def main():
    ...
    for raw in codes:
        ...
        if not ok:
            if mism_ab < 20:
                print(f"  A!=B raw={raw:#0130x} A={a:#018x} B={b:#018x}", file=sys.stderr)
            mism_ab += 1
        dump.append((raw, a))
    print(f"WITNESS CROSS-CHECK (A mpmath-exact vs B integer-construct): "
          f"{len(codes)-mism_ab}/{len(codes)} agree (mismatch={mism_ab})")
    out_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "gf512_vectors.hex")
    with open(out_path, "w") as f:
        for raw, exp in dump:
            f.write(f"{raw:0128x} {exp:016x}\n")     # 512-bit raw = 128 nybbles
    print(f"Wrote {len(dump)} vectors to gf512_vectors.hex")
