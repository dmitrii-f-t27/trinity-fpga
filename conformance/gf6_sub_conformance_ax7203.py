#!/usr/bin/env python3
# gf6_sub_conformance_ax7203.py — GF6 SUB compute-conformance on AX7203.
# SUB(a,b) = ADD(a,-b). The flashed design (fpga/vivado/gf6_sub_ax7203.v) flips
# b's sign bit (bit 5) then runs the conformant gf_adder_param #(2,3) — the SAME
# adder that is formal-PROVEN (GF6 all 4096 pairs) + silicon-PROVEN for ADD.
# Golden = gf_ref.gf_add(GF6, a, b ^ 0x20) (canonical Fraction oracle).
#
#   self-test:   python3 gf6_sub_conformance_ax7203.py --self-test
#   on hardware: python3 gf6_sub_conformance_ax7203.py --port /dev/cu.usbserial-1120 --baud 160000
import argparse, sys, os
sys.path.insert(0, os.path.dirname(os.path.abspath(__file__)))
from gf_ref import FORMATS, gf_add

GFMT = FORMATS["gf6"]          # 1S+2E+3M, bias=1, HAS_INF=0
SIGN = 1 << (GFMT.exp_bits + GFMT.mant_bits)   # sign bit = 0x20 for GF6
...
def self_test():
    T = 1 << (GFMT.exp_bits + GFMT.mant_bits + 1)   # 64 for GF6
    bad = 0
    for a in range(T):
        for b in range(T):
            r = golden_sub(a, b)
...
def run_hw(port, baud, n, exhaustive=False):
    ...
    if exhaustive:
        ...
        sample = [0x00, 0x01, 0x3F, 0x20, 0x08, 0x10, 0x30, 0x21]
