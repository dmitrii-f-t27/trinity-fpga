#!/usr/bin/env python3
# gf32_add_conformance_ax7203.py — GF32 ADD compute-conformance on AX7203.
# ADD(a,b) via gf_adder_param #(12,19), no flip. Golden = gf_ref.gf_add(GF32, a, b).
# Wider 11-byte/5-byte frame (same as gf32_sub).
...
def self_test():
    # GF32 has no Inf/NaN -> a+0=a holds for ALL inputs.
    ...
    for _ in range(3000):
        ...
        if golden_add(a, 0) != a:
            bad += 1
    spot = golden_add(0x40000000, 0)            # 2.0 + 0 -> 2.0
    print(f"self-test: a+0==a over 3000 random + corner no-crash, {bad} failures; "
          f"gf32_add(0x40000000,0)=0x{spot:08x} (expect 0x40000000)")
