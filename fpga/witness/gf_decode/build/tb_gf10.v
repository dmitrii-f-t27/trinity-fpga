...
module tb_gf10;
    ...
    gf_decode_param #(.N(10),.E(3),.M(6),.BIAS(3),.OUT_REG(0)) dut0 (
        .clk(1'b0),.rst_n(1'b1),.gf_in(raw),.fp32_out(dut),
        .is_nan_o(),.is_inf_o(),.is_zero_o(),.is_subnormal_o());
    initial begin
        fd=$fopen("/Users/playom/trinity-fpga/fpga/witness/gf_decode/build/vectors_gf10.txt","r");
        ...
        $fclose(fd);
        $display("HW RESULT: %0d/%0d bit-exact (fails=%0d) [gf10]",pass,total,fail);
        if(fail==0) $display("PASS gf10"); else $display("FAIL gf10");
        $finish;
    end
endmodule
