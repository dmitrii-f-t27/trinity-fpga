...
`timescale 1ns / 1ps
// gf10_sub_ax7203 — GF10 SUB compute-conformance (GoldenFloat10: 1S+3E+6M, HAS_INF=0).
// SUB(a,b)=ADD(a,-b): flips b's sign bit (bit 9) before gf_adder_param #(3,6).
// Golden = gf_ref.gf_add(GF10, a, b^0x200).
module gf10_sub_ax7203 (
    ...
    // GF10 SUB = ADD(a,-b): flip b's sign bit (bit 9).
    wire [9:0] add_a = op_a[9:0];
    wire [9:0] add_b = op_b[9:0] ^ 10'h200;
