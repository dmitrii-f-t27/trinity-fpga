...
`timescale 1ns / 1ps
// gf4_sub_ax7203 — GF4 SUB compute-conformance (GoldenFloat4: 1S+1E+2M).
// SUB(a,b) = ADD(a,-b): same UART/RX/TX/frame as gf4_clean, but flips b's sign
// bit before the conformant gf_adder_param #(1,2). Golden = gf_ref.gf_add(GF4, a, b^0x8).
// GF4 ADD is formal-PROVEN (all 256 pairs) + silicon-PROVEN -> SUB correct by
// reduction (b^0x8 ranges over all 4-bit values => identical input space).
module gf4_sub_ax7203 (
    ...
    // GF4 SUB = ADD(a, -b): extract low 4 bits, flip sign bit (bit 3).
    wire [3:0] add_a = op_a[3:0];
    wire [3:0] add_b = op_b[3:0] ^ 4'h8;
    wire add_in_ready, add_out_valid; wire [3:0] add_out_y;
    gf_adder_param #(.EXP_BITS(1), .MANT_BITS(2)) u_add (
