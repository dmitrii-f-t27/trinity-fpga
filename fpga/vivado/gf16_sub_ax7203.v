...
`timescale 1ns / 1ps
// gf16_sub_ax7203 — GF16 SUB compute-conformance (GoldenFloat16: 1S+6E+9M, HAS_INF=1).
// SUB(a,b) = ADD(a,-b): same UART/RX/TX/frame as gf16_clean, but flips b's sign
// bit (bit 15) before the conformant gf_adder_param #(6,9,HAS_INF=1). Golden =
// gf_ref.gf_add(GF16, a, b^0x8000). GF16 ADD is silicon-PROVEN (512/512, NaN fix)
// -> SUB correct by reduction (gf16 ADD is the HAS_INF track, not formal-PROVEN,
// so SUB rests on the same silicon evidence as gf16 ADD itself).
module gf16_sub_ax7203 (
    ...
    // GF16 SUB = ADD(a, -b): full 16-bit operands, flip sign bit (bit 15).
    wire [15:0] add_a = op_a[15:0];
    wire [15:0] add_b = op_b[15:0] ^ 16'h8000;
    wire add_in_ready, add_out_valid; wire [15:0] add_out_y;
