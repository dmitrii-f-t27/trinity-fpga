...
`timescale 1ns / 1ps
// gf12_sub_ax7203 — GF12 SUB compute-conformance (GoldenFloat12: 1S+4E+7M).
// SUB(a,b) = ADD(a,-b): same UART/RX/TX/frame as gf12_clean, but flips b's sign
// bit (bit 11) before the conformant gf_adder_param #(4,7). Golden =
// gf_ref.gf_add(GF12, a, b^0x800). GF12 ADD is formal-PROVEN (all pairs) +
// silicon-PROVEN -> SUB airtight by reduction.
module gf12_sub_ax7203 (
    ...
    // GF12 SUB = ADD(a, -b): extract low 12 bits, flip sign bit (bit 11).
    wire [11:0] add_a = op_a[11:0];
    wire [11:0] add_b = op_b[11:0] ^ 12'h800;
    wire add_in_ready, add_out_valid; wire [11:0] add_out_y;
    gf_adder_param #(.EXP_BITS(4), .MANT_BITS(7)) u_add (
