...
`timescale 1ns / 1ps
// gf8_sub_ax7203 — GF8 SUB compute-conformance (GoldenFloat8: 1S+3E+4M).
// SUB(a,b) = ADD(a, -b): reuse the conformant gf_adder_param #(3,4) with the sign
// bit of b flipped. Same frame/response protocol as gf8_clean_ax7203 (ADD):
//   TX = AA 55 a_lo a_hi b_lo b_hi <trigger>;  RX = A5 r_lo r_hi 00.
// Correctness = ADD correctness (the adder is exhaustive-sim + formal-PROVEN +
// silicon-PROVEN; SUB only adds a 1-bit XOR on b's sign, value-preserving for
// the negate). Golden = gf_ref.gf_add(a, b ^ 0x80) (conformance/gf_ref.py).
module gf8_sub_ax7203 (
    ...
    wire [7:0] add_a=op_a[7:0];
    // SUB = ADD(a, -b): flip b's sign bit (bit 7) before the conformant adder.
    wire [7:0] add_b = op_b[7:0] ^ 8'h80;
    wire add_in_ready,add_out_valid; wire [7:0] add_out_y;
    gf_adder_param #(.EXP_BITS(3), .MANT_BITS(4)) u_add (
    ...
    wire [15:0] result_y={8'b0,add_out_y};
