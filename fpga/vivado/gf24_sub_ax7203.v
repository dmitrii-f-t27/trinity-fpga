...
// =============================================================================
// gf24_sub_ax7203 — GF24 SUB HW-conformance engine (1S+9E+14M, bias=255, no Inf).
// SUB(a,b)=ADD(a,-b): identical to gf24_clean but flips b's sign bit (bit 23)
// before the conformant gf_adder_param #(9,14). Same wider 9-byte/4-byte frame:
//   Host -> FPGA: [0xAA][0x55][a_lo][a_mid][a_hi][b_lo][b_mid][b_hi][cmd]
//     op_a[23:0] = {a_hi[7:0], a_mid[7:0], a_lo[7:0]}
//   FPGA -> Host: [0xA5][r_lo][r_mid][r_hi]
// GF24 ADD is silicon-PROVEN (480/480) -> SUB correct by reduction. Golden =
// gf_ref.gf_add(GF24, a, b ^ 0x800000). Matches gf24_sub_conformance_ax7203.py.
// =============================================================================

module gf24_sub_ax7203 (
    input  wire rst_n,
    input  wire uart_rx,
    output reg  uart_tx,
    output wire [3:0] led
    ...
    // ===== GF24 SUB = ADD(a, -b): flip b's sign bit (bit 23) =====
    wire add_in_ready, add_out_valid;
    wire [23:0] add_out_y24;
    gf_adder_param #(.EXP_BITS(9), .MANT_BITS(14)) u_add (
        .clk(mclk), .rst(rst),
        .in_valid(frame_valid), .in_a(op_a[23:0]), .in_b(op_b[23:0] ^ 24'h800000),
