...
`timescale 1ns / 1ps
// gf10_mul_ax7203 — GF10 MUL compute-conformance (GoldenFloat10: 1S+3E+6M, HAS_INF=0).
// MUL(a,b) via gf_mul_param #(3,6), no flip. Golden = gf_ref.gf_mul(GF10, a, b).
module gf10_mul_ax7203 (
    ...
    // GF10 MUL (no sign flip): 10-bit operands.
    wire [9:0] mul_a = op_a[9:0];
    wire [9:0] mul_b = op_b[9:0];
    wire mul_in_ready, mul_out_valid; wire [9:0] mul_out_y;
    gf_mul_param #(.EXP_BITS(3), .MANT_BITS(6)) u_mul (
    ...
    wire [15:0] result_y = {6'b0, mul_out_y};
