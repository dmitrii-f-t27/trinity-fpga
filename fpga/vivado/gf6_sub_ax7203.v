...
`timescale 1ns / 1ps
// gf6_sub_ax7203 — GF6 SUB compute-conformance (GoldenFloat6: 1S+2E+3M).
// SUB(a,b) = ADD(a,-b): same UART/RX/TX/frame as gf6_clean, but flips b's sign
// bit before the conformant gf_adder_param #(2,3). Golden = gf_ref.gf_add(GF6, a, b^0x20).
// GF6 ADD is formal-PROVEN (all 4096 pairs) + silicon-PROVEN -> SUB correct by
// reduction (b^0x20 ranges over all 6-bit values => identical input space).
module gf6_sub_ax7203 (
    ...
    // GF6 SUB = ADD(a, -b): extract low 6 bits, flip sign bit (bit 5).
    wire [5:0] add_a = op_a[5:0];
    wire [5:0] add_b = op_b[5:0] ^ 6'h20;
