...
`timescale 1ns / 1ps
// gf14_sub_ax7203 — GF14 SUB compute-conformance (GoldenFloat14: 1S+5E+8M, HAS_INF=0).
// SUB(a,b)=ADD(a,-b): same 16-bit UART/frame as gf14_add, flips b's sign bit (bit 13)
// before gf_adder_param #(5,8). Golden = gf_ref.gf_add(GF14, a, b^0x2000).
module gf14_sub_ax7203 (
    ...
    // GF14 SUB = ADD(a,-b): flip b's sign bit (bit 13).
    wire [13:0] add_a = op_a[13:0];
    wire [13:0] add_b = op_b[13:0] ^ 14'h2000;
    wire add_in_ready, add_out_valid; wire [13:0] add_out_y;
    gf_adder_param #(.EXP_BITS(5), .MANT_BITS(8)) u_add (
    ...
    wire [15:0] result_y = {2'b0, add_out_y};
