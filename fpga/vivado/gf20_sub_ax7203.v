...
// =============================================================================
// gf20_sub_ax7203 — GF20 SUB HW-conformance engine (1S+7E+12M, bias=63, no Inf).
// SUB(a,b)=ADD(a,-b): identical to gf20_clean but flips b's sign bit (bit 19)
// before the conformant gf_adder_param #(7,12). Same wider 9-byte/4-byte frame:
//   Host -> FPGA: [0xAA][0x55][a_lo][a_mid][a_hi][b_lo][b_mid][b_hi][cmd]
//   FPGA -> Host: [0xA5][r_lo][r_mid][r_hi]
// GF20 ADD is silicon-PROVEN (480/480) -> SUB correct by reduction. Golden =
// gf_ref.gf_add(GF20, a, b ^ 0x80000). Matches gf20_sub_conformance_ax7203.py.
// =============================================================================

module gf20_sub_ax7203 (
    input  wire rst_n,
    input  wire uart_rx,
    output reg  uart_tx,
    output wire [3:0] led
    ...
    // ===== GF20 SUB = ADD(a, -b): flip b's sign bit (bit 19) =====
    wire add_in_ready, add_out_valid;
    wire [19:0] add_out_y20;
    gf_adder_param #(.EXP_BITS(7), .MANT_BITS(12)) u_add (
        .clk(mclk), .rst(rst),
        .in_valid(frame_valid), .in_a(op_a[19:0]), .in_b(op_b[19:0] ^ 20'h80000),
