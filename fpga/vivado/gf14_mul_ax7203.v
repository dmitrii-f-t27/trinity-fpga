...
`timescale 1ns / 1ps
// gf14_mul_ax7203 — GF14 MUL compute-conformance (GoldenFloat14: 1S+5E+8M, HAS_INF=0).
// MUL(a,b) via gf_mul_param #(5,8) (mirror of gf_adder_param), no sign flip.
// Same 16-bit UART/frame as gf14_add. Golden = gf_ref.gf_mul(GF14, a, b).
module gf14_mul_ax7203 (
    ...
    // GF14 MUL (no sign flip): 14-bit operands.
    wire [13:0] mul_a = op_a[13:0];
    wire [13:0] mul_b = op_b[13:0];
    wire mul_in_ready, mul_out_valid; wire [13:0] mul_out_y;
    gf_mul_param #(.EXP_BITS(5), .MANT_BITS(8)) u_mul (
        .clk(mclk), .rst(rst), .in_valid(frame_valid),
        .in_a(mul_a), .in_b(mul_b), .in_ready(mul_in_ready),
        .out_valid(mul_out_valid), .out_y(mul_out_y), .out_ready(1'b1));
    wire [15:0] result_y = {2'b0, mul_out_y};
