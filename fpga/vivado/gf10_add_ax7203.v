...
`timescale 1ns / 1ps
// gf10_add_ax7203 — GF10 ADD compute-conformance (GoldenFloat10: 1S+3E+6M, HAS_INF=0).
// Same 16-bit UART/frame as gf14_add, but 10-bit operands (code[9:0]), gf_adder_param #(3,6).
// Golden = gf_ref.gf_add(GF10, a, b).
module gf10_add_ax7203 (
    ...
    // GF10 ADD (no sign flip): 10-bit operands.
    wire [9:0] add_a = op_a[9:0];
    wire [9:0] add_b = op_b[9:0];
    wire add_in_ready, add_out_valid; wire [9:0] add_out_y;
    gf_adder_param #(.EXP_BITS(3), .MANT_BITS(6)) u_add (
    ...
    wire [15:0] result_y = {6'b0, add_out_y};
