...
`timescale 1ns / 1ps
// identity_echo_ax7203 — universal 16-bit passthrough for encoding conformance.
// Receives [AA 55 a_lo a_hi b_lo b_hi cmd], returns [A5 a_lo a_hi 00].
// NO FP logic — pure passthrough. Works for ALL ≤16-bit formats.
module identity_echo_ax7203 (
    ...
    reg [2:0] frm; reg [7:0] a_lo, a_hi; reg frame_valid;
    always @(posedge mclk or posedge rst) begin
        if(rst) begin frm<=0;a_lo<=0;a_hi<=0;frame_valid<=0; end
        else begin frame_valid<=0;
            if(rx_new) begin case(frm)
                3'd0: frm<=(rx_byte==8'hAA)?3'd1:3'd0;
                3'd1: frm<=(rx_byte==8'h55)?3'd2:3'd0;
                3'd2: begin a_lo<=rx_byte;frm<=3; end
                3'd3: begin a_hi<=rx_byte;frm<=4; end
                3'd4: frm<=5;  // b_lo — ignore
                3'd5: frm<=6;  // b_hi — ignore
                3'd6: begin frame_valid<=1;frm<=0; end  // cmd — trigger
            endcase end
        end
    end
    assign led[1]=frame_valid; assign led[2]=0;

    // UART TX: echo [A5 a_lo a_hi 00] on frame_valid — PURE PASSTHROUGH
    ...
    always @(posedge mclk or posedge rst) begin
        if(rst) begin responding<=0;tx_idx<=0;tcnt<=BAUD_DIV-1;tbi<=0;tsr<=10'h3FF;uart_tx<=1;
            ...
            if(frame_valid) begin tx_buf0<=8'hA5;tx_buf1<=a_lo;tx_buf2<=a_hi;tx_buf3<=8'h00;responding<=1;tx_idx<=0; end
