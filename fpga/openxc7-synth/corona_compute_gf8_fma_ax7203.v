...
`timescale 1ns / 1ps
// corona_compute_gf8_fma_ax7203 — GoldenFloat8 FMA (y = a*b + c) on AX7203.
// GF8: [S:1][E:4][M:3] = 8 bits, BIAS=7, HAS_INF=0.
//
// Frame protocol (7 bytes TX):
//   AA 55 fmt a b c trig
// Response (5 bytes RX):
//   A5 result 00 00 00

module corona_compute_gf8_fma_ax7203 (
    ...
    // ---- Frame FSM: AA 55 fmt a b c trig (7 bytes) ----
    ...
    always @(posedge mclk or posedge rst) begin
        if(rst) begin frm<=0;fmt_r<=0;a_r<=0;b_r<=0;c_r<=0;frame_valid<=0; end
            ...
            if(rx_new) begin case(frm)
                ...
                3'd3: begin a_r<=rx_byte;frm<=4; end
                3'd4: begin b_r<=rx_byte;frm<=5; end
                3'd5: begin c_r<=rx_byte;frm<=6; end
