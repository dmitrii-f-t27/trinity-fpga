...
`timescale 1ns / 1ps
// corona_compute_fp118_e8m109_fma_ax7203 — FP118_E8M109 FMA on AX7203.
module corona_compute_fp118_e8m109_fma_ax7203 (
    ...
    reg [5:0] frm; reg [7:0] fmt_r; reg [117:0] a_r,b_r,c_r; reg frame_valid;
    always @(posedge mclk or posedge rst) begin
        if(rst) begin frm<=0;fmt_r<=0;a_r<=0;b_r<=0;c_r<=0;frame_valid<=0; end
            ...
            if(rx_new) begin case(frm)
                ...
                6'd17: begin a_r[117:112]<=rx_byte;frm<=6'd18; end
                ...
                6'd32: begin b_r[117:112]<=rx_byte;frm<=6'd33; end
                ...
                6'd47: begin c_r[117:112]<=rx_byte;frm<=6'd48; end
    ...
    reg [117:0] a_reg,b_reg,c_reg; reg comp_trigger;
    wire [117:0] fmt_a=a_reg, fmt_b=b_reg, fmt_c=c_reg;
    wire f_sign_a = fmt_a[117];
    wire [7:0] f_exp_a = fmt_a[116:109];
    ...
    wire f_sign_b = fmt_b[117];
    wire [7:0] f_exp_b = fmt_b[116:109];
    wire [108:0] f_mant_b = fmt_b[108:0];
    ...
    wire [22:0] f_mant32_b = {f_mant_b, -86'b0};
    ...
    wire f_sign_c = fmt_c[117];
    wire [7:0] f_exp_c = fmt_c[116:109];
    wire [108:0] f_mant_c = fmt_c[108:0];
    ...
    wire [22:0] f_mant32_c = {f_mant_c, -86'b0};
    ...
    reg [117:0] q_result;
    always @(*) begin
        if(q_nan) q_result=118'd0;
        else if(q_zero) q_result=118'd0;
        else if(q_inf) q_result={q_sign, 8'd255, 109'd0};
        else if(q_exp >= 8'd254) q_result={q_sign, 8'd255, 109'd0};
        else if(q_exp < 8'd1) q_result={q_sign, 117'b0};
        else if(tgt_exp_s < 1) q_result={q_sign, 8'b0, q_mant[22:0]};
        else q_result={q_sign, tgt_exp_s[7:0], q_mant[22:0]};
    end
    reg [117:0] result_reg; reg result_ready;
    always @(posedge mclk or posedge rst) begin
