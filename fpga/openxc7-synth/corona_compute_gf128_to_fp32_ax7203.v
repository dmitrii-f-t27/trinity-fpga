...
`timescale 1ns / 1ps
// corona_compute_gf128_to_fp32_ax7203 — GF128 to IEEE binary32 converter.
// Input: GF128 [S:1][E:49][M:78]
// Output: FP32 [S:1][E:8][M:23]
module corona_compute_gf128_to_fp32_ax7203 (
    ...
    // ---- Frame FSM ----
    reg [4:0] frm; reg [7:0] fmt_r; reg [127:0] a_r; reg frame_valid;
    always @(posedge mclk or posedge rst) begin
        if(rst) begin frm<=0;fmt_r<=0;a_r<=0;frame_valid<=0; end
            ...
            if(rx_new) begin case(frm)
                ...
                5'd18: begin a_r[127:120]<=rx_byte;frm<=5'd19; end
    ...
    reg [127:0] a_reg; reg conv_trigger;
    wire gf_sign = a_reg[127];
    wire [48:0] gf_exp = a_reg[126:78];
    wire [77:0] gf_mant = a_reg[77:0];
    wire gf_zero = (gf_exp == 0) && (gf_mant == 0);
    wire gf_denorm = (gf_exp == 0) && (gf_mant != 0);
    wire gf_inf = (gf_exp == {49{1'b1}}) && (gf_mant == 0);
    wire gf_nan = (gf_exp == {49{1'b1}}) && (gf_mant != 0);
    wire [22:0] fp32_mant = gf_mant[22:0];
    wire signed [50:0] fp32_exp_calc = $signed(gf_exp) + 51'sd-281474976710528;
    ...
    always @(*) begin
        ...
        else if(gf_denorm) fp32_result = {gf_sign, 8'b0, {gf_mant, -55'b0}};
    ...
    // TX: buffer+mux (no conflicting NBA — fixes tx race). 5 bytes sliced from tx_load[135:0].
