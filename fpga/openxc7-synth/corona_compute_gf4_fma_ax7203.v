...
`timescale 1ns / 1ps
// corona_compute_gf4_fma_ax7203 — GoldenFloat4 FMA (y = a*b + c) on AX7203.
// GF4: [S:1][E:1][M:2] = 4 bits, BIAS=0, HAS_INF=0.
// Pipelined: gf_mul_param (1 cycle) -> gf_adder_param (1 cycle). Latency: 2 cycles.
// 1 byte(s) per operand, 7-byte frame.
//
// Frame protocol (7 bytes TX):
//   AA 55 fmt a0 b0 c0 trig
// Response (5 bytes RX):
//   A5 r0 00 00 00

module corona_compute_gf4_fma_ax7203 (
    ...
    always @(posedge mclk or posedge rst) begin
        if(rst) begin frm<=0;fmt_r<=0;a_r<=0;b_r<=0;c_r<=0;frame_valid<=0; end
            ...
            if(rx_new) begin case(frm)
                ...
                3'd4: begin b_r[3:0]<=rx_byte[3:0];frm<=3'd5; end
                3'd5: begin c_r[3:0]<=rx_byte[3:0];frm<=3'd6; end
