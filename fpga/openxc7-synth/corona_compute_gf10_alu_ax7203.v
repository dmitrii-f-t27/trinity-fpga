...
`timescale 1ns / 1ps
// corona_compute_gf10_alu_ax7203 — GoldenFloat10 ALU (ADD+MUL) on AX7203.
// Instantiates both gf_adder_param and gf_mul_param in parallel, selects via op byte.
// GF10: [S:1][E:3][M:6] = 10 bits, BIAS=3, HAS_INF=0.
//
...
module corona_compute_gf10_alu_ax7203 (
    ...
    // ---- Frame FSM ----
    reg [3:0] frm; reg [7:0] fmt_r; reg [7:0] op_r; reg [9:0] a_r, b_r; reg frame_valid;
    always @(posedge mclk or posedge rst) begin
        if(rst) begin frm<=0;fmt_r<=0;op_r<=0;a_r<=0;b_r<=0;frame_valid<=0; end
            ...
            if(rx_new) begin case(frm)
                ...
                4'd5: begin a_r[9:8]<=rx_byte[1:0];frm<=4'd6; end
                4'd6: begin b_r[7:0]<=rx_byte;frm<=4'd7; end
                4'd7: begin b_r[9:8]<=rx_byte[1:0];frm<=4'd8; end
    ...
    // ---- GF10 ADD + MUL (both compute in parallel, mux selects) ----
    reg [9:0] a_reg, b_reg;
    reg comp_trigger;
    wire add_in_ready, add_out_valid;
    wire mul_in_ready, mul_out_valid;
    wire [9:0] add_result, mul_result;

    gf_adder_param #(.EXP_BITS(3), .MANT_BITS(6), .HAS_INF(0)) u_add (
        .clk(mclk), .rst(rst),
        .in_valid(comp_trigger), .in_a(a_reg), .in_b(b_reg), .in_ready(add_in_ready),
        .out_valid(add_out_valid), .out_y(add_result), .out_ready(1'b1)
    );

    gf_mul_param #(.EXP_BITS(3), .MANT_BITS(6), .HAS_INF(0)) u_mul (
        .clk(mclk), .rst(rst),
    ...
    wire [9:0] comp_result = is_mul ? mul_result : add_result;
    ...
    always @(posedge mclk or posedge rst) begin
        if(rst) begin responding<=0;tx_idx<=0;tcnt<=BAUD_DIV-1;tbi<=0;tsr<=10'h3FF;uart_tx<=1;
            ...
            if(result_ready) begin
                tx_buf0<=8'hA5; tx_buf1<=result_reg[7:0]; tx_buf2<=result_reg[9:8]; tx_buf3<=8'h00; tx_buf4<=8'h00;
