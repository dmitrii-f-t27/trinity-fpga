...
`timescale 1ns / 1ps
// corona_compute_mxfp4_fma_ax7203 — MXFP4 FMA on AX7203.
module corona_compute_mxfp4_fma_ax7203 (
    ...
    wire m4_sign_a = fmt_a[3];
    wire [1:0] m4_exp_a = fmt_a[2:1];
    wire m4_mant_a = fmt_a[0];
    wire m4_zero_a = (fmt_a == 4'd0);
    wire m4_sub_a = (m4_exp_a == 2'd0) && (m4_mant_a == 1'b1);
    wire [7:0] m4_exp32_a = m4_sub_a ? 8'd126 : ({6'd0, m4_exp_a} + 8'd126);
    ...
    wire [1:0] m4_exp_b = fmt_b[2:1];
    wire m4_mant_b = fmt_b[0];
    wire m4_zero_b = (fmt_b == 4'd0);
    wire m4_sub_b = (m4_exp_b == 2'd0) && (m4_mant_b == 1'b1);
    wire [7:0] m4_exp32_b = m4_sub_b ? 8'd126 : ({6'd0, m4_exp_b} + 8'd126);
    ...
    wire m4_sign_c = fmt_c[3];
    wire [1:0] m4_exp_c = fmt_c[2:1];
    wire m4_mant_c = fmt_c[0];
    wire m4_zero_c = (fmt_c == 4'd0);
    wire m4_sub_c = (m4_exp_c == 2'd0) && (m4_mant_c == 1'b1);
    wire [7:0] m4_exp32_c = m4_sub_c ? 8'd126 : ({6'd0, m4_exp_c} + 8'd126);
    wire [22:0] m4_mant32_c = {m4_mant_c, 22'b0};
    reg [31:0] fp32_c;
    always @(*) begin
        if(m4_zero_c) fp32_c={m4_sign_c, 31'b0};
        else fp32_c={m4_sign_c, m4_exp32_c, m4_mant32_c};
    end
    ...
    wire [8:0] q_m4_e = {1'b0, q_exp} - 9'd126;
    reg [3:0] q_result;
    always @(*) begin
        if(q_nan) q_result=4'h0;
        else if(q_zero||q_exp<8'd127) q_result=4'h0;
        else if(q_exp>=8'd129) q_result={q_sign,2'd3,1'b0};
        else q_result={q_sign,q_m4_e[1:0],q_mant[22]};
    end
