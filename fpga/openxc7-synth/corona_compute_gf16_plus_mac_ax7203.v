...
`timescale 1ns / 1ps
// corona_compute_gf16_plus_mac_ax7203 — GF16+ MAC on AX7203.
// Chains gf_mul_param -> gf_quire_param via gf16_plus_mac core.
//
// Frame protocol (20 bytes TX):
//   AA 55 fmt op a_lo a_hi b_lo b_hi <11 pad bytes> trig
//     op[1:0]: 0=MAC, 1=MACSUB, 2=FLUSH
// Response (5 bytes RX):
//   A5 result[7:0] result[15:8] 00 00
// (MAC/MACSUB return a placeholder; only FLUSH returns the rounded accumulator.)

module corona_compute_gf16_plus_mac_ax7203 (
    ...
    // ---- Frame FSM: 20-byte frame AA 55 fmt op a[lo,hi] b[lo,hi] <11 pad> trig ----
    reg [4:0] frm;              // byte position 0..19
    reg [7:0] fmt_r; reg [1:0] op_r; reg [15:0] a_r, b_r; reg frame_valid;
    always @(posedge mclk or posedge rst) begin
        if(rst) begin frm<=0;fmt_r<=0;op_r<=0;a_r<=0;b_r<=0;frame_valid<=0; end
            ...
            if(rx_new) begin
                case(frm)
                    ...
                    5'd3:  begin op_r<=rx_byte[1:0]; frm<=5'd4; end
                    5'd4:  begin a_r[7:0]<=rx_byte;  frm<=5'd5; end
                    5'd5:  begin a_r[15:8]<=rx_byte; frm<=5'd6; end
                    5'd6:  begin b_r[7:0]<=rx_byte;  frm<=5'd7; end
                    5'd7:  begin b_r[15:8]<=rx_byte; frm<=5'd8; end
                    5'd19: begin frame_valid<=1;     frm<=5'd0; end   // trigger byte
                    default: frm<=frm+1;                                 // bytes 8..18 = padding
                endcase
            end
        end
    end
    ...
    gf16_plus_mac #(.EXP_BITS(6), .MANT_BITS(9), .HAS_INF(1)) u_comp (
        .clk(mclk), .rst(rst),
        .in_valid(comp_trigger), .in_op(op_reg), .in_a(a_reg), .in_b(b_reg),
    ...
    always @(posedge mclk or posedge rst) begin
        if(rst) begin a_reg<=0; b_reg<=0; op_reg<=0; comp_trigger<=0; end
            ...
            comp_trigger <= frame_valid & comp_in_ready;
            if(frame_valid && comp_in_ready) begin
                a_reg<=a_r; b_reg<=b_r; op_reg<=op_r;
            end
        end
    end
    ...
    always @(posedge mclk or posedge rst) begin
        if(rst) begin result_reg<=0; result_ready<=0; end
            ...
            result_ready <= comp_out_valid;
            if(comp_out_valid) result_reg <= comp_result;
        end
    end
    assign led[2] = |result_reg;

    // ---- UART TX (buffer + mux pattern) ----
