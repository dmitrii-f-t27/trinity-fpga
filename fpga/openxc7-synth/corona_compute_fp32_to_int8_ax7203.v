...
`timescale 1ns / 1ps
// corona_compute_fp32_to_int8_ax7203 — INT8 FP32_TO on AX7203.
module corona_compute_fp32_to_int8_ax7203 (
    ...
    always @(*) begin
        if(is_zero||is_nan||is_inf) int_result=0;
            ...
            int_result = sign_v ? -$signed(tmp_mant[31:0]) : $signed(tmp_mant[7:0]);
        end
    end
    always @(posedge mclk or posedge rst) begin
    ...
    always @(posedge mclk or posedge rst) begin
        if(rst) begin result_reg<=0;result_ready<=0; end
            ...
            if(conv_trigger) result_reg<={{24{1'b0}},int_result};
        end
    end
