...
`timescale 1ns / 1ps
// corona_compute_fp113_e8m104_fma_ax7203 — FP113_E8M104 FMA on AX7203.
module corona_compute_fp113_e8m104_fma_ax7203 (
    ...
    reg [5:0] frm; reg [7:0] fmt_r; reg [112:0] a_r,b_r,c_r; reg frame_valid;
    always @(posedge mclk or posedge rst) begin
        if(rst) begin frm<=0;fmt_r<=0;a_r<=0;b_r<=0;c_r<=0;frame_valid<=0; end
            ...
            if(rx_new) begin case(frm)
                ...
                6'd17: begin a_r[112:112]<=rx_byte;frm<=6'd18; end
                ...
                6'd32: begin b_r[112:112]<=rx_byte;frm<=6'd33; end
                ...
                6'd47: begin c_r[112:112]<=rx_byte;frm<=6'd48; end
    ...
    reg [112:0] a_reg,b_reg,c_reg; reg comp_trigger;
    wire [112:0] fmt_a=a_reg, fmt_b=b_reg, fmt_c=c_reg;
    wire f_sign_a = fmt_a[112];
    wire [7:0] f_exp_a = fmt_a[111:104];
    wire [103:0] f_mant_a = fmt_a[103:0];
    ...
    wire [22:0] f_mant32_a = {f_mant_a, -81'b0};
    ...
    wire f_sign_b = fmt_b[112];
    wire [7:0] f_exp_b = fmt_b[111:104];
    wire [103:0] f_mant_b = fmt_b[103:0];
    ...
    wire [22:0] f_mant32_b = {f_mant_b, -81'b0};
    ...
    wire f_sign_c = fmt_c[112];
    wire [7:0] f_exp_c = fmt_c[111:104];
    wire [103:0] f_mant_c = fmt_c[103:0];
    ...
    wire [22:0] f_mant32_c = {f_mant_c, -81'b0};
    ...
    reg [112:0] q_result;
    always @(*) begin
        if(q_nan) q_result=113'd0;
        else if(q_zero) q_result=113'd0;
        else if(q_inf) q_result={q_sign, 8'd255, 104'd0};
        else if(q_exp >= 8'd254) q_result={q_sign, 8'd255, 104'd0};
        else if(q_exp < 8'd1) q_result={q_sign, 112'b0};
        else if(tgt_exp_s < 1) q_result={q_sign, 8'b0, q_mant[22:0]};
        else q_result={q_sign, tgt_exp_s[7:0], q_mant[22:0]};
    end
    reg [112:0] result_reg; reg result_ready;
    always @(posedge mclk or posedge rst) begin
