...
`timescale 1ns / 1ps
// corona_compute_int8_alu_ax7203 — INT8 ALU on AX7203.
module corona_compute_int8_alu_ax7203 (
    ...
    wire signed [7:0] sa=a_reg, sb=b_reg;
    wire [7:0] add_r = sa + sb;
    wire [7:0] mul_r = sa * sb;
    wire and_r = a_reg & b_reg;
    wire or_r  = a_reg | b_reg;
    wire xor_r = a_reg ^ b_reg;
    wire [7:0] alu_result = (op_reg==8'h00)?add_r:(op_reg==8'h01)?mul_r:
                             (op_reg==8'h02)?{{8{and_r[0]}},and_r}:
                             (op_reg==8'h03)?{{8{or_r[0]}},or_r}:
                                             {{8{xor_r[0]}},xor_r};
    always @(posedge mclk or posedge rst) begin
    ...
    always @(posedge mclk or posedge rst) begin
        if(rst) begin result_reg<=0;result_ready<=0; end
            ...
            if(comp_trigger) result_reg<={{24{1'b0}},alu_result};
        end
    end
