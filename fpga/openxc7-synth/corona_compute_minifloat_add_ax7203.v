...
`timescale 1ns / 1ps
// corona_compute_minifloat_add_ax7203 — MINIFLOAT ADD on AX7203.
module corona_compute_minifloat_add_ax7203 (
    ...
    wire [7:0] exp32_b = sub_b?8'd125:({5'd0,eb_b}+8'd124);
    wire [22:0] mant32_b = {mb_b,19'b0};
    ...
    gf_adder_param #(.EXP_BITS(3), .MANT_BITS(4), .HAS_INF(1)) u_comp (
    ...
    // Quantize FP32 → minifloat
    wire cs=comp_result[31]; wire [7:0] ce=comp_result[30:23]; wire [22:0] cm=comp_result[22:0];
    wire [8:0] mf_e = {1'b0,ce} - 9'd124;
    reg [7:0] mf_result;
    always @(*) begin
        if(comp_result==32'h0) mf_result=8'h00;
        else if(comp_result==32'h7FC00000) mf_result=8'h7F;
        else if(comp_result[30:23]>=8'd131) mf_result={cs,3'd6,4'd8};
    ...
    always @(posedge mclk or posedge rst) begin
        if(rst) begin result_reg<=0;result_ready<=0; end
            ...
            if(comp_ovld) result_reg<={24'b0,mf_result};
        end
    end
