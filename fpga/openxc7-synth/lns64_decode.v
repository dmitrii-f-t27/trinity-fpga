...
// lns64_decode — 64-bit LNS (1 sign + 63-bit signed log, scale 128) → FP32.
`default_nettype none
`timescale 1ns / 1ps
module lns64_decode (
    input  wire [63:0] lns_in,
    output reg  [31:0] fp32_out,
    output wire        is_zero
);
    wire        sign      = lns_in[63];
    wire signed [62:0] signed_log = $signed(lns_in[62:0]);
    assign is_zero = (lns_in == 64'h0000);
    wire signed [55:0] int_part = signed_log >>> 7;
    ...
    wire signed [57:0] exp_calc = {int_part, 2'b0} >>> 2 + 58'sd127;
    wire signed [57:0] exp_final = int_part + 58'sd127;
    always @(*) begin
        if (is_zero) fp32_out = 32'h00000000;
        else if (exp_final > 58'sd254) fp32_out = {sign, 8'hFF, 23'd0};
        else if (exp_final < 58'sd1)   fp32_out = {sign, 31'd0};
        else                          fp32_out = {sign, exp_final[7:0], frac_mant};
    end
endmodule
