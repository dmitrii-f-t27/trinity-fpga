...
`timescale 1ns / 1ps
// corona_compute_fp110_e8m101_fma_ax7203 — FP110_E8M101 FMA on AX7203.
module corona_compute_fp110_e8m101_fma_ax7203 (
    ...
    reg [5:0] frm; reg [7:0] fmt_r; reg [109:0] a_r,b_r,c_r; reg frame_valid;
    always @(posedge mclk or posedge rst) begin
        if(rst) begin frm<=0;fmt_r<=0;a_r<=0;b_r<=0;c_r<=0;frame_valid<=0; end
            ...
            if(rx_new) begin case(frm)
                ...
                6'd16: begin a_r[109:104]<=rx_byte;frm<=6'd17; end
                6'd17: begin b_r[7:0]<=rx_byte;frm<=6'd18; end
                6'd18: begin b_r[15:8]<=rx_byte;frm<=6'd19; end
                6'd19: begin b_r[23:16]<=rx_byte;frm<=6'd20; end
                6'd20: begin b_r[31:24]<=rx_byte;frm<=6'd21; end
                6'd21: begin b_r[39:32]<=rx_byte;frm<=6'd22; end
                6'd22: begin b_r[47:40]<=rx_byte;frm<=6'd23; end
                6'd23: begin b_r[55:48]<=rx_byte;frm<=6'd24; end
                6'd24: begin b_r[63:56]<=rx_byte;frm<=6'd25; end
                6'd25: begin b_r[71:64]<=rx_byte;frm<=6'd26; end
                6'd26: begin b_r[79:72]<=rx_byte;frm<=6'd27; end
                6'd27: begin b_r[87:80]<=rx_byte;frm<=6'd28; end
                6'd28: begin b_r[95:88]<=rx_byte;frm<=6'd29; end
                6'd29: begin b_r[103:96]<=rx_byte;frm<=6'd30; end
                6'd30: begin b_r[109:104]<=rx_byte;frm<=6'd31; end
                6'd31: begin c_r[7:0]<=rx_byte;frm<=6'd32; end
                6'd32: begin c_r[15:8]<=rx_byte;frm<=6'd33; end
                6'd33: begin c_r[23:16]<=rx_byte;frm<=6'd34; end
                6'd34: begin c_r[31:24]<=rx_byte;frm<=6'd35; end
                6'd35: begin c_r[39:32]<=rx_byte;frm<=6'd36; end
                6'd36: begin c_r[47:40]<=rx_byte;frm<=6'd37; end
                6'd37: begin c_r[55:48]<=rx_byte;frm<=6'd38; end
                6'd38: begin c_r[63:56]<=rx_byte;frm<=6'd39; end
                6'd39: begin c_r[71:64]<=rx_byte;frm<=6'd40; end
                6'd40: begin c_r[79:72]<=rx_byte;frm<=6'd41; end
                6'd41: begin c_r[87:80]<=rx_byte;frm<=6'd42; end
                6'd42: begin c_r[95:88]<=rx_byte;frm<=6'd43; end
                6'd43: begin c_r[103:96]<=rx_byte;frm<=6'd44; end
                6'd44: begin c_r[109:104]<=rx_byte;frm<=6'd45; end
                6'd45: begin frame_valid<=1;frm<=0; end
            endcase end
        end
    end
    assign led[1]=frame_valid;

    reg [109:0] a_reg,b_reg,c_reg; reg comp_trigger;
    wire [109:0] fmt_a=a_reg, fmt_b=b_reg, fmt_c=c_reg;
    wire f_sign_a = fmt_a[109];
    wire [7:0] f_exp_a = fmt_a[108:101];
    wire [100:0] f_mant_a = fmt_a[100:0];
    ...
    wire signed [17:0] f_de_a = $signed({1'b0, f_exp_a}) - 18'sd127 + 18'sd127;
    wire [7:0] f_exp32_a = (f_de_a > 18'sd254) ? 8'd254 : (f_de_a < 0) ? 8'd0 : f_de_a[7:0];
    wire [22:0] f_mant32_a = {f_mant_a, -78'b0};
    ...
    wire f_sign_b = fmt_b[109];
    wire [7:0] f_exp_b = fmt_b[108:101];
    wire [100:0] f_mant_b = fmt_b[100:0];
    wire f_zero_b = (f_exp_b == 0) && ((f_mant_b == 0));
    wire f_inf_b = (f_exp_b == 255) && ((f_mant_b == 0));
    wire f_nan_b = (f_exp_b == 255) && ((f_mant_b != 0));
    wire f_sub_b = (f_exp_b == 0) && ((f_mant_b != 0));
    wire signed [17:0] f_de_b = $signed({1'b0, f_exp_b}) - 18'sd127 + 18'sd127;
    wire [7:0] f_exp32_b = (f_de_b > 18'sd254) ? 8'd254 : (f_de_b < 0) ? 8'd0 : f_de_b[7:0];
    wire [22:0] f_mant32_b = {f_mant_b, -78'b0};
    ...
    wire f_sign_c = fmt_c[109];
    wire [7:0] f_exp_c = fmt_c[108:101];
    wire [100:0] f_mant_c = fmt_c[100:0];
    wire f_zero_c = (f_exp_c == 0) && ((f_mant_c == 0));
    wire f_inf_c = (f_exp_c == 255) && ((f_mant_c == 0));
    wire f_nan_c = (f_exp_c == 255) && ((f_mant_c != 0));
    wire f_sub_c = (f_exp_c == 0) && ((f_mant_c != 0));
    wire signed [17:0] f_de_c = $signed({1'b0, f_exp_c}) - 18'sd127 + 18'sd127;
    wire [7:0] f_exp32_c = (f_de_c > 18'sd254) ? 8'd254 : (f_de_c < 0) ? 8'd0 : f_de_c[7:0];
    wire [22:0] f_mant32_c = {f_mant_c, -78'b0};
    wire [22:0] f_mant32_norm_c = f_mant_c;
    reg [31:0] fp32_c;
    always @(*) begin
        if(f_zero_c) fp32_c={f_sign_c, 31'b0};
        else if(f_inf_c) fp32_c=f_sign_c?32'hFF800000:32'h7F800000;
        else if(f_nan_c) fp32_c=32'h7FC00000;
        else if(f_sub_c) fp32_c={f_sign_c, 8'd1, f_mant32_norm_c};
    ...
    wire signed [17:0] tgt_exp_s = $signed({1'b0, q_exp}) - 18'sd127 + 18'sd127;
    reg [109:0] q_result;
    always @(*) begin
        if(q_nan) q_result=110'd0;
        else if(q_zero) q_result=110'd0;
        else if(q_inf) q_result={q_sign, 8'd255, 101'd0};
        else if(q_exp >= 8'd254) q_result={q_sign, 8'd255, 101'd0};
        else if(q_exp < 8'd1) q_result={q_sign, 109'b0};
        else if(tgt_exp_s < 1) q_result={q_sign, 8'b0, q_mant[22:0]};
        else q_result={q_sign, tgt_exp_s[7:0], q_mant[22:0]};
    end
    reg [109:0] result_reg; reg result_ready;
    always @(posedge mclk or posedge rst) begin
    ...
    localparam [3:0] TX_LEN = 15;
    // TX: buffer+mux (no conflicting NBA — fixes tx race). 15 bytes sliced from tx_load[119:0].
    wire [119:0] tx_load = {result_reg, 8'hA5};
    reg responding; reg [3:0] tx_idx; reg [7:0] tx_buf0, tx_buf1, tx_buf2, tx_buf3, tx_buf4, tx_buf5, tx_buf6, tx_buf7, tx_buf8, tx_buf9, tx_buf10, tx_buf11, tx_buf12, tx_buf13, tx_buf14;
    reg [8:0] tcnt; reg [3:0] tbi; reg [9:0] tsr;
    always @(posedge mclk or posedge rst) begin
        if(rst) begin responding<=0;tx_idx<=0;tcnt<=BAUD_DIV-1;tbi<=0;tsr<=10'h3FF;uart_tx<=1;
            tx_buf0<=8'hFF; tx_buf1<=8'hFF; tx_buf2<=8'hFF; tx_buf3<=8'hFF; tx_buf4<=8'hFF; tx_buf5<=8'hFF; tx_buf6<=8'hFF; tx_buf7<=8'hFF; tx_buf8<=8'hFF; tx_buf9<=8'hFF; tx_buf10<=8'hFF; tx_buf11<=8'hFF; tx_buf12<=8'hFF; tx_buf13<=8'hFF; tx_buf14<=8'hFF; end
        else begin uart_tx<=tsr[0];
            if(result_ready) begin
                tx_buf0<=tx_load[7:0]; tx_buf1<=tx_load[15:8]; tx_buf2<=tx_load[23:16]; tx_buf3<=tx_load[31:24]; tx_buf4<=tx_load[39:32]; tx_buf5<=tx_load[47:40]; tx_buf6<=tx_load[55:48]; tx_buf7<=tx_load[63:56]; tx_buf8<=tx_load[71:64]; tx_buf9<=tx_load[79:72]; tx_buf10<=tx_load[87:80]; tx_buf11<=tx_load[95:88]; tx_buf12<=tx_load[103:96]; tx_buf13<=tx_load[111:104]; tx_buf14<=tx_load[119:112]; responding<=1; tx_idx<=0;
            end
            if(tcnt==0) begin tcnt<=BAUD_DIV-1;
                if(tbi==9) begin tbi<=0;
                    if(responding) begin
                        ...
                        endcase
                        if(tx_idx==14) responding<=0; else tx_idx<=tx_idx+1;
