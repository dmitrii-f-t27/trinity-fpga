...
`timescale 1ns / 1ps
// corona_decode_mxgf4_ax7203 — IEEE 754 mxgf4 (half) decode on AX7203.
// mxgf4_decode is authored in-repo and EXHAUSTIVELY verified (65536/65536 vs
// the C-library mxgf4 via struct 'e'). Single-decoder build: CFGMCLK clock,
...
module corona_decode_mxgf4_ax7203 (
    ...
    wire [31:0] result;
    gf_decode_param #(.N(4), .E(1), .M(2), .BIAS(0), .HAS_INF(0), .OUT_REG(0)) u_dec (.clk(1'b0), .rst_n(1'b1), .gf_in(code_r[3:0]), .fp32_out(result), .is_nan_o(), .is_inf_o(), .is_zero_o(), .is_subnormal_o());
    assign led[2] = |result;
