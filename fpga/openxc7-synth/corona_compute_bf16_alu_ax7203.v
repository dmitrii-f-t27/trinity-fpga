...
`timescale 1ns / 1ps
// corona_compute_bf16_alu_ax7203 — BF16 ALU on AX7203.
module corona_compute_bf16_alu_ax7203 (
    ...
    gf_adder_param #(.EXP_BITS(8),.MANT_BITS(7),.HAS_INF(1)) u_add (
    ...
    gf_mul_param #(.EXP_BITS(8),.MANT_BITS(7),.HAS_INF(1)) u_mul (
        .clk(mclk),.rst(rst),.in_valid(comp_trigger),.in_a(a_reg),.in_b(b_reg),
