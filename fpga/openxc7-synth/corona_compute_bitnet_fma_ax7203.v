...
`timescale 1ns / 1ps
// corona_compute_bitnet_fma_ax7203 — BitNet FMA on AX7203.
module corona_compute_bitnet_fma_ax7203 (
    ...
    always @(*) begin
        casez(a_reg)
            2'd0: fp32_a = 32'h00000000;
            2'd1: fp32_a = 32'h3F800000;
            2'd2: fp32_a = 32'hBF800000;
            2'd3: fp32_a = 32'h00000000;
            default: fp32_a = 32'h00000000;
        endcase
        casez(b_reg)
            2'd0: fp32_b = 32'h00000000;
            2'd1: fp32_b = 32'h3F800000;
            2'd2: fp32_b = 32'hBF800000;
            2'd3: fp32_b = 32'h00000000;
            default: fp32_b = 32'h00000000;
        endcase
        casez(c_reg)
            2'd0: fp32_c = 32'h00000000;
            2'd1: fp32_c = 32'h3F800000;
            2'd2: fp32_c = 32'hBF800000;
            2'd3: fp32_c = 32'h00000000;
    ...
    reg [1:0] code_result;
    always @(*) begin
        if(add_result == 32'h00000000) code_result = 2'd0;
        else if(add_result[31]) begin
            if(add_result >= 32'hBF000000) code_result = 2'd2;  // <= -0.5 → -1
            else code_result = 2'd0;                     // > -0.5 → 0
        end else begin
            if(add_result >= 32'h3F000000) code_result = 2'd1;   // >= 0.5 → +1
            else code_result = 2'd0;                      // < 0.5 → 0
        end
    end
    ...
    always @(posedge mclk or posedge rst) begin
        if(rst) begin result_reg<=0;result_ready<=0; end
            ...
            if(add_ovld) result_reg<={30'b0,code_result};
        end
    end
