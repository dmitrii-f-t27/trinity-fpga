...
`timescale 1ns / 1ps
// corona_compute_fp32_to_gf16_ax7203 — IEEE binary32 to GF16 converter.
// Input: FP32 [S:1][E:8][M:23] (bias=127)
// Output: GF16 [S:1][E:6][M:9] (bias=31, HAS_INF=1)
// Mantissa truncation (no rounding — faithful 1ulp).
// Overflow → Inf, underflow → zero.
//
// Frame protocol (7 bytes TX):
//   AA 55 fmt a0 a1 a2 a3 trig
// Response (5 bytes RX):
//   A5 r_lo r_hi 00 00

module corona_compute_fp32_to_gf16_ax7203 (
    ...
    // ---- Frame FSM: AA 55 fmt a0 a1 a2 a3 trig (7 bytes) ----
    ...
    always @(posedge mclk or posedge rst) begin
        if(rst) begin frm<=0;fmt_r<=0;a_r<=0;frame_valid<=0; end
            ...
            if(rx_new) begin case(frm)
                ...
                3'd3: begin a_r[7:0]<=rx_byte;frm<=4; end
                3'd4: begin a_r[15:8]<=rx_byte;frm<=5; end
                3'd5: begin a_r[23:16]<=rx_byte;frm<=6; end
                3'd6: begin a_r[31:24]<=rx_byte;frm<=7; end
    ...
    // GF16 exp = fp_exp - 96 (= fp_exp - 127 + 31)
    wire signed [9:0] gf_exp_calc = $signed({2'b0, fp_exp}) - 10'sd96;

    // Overflow: gf_exp > 62 → Inf
    wire overflow  = (gf_exp_calc > 10'sd62);
    // Underflow: gf_exp < 1 → zero (denormals too small for GF16)
    wire underflow = (gf_exp_calc < 10'sd1);

    // Truncate mantissa from 23 bits to 9 bits (take top 9)
    wire [8:0] gf_mant_trunc = fp_mant[22:14];

    reg [15:0] gf16_result;
    always @(*) begin
        if (fp_nan)
            gf16_result = {1'b0, 6'b111111, 8'b0, 1'b1};           // canonical qNaN
        else if (fp_inf || overflow)
            gf16_result = {fp_sign, 6'b111111, 9'b0};              // Inf (or overflow→Inf)
        else if (fp_zero || fp_denorm || underflow)
            gf16_result = {fp_sign, 6'b0, 9'b0};                   // zero
        else
            gf16_result = {fp_sign, gf_exp_calc[5:0], gf_mant_trunc}; // normal
    end

    always @(posedge mclk or posedge rst) begin
        if(rst) begin a_reg<=0; conv_trigger<=0; end
        else begin
            conv_trigger <= frame_valid;
    ...
    always @(posedge mclk or posedge rst) begin
        if(rst) begin result_reg<=0; result_ready<=0; end
            ...
            result_ready <= conv_trigger;
            if(conv_trigger) result_reg <= gf16_result;
        end
    end
    assign led[2] = |result_reg;

    // ---- UART TX (5 bytes: A5 r_lo r_hi 00 00) ----
