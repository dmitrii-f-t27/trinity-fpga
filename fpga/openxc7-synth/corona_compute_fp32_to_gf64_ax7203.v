...
`timescale 1ns / 1ps
// corona_compute_fp32_to_gf64_ax7203 — IEEE binary32 to GF64 converter.
// Output: GF64 [S:1][E:24][M:39]
module corona_compute_fp32_to_gf64_ax7203 (
    ...
    wire signed [25:0] gf_exp_calc = $signed({2'b0, fp_exp}) + 26'sd8388480;
    wire overflow = (gf_exp_calc > 26'sd16777214);
    wire underflow = (gf_exp_calc < 26'sd1);
    wire [38:0] gf_mant_trunc = fp_mant[22:-16];
    reg [63:0] gf_result;
    always @(*) begin
        if(fp_nan) gf_result = {1'b0, {24{1'b1}}, {38{1'b0}}, 1'b1};
        else if(fp_inf || overflow) gf_result = {fp_sign, {24{1'b1}}, {39{1'b0}}};
        else if(fp_zero || fp_denorm || underflow) gf_result = {fp_sign, {24{1'b0}}, {39{1'b0}}};
        else gf_result = {fp_sign, gf_exp_calc[23:0], gf_mant_trunc};
    end
    always @(posedge mclk or posedge rst) begin
    ...
    // ---- UART TX (shift-register, 9 bytes) ----
