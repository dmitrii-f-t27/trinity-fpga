...
`timescale 1ns / 1ps
// corona_compute_tf32_fma_ax7203 — TF32 FMA on AX7203.
module corona_compute_tf32_fma_ax7203 (
    ...
    wire mul_irdy,mul_ovld; wire [18:0] mul_result;
    wire add_irdy,add_ovld; wire [18:0] add_result;
    gf_mul_param #(.EXP_BITS(8),.MANT_BITS(10),.HAS_INF(1)) u_mul (
    ...
    gf_adder_param #(.EXP_BITS(8),.MANT_BITS(10),.HAS_INF(1)) u_add (
        .clk(mclk),.rst(rst),.in_valid(mul_ovld),.in_a(mul_result),.in_b(c_reg),
    ...
    always @(posedge mclk or posedge rst) begin
        if(rst) begin result_reg<=0;result_ready<=0; end
            ...
            if(add_ovld) result_reg<={{13{1'b0}},add_result};
        end
    end
