...
`timescale 1ns / 1ps
// corona_compute_int8_to_fp32_ax7203 — signed INT8 to IEEE FP32 conversion.
module corona_compute_int8_to_fp32_ax7203 (
    ...
    always @(posedge mclk or posedge rst) begin
        if(rst) begin frm<=0;fmt_r<=0;a_r<=0;frame_valid<=0; end
            ...
            if(rx_new) begin case(frm)
                ...
                3'd3: begin a_r<=rx_byte;frm<=3'd4; end
    ...
    wire signed [7:0] sint = a_reg;
    wire neg = sint[7];
    wire [6:0] absval = neg ? ~a_reg[6:0] + 7'd1 : a_reg[6:0];
    reg [31:0] fp32_result;
    always @(*) begin
        if(sint == 0) fp32_result = 32'h00000000;
        else begin
            casez (absval)
                7'b1??????: fp32_result = {neg, 8'd134, absval[5:0], 17'b0};
                7'b01?????: fp32_result = {neg, 8'd133, absval[4:0], 18'b0};
                7'b001????: fp32_result = {neg, 8'd132, absval[3:0], 19'b0};
                7'b0001???: fp32_result = {neg, 8'd131, absval[2:0], 20'b0};
                7'b00001??: fp32_result = {neg, 8'd130, absval[1:0], 21'b0};
                7'b000001?: fp32_result = {neg, 8'd129, absval[0],   22'b0};
                default:    fp32_result = {neg, 8'd128, 23'b0};
            endcase
        end
    end
    always @(posedge mclk or posedge rst) begin
