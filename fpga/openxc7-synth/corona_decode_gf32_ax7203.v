...
module corona_decode_gf32_ax7203 (
    ...
    wire [31:0] result;
    wire        is_zero;
    gf_decode_param #(.N(32), .E(12, .HAS_INF(0)), .M(19), .BIAS(2047), .OUT_REG(1)) u_dec (.clk(mclk), .rst_n(~rst), .gf_in(code_r[31:0]), .fp32_out(result), .is_nan_o(), .is_inf_o(), .is_zero_o(), .is_subnormal_o());
    assign led[2] = |result;
