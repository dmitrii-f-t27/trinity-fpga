...
`timescale 1ns / 1ps
// corona_compute_fp99_e8m90_fma_ax7203 — FP99_E8M90 FMA on AX7203.
module corona_compute_fp99_e8m90_fma_ax7203 (
    ...
    reg [5:0] frm; reg [7:0] fmt_r; reg [98:0] a_r,b_r,c_r; reg frame_valid;
    always @(posedge mclk or posedge rst) begin
        if(rst) begin frm<=0;fmt_r<=0;a_r<=0;b_r<=0;c_r<=0;frame_valid<=0; end
            ...
            if(rx_new) begin case(frm)
                ...
                6'd15: begin a_r[98:96]<=rx_byte;frm<=6'd16; end
                ...
                6'd28: begin b_r[98:96]<=rx_byte;frm<=6'd29; end
                ...
                6'd41: begin c_r[98:96]<=rx_byte;frm<=6'd42; end
    ...
    reg [98:0] a_reg,b_reg,c_reg; reg comp_trigger;
    wire [98:0] fmt_a=a_reg, fmt_b=b_reg, fmt_c=c_reg;
    wire f_sign_a = fmt_a[98];
    wire [7:0] f_exp_a = fmt_a[97:90];
    wire [89:0] f_mant_a = fmt_a[89:0];
    ...
    wire [22:0] f_mant32_a = {f_mant_a, -67'b0};
    ...
    wire f_sign_b = fmt_b[98];
    wire [7:0] f_exp_b = fmt_b[97:90];
    wire [89:0] f_mant_b = fmt_b[89:0];
    ...
    wire [22:0] f_mant32_b = {f_mant_b, -67'b0};
    ...
    wire f_sign_c = fmt_c[98];
    wire [7:0] f_exp_c = fmt_c[97:90];
    wire [89:0] f_mant_c = fmt_c[89:0];
    ...
    wire [22:0] f_mant32_c = {f_mant_c, -67'b0};
    ...
    reg [98:0] q_result;
    always @(*) begin
        if(q_nan) q_result=99'd0;
        else if(q_zero) q_result=99'd0;
        else if(q_inf) q_result={q_sign, 8'd255, 90'd0};
        else if(q_exp >= 8'd254) q_result={q_sign, 8'd255, 90'd0};
        else if(q_exp < 8'd1) q_result={q_sign, 98'b0};
        else if(tgt_exp_s < 1) q_result={q_sign, 8'b0, q_mant[22:0]};
        else q_result={q_sign, tgt_exp_s[7:0], q_mant[22:0]};
    end
    reg [98:0] result_reg; reg result_ready;
    always @(posedge mclk or posedge rst) begin
