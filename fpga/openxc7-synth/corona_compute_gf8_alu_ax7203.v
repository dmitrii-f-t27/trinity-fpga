...
`timescale 1ns / 1ps
// corona_compute_gf8_alu_ax7203 — GoldenFloat8 ALU (ADD+MUL) on AX7203.
...
module corona_compute_gf8_alu_ax7203 (
    ...
    // ---- Frame FSM ----
    reg [2:0] frm; reg [7:0] fmt_r; reg [7:0] op_r; reg [7:0] a_r, b_r; reg frame_valid;
    always @(posedge mclk or posedge rst) begin
    ...
    // ---- GF8 ADD + MUL (both compute in parallel, mux selects) ----
    reg [7:0] a_reg, b_reg;
    reg comp_trigger;
    wire add_in_ready, add_out_valid;
    wire mul_in_ready, mul_out_valid;
    wire [7:0] add_result, mul_result;
    ...
    wire [7:0] comp_result = is_mul ? mul_result : add_result;
