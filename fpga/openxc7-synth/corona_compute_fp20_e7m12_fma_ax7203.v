...
`timescale 1ns / 1ps
// corona_compute_fp20_e7m12_fma_ax7203 — FP20_E7M12 FMA on AX7203.
module corona_compute_fp20_e7m12_fma_ax7203 (
    ...
    wire signed [16:0] f_de_a = $signed({1'b0, f_exp_a}) - 17'sd127 + 17'sd127;
    ...
    wire signed [16:0] f_de_b = $signed({1'b0, f_exp_b}) - 17'sd127 + 17'sd127;
    ...
    wire signed [16:0] f_de_c = $signed({1'b0, f_exp_c}) - 17'sd127 + 17'sd127;
    ...
    wire signed [16:0] tgt_exp_s = $signed({1'b0, q_exp}) - 17'sd127 + 17'sd127;
    reg [19:0] q_result;
    always @(*) begin
        if(q_nan) q_result=20'd0;
        else if(q_zero) q_result=20'd0;
        else if(q_inf) q_result={q_sign, 7'd127, 12'd0};
        else if(q_exp >= 8'd128) q_result={q_sign, 7'd127, 12'd0};
        else if(q_exp < 8'd1) q_result={q_sign, 19'b0};
        else if(tgt_exp_s < 1) q_result={q_sign, 7'b0, q_mant[22:11]};
        else q_result={q_sign, tgt_exp_s[6:0], q_mant[22:11]};
    end
