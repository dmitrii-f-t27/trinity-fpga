...
`timescale 1ns / 1ps
// corona_compute_int4_to_fp32_ax7203 — signed INT4 to IEEE FP32 conversion.
module corona_compute_int4_to_fp32_ax7203 (
    ...
    wire signed [3:0] sint = a_reg[3:0];
    wire neg = sint[3];
    wire [2:0] absval = neg ? ~a_reg[2:0] + 3'd1 : a_reg[2:0];
    reg [31:0] fp32_result;
    always @(*) begin
        if(sint == 0) fp32_result = 32'h00000000;
        else begin
            casez (absval)
                3'b1??: fp32_result = {neg, 8'd131, absval[1:0], 21'b0};
                3'b01?: fp32_result = {neg, 8'd130, absval[0],   22'b0};
                default: fp32_result = {neg, 8'd129, 23'b0};
            endcase
        end
    end
    always @(posedge mclk or posedge rst) begin
