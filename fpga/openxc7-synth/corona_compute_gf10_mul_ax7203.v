...
`timescale 1ns / 1ps
// corona_compute_gf10_mul_ax7203 — GoldenFloat10 MUL on AX7203.
// GF10: [S:1][E:3][M:6] = 10 bits, BIAS=3, HAS_INF=0.
module corona_compute_gf10_mul_ax7203 (
    ...
    always @(posedge mclk or posedge rst) begin
        if(rst) begin frm<=0;fmt_r<=0;a_r<=0;b_r<=0;frame_valid<=0; end
            ...
            if(rx_new) begin case(frm)
                ...
                3'd6: begin b_r[9:8]<=rx_byte[1:0];frm<=3'd7; end
    ...
    wire comp_in_ready, comp_out_valid; wire [9:0] comp_result;
    gf_mul_param #(.EXP_BITS(3), .MANT_BITS(6), .HAS_INF(0)) u_comp (
        .clk(mclk), .rst(rst),
