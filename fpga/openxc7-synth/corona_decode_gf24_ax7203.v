...
module corona_decode_gf24_ax7203 (
    ...
    wire [31:0] result;
    wire        is_zero;
    gf_decode_param #(.N(24), .E(9, .HAS_INF(0)), .M(14), .BIAS(255), .OUT_REG(1)) u_dec (.clk(mclk), .rst_n(~rst), .gf_in(code_r[23:0]), .fp32_out(result), .is_nan_o(), .is_inf_o(), .is_zero_o(), .is_subnormal_o());
    assign led[2] = |result;
