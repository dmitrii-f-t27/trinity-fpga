...
`timescale 1ns / 1ps
// corona_compute_bcd_fma_ax7203 — BCD FMA on AX7203.
module corona_compute_bcd_fma_ax7203 (
    ...
    wire [3:0] bcd_t_c = fmt_c[7:4];
    wire [3:0] bcd_o_c = fmt_c[3:0];
    wire [6:0] bcd_int_c = bcd_t_c * 5'd10 + {3'b0, bcd_o_c};
    wire bcd_zero_c = (bcd_int_c == 7'd0);
    reg [2:0] bcd_lop_c;
    always @(*) begin
        casez(bcd_int_c)
            7'b1??????: bcd_lop_c=3'd6;
            7'b01?????: bcd_lop_c=3'd5;
            7'b001????: bcd_lop_c=3'd4;
            7'b0001???: bcd_lop_c=3'd3;
            7'b00001??: bcd_lop_c=3'd2;
            7'b000001?: bcd_lop_c=3'd1;
            default: bcd_lop_c=3'd0;
        endcase
    end
    wire [7:0] bcd_exp_c = 8'd127 + {5'b0, bcd_lop_c};
    reg [22:0] bcd_mant_c;
    always @(*) begin
        case(bcd_lop_c)
            3'd6: bcd_mant_c={bcd_int_c[5:0],17'b0};
            3'd5: bcd_mant_c={bcd_int_c[4:0],18'b0};
            3'd4: bcd_mant_c={bcd_int_c[3:0],19'b0};
            3'd3: bcd_mant_c={bcd_int_c[2:0],20'b0};
            3'd2: bcd_mant_c={bcd_int_c[1:0],21'b0};
            3'd1: bcd_mant_c={bcd_int_c[0],22'b0};
            default: bcd_mant_c=23'b0;
        endcase
    end
    reg [31:0] fp32_c;
    always @(*) begin
        if(bcd_zero_c) fp32_c=32'h00000000;
        else fp32_c={1'b0, bcd_exp_c, bcd_mant_c};
    end
