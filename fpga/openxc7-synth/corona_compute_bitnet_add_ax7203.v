...
`timescale 1ns / 1ps
// corona_compute_bitnet_add_ax7203 — BitNet ADD on AX7203.
module corona_compute_bitnet_add_ax7203 (
    ...
    reg [1:0] code_result;
    always @(*) begin
        if(comp_result == 32'h00000000) code_result = 2'd0;
        else if(comp_result[31]) begin
            if(comp_result >= 32'hBF000000) code_result = 2'd2;  // <= -0.5 → -1
            else code_result = 2'd0;                     // > -0.5 → 0
        end else begin
            if(comp_result >= 32'h3F000000) code_result = 2'd1;   // >= 0.5 → +1
            else code_result = 2'd0;                      // < 0.5 → 0
        end
    end
    ...
    always @(posedge mclk or posedge rst) begin
        if(rst) begin result_reg<=0;result_ready<=0; end
            ...
            if(comp_ovld) result_reg<={30'b0,code_result};
        end
    end
