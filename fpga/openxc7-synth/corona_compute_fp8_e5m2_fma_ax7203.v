...
`timescale 1ns / 1ps
// corona_compute_fp8_e5m2_fma_ax7203 — FP8_E5M2 FMA on AX7203.
module corona_compute_fp8_e5m2_fma_ax7203 (
    ...
    wire mul_irdy,mul_ovld; wire [7:0] mul_result;
    wire add_irdy,add_ovld; wire [7:0] add_result;
    gf_mul_param #(.EXP_BITS(5),.MANT_BITS(2),.HAS_INF(1)) u_mul (
    ...
    gf_adder_param #(.EXP_BITS(5),.MANT_BITS(2),.HAS_INF(1)) u_add (
    ...
    always @(posedge mclk or posedge rst) begin
        if(rst) begin result_reg<=0;result_ready<=0; end
            ...
            if(add_ovld) result_reg<={{24{1'b0}},add_result};
        end
    end
