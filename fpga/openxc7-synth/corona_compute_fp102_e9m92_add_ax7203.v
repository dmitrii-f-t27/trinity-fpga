...
`timescale 1ns / 1ps
// corona_compute_fp102_e9m92_add_ax7203 — FP102_E9M92 ADD on AX7203.
module corona_compute_fp102_e9m92_add_ax7203 (
    ...
    reg [4:0] frm; reg [7:0] fmt_r; reg [101:0] a_r,b_r; reg frame_valid;
    always @(posedge mclk or posedge rst) begin
        if(rst) begin frm<=0;fmt_r<=0;a_r<=0;b_r<=0;frame_valid<=0; end
            ...
            if(rx_new) begin case(frm)
                ...
                5'd15: begin a_r[101:96]<=rx_byte;frm<=5'd16; end
                ...
                5'd28: begin b_r[101:96]<=rx_byte;frm<=5'd29; end
                5'd29: begin frame_valid<=1;frm<=0; end
            endcase end
        end
    end
    ...
    reg [101:0] a_reg,b_reg; reg comp_trigger;
    wire [101:0] fmt_a=a_reg, fmt_b=b_reg;
    wire f_sign_a = fmt_a_a[101];
    wire [8:0] f_exp_a = fmt_a_a[100:92];
    ...
    wire [8:0] f_exp_b = fmt_b_b[100:92];
    ...
    always @(*) begin
        ...
        else if(q_inf) q_result={q_sign, 9'd511, 92'd0};
        else if(q_exp >= 8'd254) q_result={q_sign, 9'd511, 92'd0};
