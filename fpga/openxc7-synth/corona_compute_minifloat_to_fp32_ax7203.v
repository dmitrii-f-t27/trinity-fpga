...
`timescale 1ns / 1ps
// corona_compute_minifloat_to_fp32_ax7203 — MINIFLOAT TO_FP32 on AX7203.
module corona_compute_minifloat_to_fp32_ax7203 (
    ...
    wire [7:0] mf_val=a_reg;
    wire sign=mf_val[7]; wire [2:0] exp_in=mf_val[6:4]; wire [3:0] mant=mf_val[3:0];
    wire is_zero=(exp_in==0)&&(mant==0); wire is_nan=(exp_in==7)&&(mant!=0);
    wire is_inf=(exp_in==7)&&(mant==0); wire is_sub=(exp_in==0)&&(mant!=0);
    wire [7:0] exp32=is_sub?8'd125:({5'd0,exp_in}+8'd124);
    wire [31:0] fp32_val=is_zero?32'h0:is_nan?32'h7FC00000:is_inf?{sign,8'hFF,23'b0}:{sign,exp32,{mant,19'b0}};
    always @(posedge mclk or posedge rst) begin
