...
`timescale 1ns / 1ps
// corona_compute_gf48_to_fp32_ax7203 — GF48 to IEEE binary32 converter.
// Input: GF48 [S:1][E:18][M:29]
// Output: FP32 [S:1][E:8][M:23]
module corona_compute_gf48_to_fp32_ax7203 (
    ...
    // ---- Frame FSM ----
    reg [3:0] frm; reg [7:0] fmt_r; reg [47:0] a_r; reg frame_valid;
    always @(posedge mclk or posedge rst) begin
    ...
    reg [47:0] a_reg; reg conv_trigger;
    wire gf_sign = a_reg[47];
    wire [17:0] gf_exp = a_reg[46:29];
    wire [28:0] gf_mant = a_reg[28:0];
    wire gf_zero = (gf_exp == 0) && (gf_mant == 0);
    wire gf_denorm = (gf_exp == 0) && (gf_mant != 0);
    wire gf_inf = (gf_exp == {18{1'b1}}) && (gf_mant == 0);
    wire gf_nan = (gf_exp == {18{1'b1}}) && (gf_mant != 0);
    wire [22:0] fp32_mant = gf_mant[22:0];
    wire signed [19:0] fp32_exp_calc = $signed(gf_exp) + 20'sd-130944;
    ...
    always @(*) begin
        ...
        else if(gf_denorm) fp32_result = {gf_sign, 8'b0, {gf_mant, -6'b0}};
