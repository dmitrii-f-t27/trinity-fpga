...
module corona_decode_gf20_ax7203 (
    ...
    wire [31:0] result;
    wire        is_zero;
    gf_decode_param #(.N(20), .E(7, .HAS_INF(0)), .M(12), .BIAS(63), .OUT_REG(0)) u_dec (.clk(1'b0), .rst_n(1'b1), .gf_in(code_r[19:0]), .fp32_out(result), .is_nan_o(), .is_inf_o(), .is_zero_o(), .is_subnormal_o());
    assign led[2] = |result;
