...
`timescale 1ns / 1ps
// corona_compute_uint8_fma_ax7203 — UINT8 FMA on AX7203.
module corona_compute_uint8_fma_ax7203 (
    ...
    always @(*) begin
        casez(uval_a)
            ...
            8'b01??????: u_lzd_a=4'd1;
            8'b001?????: u_lzd_a=4'd2;
            8'b0001????: u_lzd_a=4'd3;
            8'b00001???: u_lzd_a=4'd4;
            8'b000001??: u_lzd_a=4'd5;
            8'b0000001?: u_lzd_a=4'd6;
            default: u_lzd_a=4'd7;
        endcase
    end
    wire [7:0] u_fp32_exp_a = 8'd127 + 8'd7 - {4'b0, u_lzd_a};
    wire [7:0] u_shifted_a = uval_a << {u_lzd_a, 1'b0};
    wire [22:0] u_fp32_mant_a = {u_shifted_a[6:0], 16'b0};
    reg [31:0] fp32_a;
    always @(*) begin
    ...
    always @(*) begin
        casez(uval_b)
            ...
            8'b01??????: u_lzd_b=4'd1;
            8'b001?????: u_lzd_b=4'd2;
            8'b0001????: u_lzd_b=4'd3;
            8'b00001???: u_lzd_b=4'd4;
            8'b000001??: u_lzd_b=4'd5;
            8'b0000001?: u_lzd_b=4'd6;
            default: u_lzd_b=4'd7;
        endcase
    end
    wire [7:0] u_fp32_exp_b = 8'd127 + 8'd7 - {4'b0, u_lzd_b};
    wire [7:0] u_shifted_b = uval_b << {u_lzd_b, 1'b0};
    wire [22:0] u_fp32_mant_b = {u_shifted_b[6:0], 16'b0};
    reg [31:0] fp32_b;
    always @(*) begin
    ...
    always @(*) begin
        casez(uval_c)
            ...
            8'b01??????: u_lzd_c=4'd1;
            8'b001?????: u_lzd_c=4'd2;
            8'b0001????: u_lzd_c=4'd3;
            8'b00001???: u_lzd_c=4'd4;
            8'b000001??: u_lzd_c=4'd5;
            8'b0000001?: u_lzd_c=4'd6;
            default: u_lzd_c=4'd7;
        endcase
    end
    wire [7:0] u_fp32_exp_c = 8'd127 + 8'd7 - {4'b0, u_lzd_c};
    wire [7:0] u_shifted_c = uval_c << {u_lzd_c, 1'b0};
    wire [22:0] u_fp32_mant_c = {u_shifted_c[6:0], 16'b0};
    reg [31:0] fp32_c;
    always @(*) begin
    ...
    always @(*) begin
        ...
        else if(q_sign) q_result=8'd0;
        else if(q_shifted > 16'd255) q_result=8'hFF;
        else q_result=q_shifted[7:0];
    end
