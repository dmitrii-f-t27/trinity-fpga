...
`timescale 1ns / 1ps
// corona_compute_gf14_mul_ax7203 — GoldenFloat14 MUL on AX7203.
// GF14: [S:1][E:5][M:8] = 14 bits, BIAS=15, HAS_INF=0.
module corona_compute_gf14_mul_ax7203 (
    ...
    always @(posedge mclk or posedge rst) begin
        if(rst) begin frm<=0;fmt_r<=0;a_r<=0;b_r<=0;frame_valid<=0; end
            ...
            if(rx_new) begin case(frm)
                ...
                3'd6: begin b_r[13:8]<=rx_byte[5:0];frm<=3'd7; end
    ...
    gf_mul_param #(.EXP_BITS(5), .MANT_BITS(8), .HAS_INF(0)) u_comp (
        .clk(mclk), .rst(rst),
