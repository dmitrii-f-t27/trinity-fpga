...
`timescale 1ns / 1ps
// corona_compute_fp5_e3m1_fma_ax7203 — FP5_E3M1 FMA on AX7203.
module corona_compute_fp5_e3m1_fma_ax7203 (
    ...
    reg [2:0] frm; reg [7:0] fmt_r; reg [4:0] a_r,b_r,c_r; reg frame_valid;
    always @(posedge mclk or posedge rst) begin
        if(rst) begin frm<=0;fmt_r<=0;a_r<=0;b_r<=0;c_r<=0;frame_valid<=0; end
            ...
            if(rx_new) begin case(frm)
                ...
                3'd3: begin a_r[4:0]<=rx_byte;frm<=3'd4; end
                3'd4: begin b_r[4:0]<=rx_byte;frm<=3'd5; end
                3'd5: begin c_r[4:0]<=rx_byte;frm<=3'd6; end
    ...
    reg [4:0] a_reg,b_reg,c_reg; reg comp_trigger;
    wire [4:0] fmt_a=a_reg, fmt_b=b_reg, fmt_c=c_reg;
    wire f_sign_a = fmt_a[4];
    wire [2:0] f_exp_a = fmt_a[3:1];
    ...
    wire f_sign_b = fmt_b[4];
    wire [2:0] f_exp_b = fmt_b[3:1];
    ...
    wire f_sign_c = fmt_c[4];
    wire [2:0] f_exp_c = fmt_c[3:1];
    ...
    reg [4:0] q_result;
    always @(*) begin
        if(q_nan) q_result=5'd0;
        else if(q_zero) q_result=5'd0;
        else if(q_inf) q_result={q_sign, 3'd7, 1'd0};
        else if(q_exp >= 8'd132) q_result={q_sign, 3'd7, 1'd0};
        else if(q_exp < 8'd125) q_result={q_sign, 4'b0};
        else if(tgt_exp_s < 1) q_result={q_sign, 3'b0, q_mant[22:22]};
        else q_result={q_sign, tgt_exp_s[2:0], q_mant[22:22]};
    end
    reg [31:0] result_reg; reg result_ready;
    always @(posedge mclk or posedge rst) begin
        if(rst) begin result_reg<=0;result_ready<=0; end
        else begin result_ready<=add_ovld;
            if(add_ovld) result_reg<={27'b0,q_result};
        end
    end
