...
`timescale 1ns / 1ps
// corona_compute_gf16_cmp_ax7203 — GoldenFloat16 comparison on AX7203.
// GF16: [S:1][E:6][M:9] = 16 bits, BIAS=31, HAS_INF=1.
//
// Frame protocol (9 bytes TX):
//   AA 55 fmt op a_lo a_hi b_lo b_hi trig
//   op: 0x00=EQ, 0x01=LT, 0x02=LE
// Response (5 bytes RX):
//   A5 result 00 00 00 (result = 0x01 true, 0x00 false)

module corona_compute_gf16_cmp_ax7203 (
    ...
    // ---- Frame FSM: AA 55 fmt op a_lo a_hi b_lo b_hi trig ----
    ...
    // ---- Comparison (combinational, registered output) ----
    ...
    // Field extraction
    wire        sa = a_reg[15];
    wire [5:0]  ea = a_reg[14:9];
    wire [8:0]  ma = a_reg[8:0];
    wire        sb = b_reg[15];
    wire [5:0]  eb = b_reg[14:9];
    wire [8:0]  mb = b_reg[8:0];

    wire a_zero = (ea == 0) && (ma == 0);
    wire b_zero = (eb == 0) && (mb == 0);
    wire a_nan  = (ea == 6'b111111) && (ma != 0);
    wire b_nan  = (eb == 6'b111111) && (mb != 0);

    // Magnitude comparison (exp:mant as single field)
    wire [14:0] abs_a = {ea, ma};
    wire [14:0] abs_b = {eb, mb};
    wire mag_lt = (abs_a < abs_b);
    wire mag_eq = (abs_a == abs_b);

    wire both_zero = a_zero | b_zero && (a_zero && b_zero);
    wire both_pos  = (~sa) && (~sb) && ~(a_zero && b_zero);
    wire both_neg  = sa && sb && ~(a_zero && b_zero);

    // EQ: true if both zero (sign doesn't matter) or same sign + same magnitude
    ...
    wire cmp_lt = (a_nan | b_nan) ? 1'b0 :
                  ...
                  (sa && ~sb) ? 1'b1 :       // neg < pos
                  (~sa && sb) ? 1'b0 :       // pos > neg
                  both_neg ? (abs_a > abs_b) : // both neg: larger mag = smaller
                  mag_lt;                      // both pos: normal
