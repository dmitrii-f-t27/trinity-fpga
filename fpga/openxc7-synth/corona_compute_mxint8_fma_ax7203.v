...
`timescale 1ns / 1ps
// corona_compute_mxint8_fma_ax7203 — MXINT8 FMA on AX7203.
module corona_compute_mxint8_fma_ax7203 (
    ...
    wire [7:0] mx_val_a = fmt_a;
    wire mx_zero_a = (mx_val_a==8'h00);
    wire mx_res_a = (mx_val_a==8'h80);
    wire mx_sign_a = mx_val_a[7];
    wire [6:0] mx_abs_a = mx_sign_a ? (~mx_val_a[6:0]+7'd1) : mx_val_a[6:0];
    reg [2:0] mx_lop_a;
    always @(*) begin
        casez(mx_abs_a)
            7'b1??????: mx_lop_a=3'd6;
            7'b01?????: mx_lop_a=3'd5;
            7'b001????: mx_lop_a=3'd4;
            7'b0001???: mx_lop_a=3'd3;
            7'b00001??: mx_lop_a=3'd2;
            7'b000001?: mx_lop_a=3'd1;
            default: mx_lop_a=3'd0;
        endcase
    end
    wire [7:0] mx_exp_a = 8'd121 + {5'b0,mx_lop_a};
    reg [22:0] mx_mant_a;
    always @(*) begin
        case(mx_lop_a)
            3'd6: mx_mant_a={mx_abs_a[5:0],17'b0};
            3'd5: mx_mant_a={mx_abs_a[4:0],18'b0};
            3'd4: mx_mant_a={mx_abs_a[3:0],19'b0};
            3'd3: mx_mant_a={mx_abs_a[2:0],20'b0};
            3'd2: mx_mant_a={mx_abs_a[1:0],21'b0};
            3'd1: mx_mant_a={mx_abs_a[0],22'b0};
            default: mx_mant_a=23'b0;
        endcase
    end
    reg [31:0] fp32_a;
    always @(*) begin
    ...
    wire [7:0] mx_val_b = fmt_b;
    wire mx_zero_b = (mx_val_b==8'h00);
    wire mx_res_b = (mx_val_b==8'h80);
    wire mx_sign_b = mx_val_b[7];
    wire [6:0] mx_abs_b = mx_sign_b ? (~mx_val_b[6:0]+7'd1) : mx_val_b[6:0];
    reg [2:0] mx_lop_b;
    always @(*) begin
        casez(mx_abs_b)
            7'b1??????: mx_lop_b=3'd6;
            7'b01?????: mx_lop_b=3'd5;
            7'b001????: mx_lop_b=3'd4;
            7'b0001???: mx_lop_b=3'd3;
            7'b00001??: mx_lop_b=3'd2;
            7'b000001?: mx_lop_b=3'd1;
            default: mx_lop_b=3'd0;
        endcase
    end
    wire [7:0] mx_exp_b = 8'd121 + {5'b0,mx_lop_b};
    reg [22:0] mx_mant_b;
    always @(*) begin
        case(mx_lop_b)
            3'd6: mx_mant_b={mx_abs_b[5:0],17'b0};
            3'd5: mx_mant_b={mx_abs_b[4:0],18'b0};
            3'd4: mx_mant_b={mx_abs_b[3:0],19'b0};
            3'd3: mx_mant_b={mx_abs_b[2:0],20'b0};
            3'd2: mx_mant_b={mx_abs_b[1:0],21'b0};
            3'd1: mx_mant_b={mx_abs_b[0],22'b0};
            default: mx_mant_b=23'b0;
        endcase
    end
    reg [31:0] fp32_b;
    always @(*) begin
    ...
    wire [7:0] mx_val_c = fmt_c;
    wire mx_zero_c = (mx_val_c==8'h00);
    wire mx_res_c = (mx_val_c==8'h80);
    wire mx_sign_c = mx_val_c[7];
    wire [6:0] mx_abs_c = mx_sign_c ? (~mx_val_c[6:0]+7'd1) : mx_val_c[6:0];
    reg [2:0] mx_lop_c;
    always @(*) begin
        casez(mx_abs_c)
            7'b1??????: mx_lop_c=3'd6;
            7'b01?????: mx_lop_c=3'd5;
            7'b001????: mx_lop_c=3'd4;
            7'b0001???: mx_lop_c=3'd3;
            7'b00001??: mx_lop_c=3'd2;
            7'b000001?: mx_lop_c=3'd1;
            default: mx_lop_c=3'd0;
        endcase
    end
    wire [7:0] mx_exp_c = 8'd121 + {5'b0,mx_lop_c};
    reg [22:0] mx_mant_c;
    always @(*) begin
        case(mx_lop_c)
            3'd6: mx_mant_c={mx_abs_c[5:0],17'b0};
            3'd5: mx_mant_c={mx_abs_c[4:0],18'b0};
            3'd4: mx_mant_c={mx_abs_c[3:0],19'b0};
            3'd3: mx_mant_c={mx_abs_c[2:0],20'b0};
            3'd2: mx_mant_c={mx_abs_c[1:0],21'b0};
            3'd1: mx_mant_c={mx_abs_c[0],22'b0};
            default: mx_mant_c=23'b0;
        endcase
    end
    reg [31:0] fp32_c;
    always @(*) begin
        if(mx_zero_c) fp32_c={mx_sign_c, 31'b0};
        else if(mx_res_c) fp32_c=32'h7FC00000;
        else fp32_c={mx_sign_c,mx_exp_c,mx_mant_c};
    end
    ...
    // MXINT8 = int × 2^-6, so we need exp-127 = lop-6, i.e. exp = 121+lop
    // Range: lop 0..6 → exp 121..127
    wire [8:0] q_lop = {1'b0,q_exp} - 9'd121;
    wire [6:0] q_abs;
    reg [7:0] q_result;
    always @(*) begin
        if(q_nan) q_result=8'h80;
        else if(q_zero) q_result=8'h00;
        else if(q_exp<8'd121) q_result=8'h00;
        else if(q_exp>8'd127) q_result=q_sign?8'h81:8'h7F;
        else begin
            case(q_lop[2:0])
                3'd6: q_abs={1'b1,q_mant[22:17]};
                3'd5: q_abs={1'b1,q_mant[22:18],1'b0};
                3'd4: q_abs={1'b1,q_mant[22:19],2'b0};
                3'd3: q_abs={1'b1,q_mant[22:20],3'b0};
                3'd2: q_abs={1'b1,q_mant[22:21],4'b0};
                3'd1: q_abs={1'b1,q_mant[22],5'b0};
                default: q_abs=7'd1;
            endcase
            if(q_sign) q_result={1'b1,(~q_abs+7'd1)};
