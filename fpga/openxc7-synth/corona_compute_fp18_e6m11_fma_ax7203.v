...
`timescale 1ns / 1ps
// corona_compute_fp18_e6m11_fma_ax7203 — FP18_E6M11 FMA on AX7203.
module corona_compute_fp18_e6m11_fma_ax7203 (
    ...
    wire f_sign_a = fmt_a[17];
    wire [5:0] f_exp_a = fmt_a[16:11];
    wire [10:0] f_mant_a = fmt_a[10:0];
    ...
    wire signed [15:0] f_de_a = $signed({1'b0, f_exp_a}) - 16'sd63 + 16'sd127;
    ...
    wire f_sign_b = fmt_b[17];
    wire [5:0] f_exp_b = fmt_b[16:11];
    wire [10:0] f_mant_b = fmt_b[10:0];
    ...
    wire signed [15:0] f_de_b = $signed({1'b0, f_exp_b}) - 16'sd63 + 16'sd127;
    ...
    wire f_sign_c = fmt_c[17];
    wire [5:0] f_exp_c = fmt_c[16:11];
    wire [10:0] f_mant_c = fmt_c[10:0];
    ...
    wire signed [15:0] f_de_c = $signed({1'b0, f_exp_c}) - 16'sd63 + 16'sd127;
    wire [7:0] f_exp32_c = (f_de_c > 16'sd254) ? 8'd254 : (f_de_c < 0) ? 8'd0 : f_de_c[7:0];
    wire [22:0] f_mant32_c = {f_mant_c, 12'b0};
    wire [22:0] f_mant32_norm_c = {1'b0, f_mant_c, 11'b0};
    reg [31:0] fp32_c;
    always @(*) begin
    ...
    wire signed [15:0] tgt_exp_s = $signed({1'b0, q_exp}) - 16'sd127 + 16'sd63;
    reg [17:0] q_result;
    always @(*) begin
        if(q_nan) q_result=18'd0;
        else if(q_zero) q_result=18'd0;
        else if(q_inf) q_result={q_sign, 6'd63, 11'd0};
        else if(q_exp >= 8'd128) q_result={q_sign, 6'd63, 11'd0};
        else if(q_exp < 8'd65) q_result={q_sign, 17'b0};
        else if(tgt_exp_s < 1) q_result={q_sign, 6'b0, q_mant[22:12]};
        else q_result={q_sign, tgt_exp_s[5:0], q_mant[22:12]};
    end
