...
`timescale 1ns / 1ps
// corona_compute_gf48_cmp_ax7203 — GoldenFloat48 comparison on AX7203.
// GF48: [S:1][E:18][M:29] = 48 bits, HAS_INF=1.
// op: 0x00=EQ, 0x01=LT, 0x02=LE
module corona_compute_gf48_cmp_ax7203 (
    ...
    wire sa = a_reg[47];
    wire [17:0] ea = a_reg[46:29];
    wire [28:0] ma = a_reg[28:0];
    wire sb = b_reg[47];
    wire [17:0] eb = b_reg[46:29];
    wire [28:0] mb = b_reg[28:0];
    wire a_zero = (ea == 0) && (ma == 0);
    wire b_zero = (eb == 0) && (mb == 0);
    wire a_nan = (ea == {18{1'b1}}) && (ma != 0);
    wire b_nan = (eb == {18{1'b1}}) && (mb != 0);
    wire [46:0] abs_a = {ea, ma};
    wire [46:0] abs_b = {eb, mb};
    ...
    // TX: buffer+mux (no conflicting NBA — fixes tx race). 5 bytes sliced from tx_load[55:0].
    ...
    always @(posedge mclk or posedge rst) begin
        if(rst) begin responding<=0;tx_idx<=0;tcnt<=BAUD_DIV-1;tbi<=0;tsr<=10'h3FF;uart_tx<=1;
            ...
            if(result_ready) begin
                tx_buf0<=tx_load[7:0]; tx_buf1<=tx_load[15:8]; tx_buf2<=tx_load[23:16]; tx_buf3<=tx_load[31:24]; tx_buf4<=tx_load[39:32]; responding<=1; tx_idx<=0;
            end
