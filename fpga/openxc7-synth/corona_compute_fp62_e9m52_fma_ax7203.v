...
`timescale 1ns / 1ps
// corona_compute_fp62_e9m52_fma_ax7203 — FP62_E9M52 FMA on AX7203.
module corona_compute_fp62_e9m52_fma_ax7203 (
    ...
    reg [4:0] frm; reg [7:0] fmt_r; reg [61:0] a_r,b_r,c_r; reg frame_valid;
    always @(posedge mclk or posedge rst) begin
        if(rst) begin frm<=0;fmt_r<=0;a_r<=0;b_r<=0;c_r<=0;frame_valid<=0; end
            ...
            if(rx_new) begin case(frm)
                ...
                5'd10: begin a_r[61:56]<=rx_byte;frm<=5'd11; end
                ...
                5'd18: begin b_r[61:56]<=rx_byte;frm<=5'd19; end
                ...
                5'd26: begin c_r[61:56]<=rx_byte;frm<=5'd27; end
    ...
    reg [61:0] a_reg,b_reg,c_reg; reg comp_trigger;
    wire [61:0] fmt_a=a_reg, fmt_b=b_reg, fmt_c=c_reg;
    wire f_sign_a = fmt_a[61];
    wire [8:0] f_exp_a = fmt_a[60:52];
    ...
    wire f_sign_b = fmt_b[61];
    wire [8:0] f_exp_b = fmt_b[60:52];
    ...
    wire f_sign_c = fmt_c[61];
    wire [8:0] f_exp_c = fmt_c[60:52];
    ...
    reg [61:0] q_result;
    always @(*) begin
        if(q_nan) q_result=62'd0;
        else if(q_zero) q_result=62'd0;
        else if(q_inf) q_result={q_sign, 9'd511, 52'd0};
        else if(q_exp >= 8'd254) q_result={q_sign, 9'd511, 52'd0};
        else if(q_exp < 8'd0) q_result={q_sign, 61'b0};
        else if(tgt_exp_s < 1) q_result={q_sign, 9'b0, q_mant[22:0]};
        else q_result={q_sign, tgt_exp_s[8:0], q_mant[22:0]};
    end
    reg [61:0] result_reg; reg result_ready;
    always @(posedge mclk or posedge rst) begin
