...
`timescale 1ns / 1ps
// corona_compute_fp124_e8m115_fma_ax7203 — FP124_E8M115 FMA on AX7203.
module corona_compute_fp124_e8m115_fma_ax7203 (
    ...
    reg [5:0] frm; reg [7:0] fmt_r; reg [123:0] a_r,b_r,c_r; reg frame_valid;
    always @(posedge mclk or posedge rst) begin
        if(rst) begin frm<=0;fmt_r<=0;a_r<=0;b_r<=0;c_r<=0;frame_valid<=0; end
            ...
            if(rx_new) begin case(frm)
                ...
                6'd18: begin a_r[123:120]<=rx_byte;frm<=6'd19; end
                ...
                6'd34: begin b_r[123:120]<=rx_byte;frm<=6'd35; end
                ...
                6'd50: begin c_r[123:120]<=rx_byte;frm<=6'd51; end
    ...
    reg [123:0] a_reg,b_reg,c_reg; reg comp_trigger;
    wire [123:0] fmt_a=a_reg, fmt_b=b_reg, fmt_c=c_reg;
    wire f_sign_a = fmt_a[123];
    wire [7:0] f_exp_a = fmt_a[122:115];
    wire [114:0] f_mant_a = fmt_a[114:0];
    ...
    wire [22:0] f_mant32_a = {f_mant_a, -92'b0};
    ...
    wire f_sign_b = fmt_b[123];
    wire [7:0] f_exp_b = fmt_b[122:115];
    wire [114:0] f_mant_b = fmt_b[114:0];
    ...
    wire [22:0] f_mant32_b = {f_mant_b, -92'b0};
    ...
    wire f_sign_c = fmt_c[123];
    wire [7:0] f_exp_c = fmt_c[122:115];
    wire [114:0] f_mant_c = fmt_c[114:0];
    ...
    wire [22:0] f_mant32_c = {f_mant_c, -92'b0};
    ...
    always @(*) begin
        ...
        else if(q_inf) q_result={q_sign, 8'd255, 115'd0};
        else if(q_exp >= 8'd254) q_result={q_sign, 8'd255, 115'd0};
        else if(q_exp < 8'd1) q_result={q_sign, 123'b0};
