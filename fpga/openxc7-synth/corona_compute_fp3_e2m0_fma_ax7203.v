...
`timescale 1ns / 1ps
// corona_compute_fp3_e2m0_fma_ax7203 — FP3_E2M0 FMA on AX7203.
module corona_compute_fp3_e2m0_fma_ax7203 (
    ...
    wire [1:0] f_exp_a = fmt_a[1:0];
    wire f_zero_a = (f_exp_a == 0) && (1'b1);
    wire f_inf_a = (f_exp_a == 3) && (1'b1);
    wire f_nan_a = (f_exp_a == 3) && (1'b0);
    wire f_sub_a = (f_exp_a == 0) && (1'b0);
    ...
    wire [22:0] f_mant32_a = 23'b0;
    wire [22:0] f_mant32_norm_a = 23'b0;
    reg [31:0] fp32_a;
    always @(*) begin
    ...
    wire [1:0] f_exp_b = fmt_b[1:0];
    wire f_zero_b = (f_exp_b == 0) && (1'b1);
    wire f_inf_b = (f_exp_b == 3) && (1'b1);
    wire f_nan_b = (f_exp_b == 3) && (1'b0);
    wire f_sub_b = (f_exp_b == 0) && (1'b0);
    wire signed [11:0] f_de_b = $signed({1'b0, f_exp_b}) - 12'sd1 + 12'sd127;
    wire [7:0] f_exp32_b = (f_de_b > 12'sd254) ? 8'd254 : (f_de_b < 0) ? 8'd0 : f_de_b[7:0];
    wire [22:0] f_mant32_b = 23'b0;
    wire [22:0] f_mant32_norm_b = 23'b0;
    reg [31:0] fp32_b;
    always @(*) begin
    ...
    wire [1:0] f_exp_c = fmt_c[1:0];
    wire f_zero_c = (f_exp_c == 0) && (1'b1);
    wire f_inf_c = (f_exp_c == 3) && (1'b1);
    wire f_nan_c = (f_exp_c == 3) && (1'b0);
    wire f_sub_c = (f_exp_c == 0) && (1'b0);
    wire signed [11:0] f_de_c = $signed({1'b0, f_exp_c}) - 12'sd1 + 12'sd127;
    wire [7:0] f_exp32_c = (f_de_c > 12'sd254) ? 8'd254 : (f_de_c < 0) ? 8'd0 : f_de_c[7:0];
    wire [22:0] f_mant32_c = 23'b0;
    wire [22:0] f_mant32_norm_c = 23'b0;
    reg [31:0] fp32_c;
    always @(*) begin
    ...
    always @(*) begin
        ...
        else if(q_inf) q_result={q_sign, 2'd3};
        else if(q_exp >= 8'd130) q_result={q_sign, 2'd3};
        else if(q_exp < 8'd127) q_result={q_sign, 2'b0};
        else if(tgt_exp_s < 1) q_result={q_sign, 2'b0};
        else q_result={q_sign, tgt_exp_s[1:0]};
    end
