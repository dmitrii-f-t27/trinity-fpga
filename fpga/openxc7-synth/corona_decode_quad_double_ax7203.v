...
`timescale 1ns / 1ps
// corona_decode_quad_double_ax7203 — Bailey/Hida quad-double (4x FP64) decode on AX7203.
// 256-bit decode frame: AA 55 fmt code[0..31] trig (35 bytes) -> A5 FP32 LE.
module corona_decode_quad_double_ax7203 (
    ...
    // 256-bit decode frame: AA 55 fmt code[0..31] trig; counter collects 32 code bytes
    reg [5:0] frm; reg [7:0] fmt_r; reg [255:0] code_r; reg [4:0] bcnt; reg frame_valid;
    always @(posedge mclk or posedge rst) begin
        if(rst) begin frm<=0;fmt_r<=0;code_r<=0;bcnt<=0;frame_valid<=0; end
            ...
            if(rx_new) begin case(frm)
                ...
                6'd2: begin fmt_r<=rx_byte; frm<=6'd3; bcnt<=0; end
                6'd3: begin code_r[bcnt*8 +: 8]<=rx_byte;
                       if(bcnt==5'd31) frm<=6'd4; else bcnt<=bcnt+5'd1; end
                6'd4: begin frame_valid<=1; frm<=6'd0; end
                default: frm<=0;
            endcase end
        end
    end
    ...
    wire [31:0] result;
    quad_double_decode u_dec (.qd(code_r[255:0]), .fp32_out(result));
    assign led[2] = |result;
