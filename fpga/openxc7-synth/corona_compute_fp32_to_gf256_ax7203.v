...
`timescale 1ns / 1ps
// corona_compute_fp32_to_gf256_ax7203 — IEEE binary32 to GF256 converter.
// Output: GF256 [S:1][E:97][M:158]
module corona_compute_fp32_to_gf256_ax7203 (
    ...
    wire signed [98:0] gf_exp_calc = $signed({2'b0, fp_exp}) + 99'sd79228162514264337593543950208;
    wire overflow = (gf_exp_calc > 99'sd158456325028528675187087900670);
    wire underflow = (gf_exp_calc < 99'sd1);
    wire [157:0] gf_mant_trunc = fp_mant[22:-135];
    reg [255:0] gf_result;
    always @(*) begin
        if(fp_nan) gf_result = {1'b0, {97{1'b1}}, {157{1'b0}}, 1'b1};
        else if(fp_inf || overflow) gf_result = {fp_sign, {97{1'b1}}, {158{1'b0}}};
        else if(fp_zero || fp_denorm || underflow) gf_result = {fp_sign, {97{1'b0}}, {158{1'b0}}};
        else gf_result = {fp_sign, gf_exp_calc[96:0], gf_mant_trunc};
    end
    always @(posedge mclk or posedge rst) begin
