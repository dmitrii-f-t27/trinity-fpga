...
`timescale 1ns / 1ps
// corona_compute_gf64_cmp_ax7203 — GoldenFloat64 comparison on AX7203.
// GF64: [S:1][E:24][M:39] = 64 bits, HAS_INF=0.
// op: 0x00=EQ, 0x01=LT, 0x02=LE
module corona_compute_gf64_cmp_ax7203 (
    ...
    wire sa = a_reg[63];
    wire [23:0] ea = a_reg[62:39];
    wire [38:0] ma = a_reg[38:0];
    wire sb = b_reg[63];
    wire [23:0] eb = b_reg[62:39];
    wire [38:0] mb = b_reg[38:0];
    wire a_zero = (ea == 0) && (ma == 0);
    wire b_zero = (eb == 0) && (mb == 0);
    wire a_nan = (ea == {24{1'b1}}) && (ma != 0);
    wire b_nan = (eb == {24{1'b1}}) && (mb != 0);
    wire [62:0] abs_a = {ea, ma};
    wire [62:0] abs_b = {eb, mb};
    ...
    // TX: buffer+mux (no conflicting NBA — fixes tx race). 5 bytes sliced from tx_load[71:0].
