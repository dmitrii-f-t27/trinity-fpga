...
`timescale 1ns / 1ps
// corona_compute_gf16_add_ax7203 — GoldenFloat16 ADD on AX7203.
// Accepts two GF16 operands via UART, returns GF16 sum.
//
...
module corona_compute_gf16_add_ax7203 (
    ...
    // ---- GF16 ADD compute (registered, 1-cycle latency) ----
    reg [15:0] a_reg, b_reg;
    reg comp_trigger;
    wire comp_in_ready, comp_out_valid;
    wire [15:0] comp_result;

    gf_adder_param #(.EXP_BITS(6), .MANT_BITS(9), .HAS_INF(1)) u_comp (
        .clk(mclk), .rst(rst),
