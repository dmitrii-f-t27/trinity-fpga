...
`timescale 1ns / 1ps
// corona_compute_fp78_e12m65_alu_ax7203 — FP78_E12M65 ALU on AX7203.
module corona_compute_fp78_e12m65_alu_ax7203 (
    ...
    reg [4:0] frm; reg [7:0] fmt_r,op_r; reg [77:0] a_r,b_r; reg frame_valid;
    always @(posedge mclk or posedge rst) begin
        if(rst) begin frm<=0;fmt_r<=0;op_r<=0;a_r<=0;b_r<=0;frame_valid<=0; end
            ...
            if(rx_new) begin case(frm)
                ...
                5'd13: begin a_r[77:72]<=rx_byte;frm<=5'd14; end
                ...
                5'd23: begin b_r[77:72]<=rx_byte;frm<=5'd24; end
    ...
    reg [77:0] a_reg,b_reg; reg [7:0] op_reg; reg comp_trigger;
    wire [77:0] fmt_a=a_reg, fmt_b=b_reg;
    wire f_sign_a = fmt_a_a[77];
    wire [11:0] f_exp_a = fmt_a_a[76:65];
    wire [64:0] f_mant_a = fmt_a_a[64:0];
    ...
    wire [22:0] f_mant32_a = {f_mant_a, -42'b0};
    ...
    wire f_sign_b = fmt_b_b[77];
    wire [11:0] f_exp_b = fmt_b_b[76:65];
    wire [64:0] f_mant_b = fmt_b_b[64:0];
    ...
    wire [22:0] f_mant32_b = {f_mant_b, -42'b0};
    ...
    reg [77:0] q_result;
    always @(*) begin
        if(q_nan) q_result=78'd0;
        else if(q_zero) q_result=78'd0;
        else if(q_inf) q_result={q_sign, 12'd4095, 65'd0};
        else if(q_exp >= 8'd254) q_result={q_sign, 12'd4095, 65'd0};
        else if(q_exp < 8'd0) q_result={q_sign, 77'b0};
        else if(tgt_exp_s < 1) q_result={q_sign, 12'b0, q_mant[22:0]};
        else q_result={q_sign, tgt_exp_s[11:0], q_mant[22:0]};
    end
    reg [77:0] result_reg; reg result_ready;
    always @(posedge mclk or posedge rst) begin
