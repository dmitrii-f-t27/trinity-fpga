...
`timescale 1ns / 1ps
// corona_compute_fp6_e3m2_to_fp32_ax7203 — FP6_E3M2 TO_FP32 on AX7203.
module corona_compute_fp6_e3m2_to_fp32_ax7203 (
    ...
    wire sv=a_reg[5]; wire [2:0] ev=a_reg[4:2]; wire [1:0] mv=a_reg[1:0];
    wire iz=(ev==0)&&(mv==0);
    wire ii=(0)&&(ev=={3{1'b1}})&&(mv==0);
    wire in=(0)&&(ev=={3{1'b1}})&&(mv!=0);
    reg [31:0] fr;
    always @(*) begin
        if(in) fr=32'h7FC00000;
        else if(ii) fr={sv,8'hFF,23'h0};
        else if(iz) fr={sv,8'h00,23'h0};
        else fr={sv,{8'b0,ev}+9'd124,{21{1'b0}},mv};
    end
    always @(posedge mclk or posedge rst) begin
