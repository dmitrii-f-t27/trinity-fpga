...
`timescale 1ns / 1ps
// corona_compute_gf14_to_fp32_ax7203 — GF14 to IEEE binary32 converter.
// Input: GF14 [S:1][E:5][M:8] (bias=15, HAS_INF=0)
// Output: FP32 [S:1][E:8][M:23] (bias=127)
//
// Frame protocol (6 bytes TX):
//   AA 55 fmt a0 a1 trig
// Response (5 bytes RX):
//   A5 r0 r1 r2 r3

module corona_compute_gf14_to_fp32_ax7203 (
    ...
    // ---- Frame FSM ----
    reg [2:0] frm; reg [7:0] fmt_r; reg [13:0] a_r; reg frame_valid;
    always @(posedge mclk or posedge rst) begin
        if(rst) begin frm<=0;fmt_r<=0;a_r<=0;frame_valid<=0; end
            ...
            if(rx_new) begin case(frm)
                ...
                3'd4: begin a_r[13:8]<=rx_byte[5:0];frm<=3'd5; end
    ...
    // ---- GF14->FP32 conversion ----
    reg [13:0] a_reg;
    reg conv_trigger;

    wire        gf_sign = a_reg[13];
    wire [4:0] gf_exp  = a_reg[12:8];
    wire [7:0] gf_mant = a_reg[7:0];
    ...
    // FP32 exp = gf_exp + 112 (= gf_exp - 15 + 127)
    wire [9:0] fp32_exp_norm = $unsigned(gf_exp) + 10'd112;
    wire [22:0] fp32_mant_norm = {gf_mant, 15'b0};
    ...
    always @(*) begin
        if (gf_zero)
            ...
            fp32_result = {gf_sign, 8'b0, {gf_mant, 15'b0}};
        else
