...
`timescale 1ns / 1ps
// corona_compute_fp17_e8m8_fma_ax7203 — FP17_E8M8 FMA on AX7203.
module corona_compute_fp17_e8m8_fma_ax7203 (
    ...
    reg [3:0] frm; reg [7:0] fmt_r; reg [16:0] a_r,b_r,c_r; reg frame_valid;
    always @(posedge mclk or posedge rst) begin
    ...
    reg [16:0] a_reg,b_reg,c_reg; reg comp_trigger;
    wire [16:0] fmt_a=a_reg, fmt_b=b_reg, fmt_c=c_reg;
    wire f_sign_a = fmt_a[16];
    wire [7:0] f_exp_a = fmt_a[15:8];
    wire [7:0] f_mant_a = fmt_a[7:0];
    ...
    wire [22:0] f_mant32_norm_a = {1'b0, f_mant_a, 14'b0};
    reg [31:0] fp32_a;
    always @(*) begin
    ...
    wire f_sign_b = fmt_b[16];
    wire [7:0] f_exp_b = fmt_b[15:8];
    wire [7:0] f_mant_b = fmt_b[7:0];
    ...
    wire [22:0] f_mant32_norm_b = {1'b0, f_mant_b, 14'b0};
    reg [31:0] fp32_b;
    always @(*) begin
    ...
    wire f_sign_c = fmt_c[16];
    wire [7:0] f_exp_c = fmt_c[15:8];
    wire [7:0] f_mant_c = fmt_c[7:0];
    ...
    wire [22:0] f_mant32_norm_c = {1'b0, f_mant_c, 14'b0};
    reg [31:0] fp32_c;
    always @(*) begin
    ...
    reg [16:0] q_result;
    always @(*) begin
        if(q_nan) q_result=17'd0;
        else if(q_zero) q_result=17'd0;
        else if(q_inf) q_result={q_sign, 8'd255, 8'd0};
        else if(q_exp >= 8'd254) q_result={q_sign, 8'd255, 8'd0};
        else if(q_exp < 8'd1) q_result={q_sign, 16'b0};
        else if(tgt_exp_s < 1) q_result={q_sign, 8'b0, q_mant[22:15]};
        else q_result={q_sign, tgt_exp_s[7:0], q_mant[22:15]};
    end
    reg [31:0] result_reg; reg result_ready;
    always @(posedge mclk or posedge rst) begin
        if(rst) begin result_reg<=0;result_ready<=0; end
        else begin result_ready<=add_ovld;
            if(add_ovld) result_reg<={15'b0,q_result};
        end
    end
