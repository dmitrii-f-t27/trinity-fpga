...
`timescale 1ns / 1ps
// corona_compute_fp91_e8m82_add_ax7203 — FP91_E8M82 ADD on AX7203.
module corona_compute_fp91_e8m82_add_ax7203 (
    ...
    reg [4:0] frm; reg [7:0] fmt_r; reg [90:0] a_r,b_r; reg frame_valid;
    always @(posedge mclk or posedge rst) begin
        if(rst) begin frm<=0;fmt_r<=0;a_r<=0;b_r<=0;frame_valid<=0; end
            ...
            if(rx_new) begin case(frm)
                ...
                5'd14: begin a_r[90:88]<=rx_byte;frm<=5'd15; end
                ...
                5'd26: begin b_r[90:88]<=rx_byte;frm<=5'd27; end
    ...
    reg [90:0] a_reg,b_reg; reg comp_trigger;
    wire [90:0] fmt_a=a_reg, fmt_b=b_reg;
    wire f_sign_a = fmt_a[90];
    wire [7:0] f_exp_a = fmt_a[89:82];
    wire [81:0] f_mant_a = fmt_a[81:0];
    ...
    wire [22:0] f_mant32_a = {f_mant_a, -59'b0};
    ...
    wire f_sign_b = fmt_b[90];
    wire [7:0] f_exp_b = fmt_b[89:82];
    wire [81:0] f_mant_b = fmt_b[81:0];
    ...
    wire [22:0] f_mant32_b = {f_mant_b, -59'b0};
    ...
    always @(*) begin
        ...
        else if(q_inf) q_result={q_sign, 8'd255, 82'd0};
        else if(q_exp >= 8'd254) q_result={q_sign, 8'd255, 82'd0};
        else if(q_exp < 8'd1) q_result={q_sign, 90'b0};
