...
`timescale 1ns / 1ps
// corona_compute_fp120_e8m111_fma_ax7203 — FP120_E8M111 FMA on AX7203.
module corona_compute_fp120_e8m111_fma_ax7203 (
    ...
    reg [5:0] frm; reg [7:0] fmt_r; reg [119:0] a_r,b_r,c_r; reg frame_valid;
    always @(posedge mclk or posedge rst) begin
        if(rst) begin frm<=0;fmt_r<=0;a_r<=0;b_r<=0;c_r<=0;frame_valid<=0; end
            ...
            if(rx_new) begin case(frm)
                ...
                6'd32: begin b_r[119:112]<=rx_byte;frm<=6'd33; end
                ...
                6'd47: begin c_r[119:112]<=rx_byte;frm<=6'd48; end
    ...
    reg [119:0] a_reg,b_reg,c_reg; reg comp_trigger;
    wire [119:0] fmt_a=a_reg, fmt_b=b_reg, fmt_c=c_reg;
    wire f_sign_a = fmt_a[119];
    wire [7:0] f_exp_a = fmt_a[118:111];
    wire [110:0] f_mant_a = fmt_a[110:0];
    ...
    wire [22:0] f_mant32_a = {f_mant_a, -88'b0};
    ...
    wire f_sign_b = fmt_b[119];
    wire [7:0] f_exp_b = fmt_b[118:111];
    wire [110:0] f_mant_b = fmt_b[110:0];
    ...
    wire [22:0] f_mant32_b = {f_mant_b, -88'b0};
    ...
    wire f_sign_c = fmt_c[119];
    wire [7:0] f_exp_c = fmt_c[118:111];
    wire [110:0] f_mant_c = fmt_c[110:0];
    ...
    wire [22:0] f_mant32_c = {f_mant_c, -88'b0};
    ...
    always @(*) begin
        ...
        else if(q_inf) q_result={q_sign, 8'd255, 111'd0};
        else if(q_exp >= 8'd254) q_result={q_sign, 8'd255, 111'd0};
        else if(q_exp < 8'd1) q_result={q_sign, 119'b0};
