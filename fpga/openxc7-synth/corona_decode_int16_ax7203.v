...
`timescale 1ns / 1ps
// corona_decode_int16_ax7203 — INT16 decode on AX7203.
// Single-decoder build: CFGMCLK (~70 MHz, STARTUPE2) clock, BAUD_DIV 434 (~161290).
// Frame: AA 55 fmt code_lo code_hi trig -> A5 r0 r1 r2 r3 (32-bit LE, sign-extended int32).
// Identical infrastructure to corona_decode_e8m0_ax7203; only the decoder differs
// (int16_decode: 16-bit signed -> 32-bit sign-extension, uses code_r[15:0]).
module corona_decode_int16_ax7203 (
    ...
    wire [31:0] result;
    wire        is_zero;
    int16_decode u_dec (.int16_in(code_r[15:0]), .int32_out(result), .is_zero(is_zero));
    assign led[2] = |result;
