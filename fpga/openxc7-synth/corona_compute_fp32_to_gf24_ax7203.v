...
`timescale 1ns / 1ps
// corona_compute_fp32_to_gf24_ax7203 — IEEE binary32 to GF24 converter.
// Input: FP32 [S:1][E:8][M:23] (bias=127)
// Output: GF24 [S:1][E:9][M:14] (bias=255, HAS_INF=0)
// Mantissa truncation. Overflow -> max, underflow -> zero.
module corona_compute_fp32_to_gf24_ax7203 (
    ...
    // ---- Frame FSM: AA 55 fmt a0 a1 a2 a3 trig (8 bytes) ----
    ...
    wire signed [10:0] gf_exp_calc = $signed({2'b0, fp_exp}) + 11'sd128;
    wire overflow  = (gf_exp_calc > 11'sd511);
    wire underflow = (gf_exp_calc < 11'sd1);
    wire [13:0] gf_mant_trunc = fp_mant[22:9];
    reg [23:0] gf_result;
    always @(*) begin
        if (fp_nan)
            gf_result = {fp_sign, {9{1'b1}}, {14{1'b0}}};
        else if (fp_inf || overflow)
            gf_result = {fp_sign, gf_exp_calc[8:0] | {9{1'b1}}, {14{1'b0}}};
        else if (fp_zero || fp_denorm || underflow)
            gf_result = {fp_sign, {9{1'b0}}, {14{1'b0}}};
        else
            gf_result = {fp_sign, gf_exp_calc[8:0], gf_mant_trunc};
    end
    always @(posedge mclk or posedge rst) begin
        if(rst) begin a_reg<=0; conv_trigger<=0; end
        else begin conv_trigger <= frame_valid; if(frame_valid) a_reg <= a_r; end
    end
    reg [23:0] result_reg; reg result_ready;
    always @(posedge mclk or posedge rst) begin
