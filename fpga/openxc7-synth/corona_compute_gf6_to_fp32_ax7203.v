...
`timescale 1ns / 1ps
// corona_compute_gf6_to_fp32_ax7203 — GF6 to IEEE binary32 converter.
// Input: GF6 [S:1][E:2][M:3] (bias=1, HAS_INF=0)
...
module corona_compute_gf6_to_fp32_ax7203 (
    ...
    always @(posedge mclk or posedge rst) begin
        if(rst) begin frm<=0;fmt_r<=0;a_r<=0;frame_valid<=0; end
            ...
            if(rx_new) begin case(frm)
                ...
                3'd3: begin a_r[5:0]<=rx_byte[5:0];frm<=3'd4; end
    ...
    wire        gf_sign = a_reg[5];
    wire [1:0] gf_exp  = a_reg[4:3];
    wire [2:0] gf_mant = a_reg[2:0];
    ...
    // FP32 exp = gf_exp + 126 (= gf_exp - 1 + 127)
    wire [9:0] fp32_exp_norm = $unsigned(gf_exp) + 10'd126;
    wire [22:0] fp32_mant_norm = {gf_mant, 20'b0};
    ...
    always @(*) begin
        if (gf_zero)
            ...
            fp32_result = {gf_sign, 8'b0, {gf_mant, 20'b0}};
        else
