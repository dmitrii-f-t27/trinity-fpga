...
`timescale 1ns / 1ps
// corona_compute_fp48_e8m39_fma_ax7203 — FP48_E8M39 FMA on AX7203.
module corona_compute_fp48_e8m39_fma_ax7203 (
    ...
    reg [4:0] frm; reg [7:0] fmt_r; reg [47:0] a_r,b_r,c_r; reg frame_valid;
    always @(posedge mclk or posedge rst) begin
        if(rst) begin frm<=0;fmt_r<=0;a_r<=0;b_r<=0;c_r<=0;frame_valid<=0; end
            ...
            if(rx_new) begin case(frm)
                ...
                5'd14: begin b_r[47:40]<=rx_byte;frm<=5'd15; end
                ...
                5'd20: begin c_r[47:40]<=rx_byte;frm<=5'd21; end
    ...
    reg [47:0] a_reg,b_reg,c_reg; reg comp_trigger;
    wire [47:0] fmt_a=a_reg, fmt_b=b_reg, fmt_c=c_reg;
    wire f_sign_a = fmt_a[47];
    wire [7:0] f_exp_a = fmt_a[46:39];
    ...
    wire f_sign_b = fmt_b[47];
    wire [7:0] f_exp_b = fmt_b[46:39];
    ...
    wire f_sign_c = fmt_c[47];
    wire [7:0] f_exp_c = fmt_c[46:39];
    ...
    reg [47:0] q_result;
    always @(*) begin
        if(q_nan) q_result=48'd0;
        else if(q_zero) q_result=48'd0;
        else if(q_inf) q_result={q_sign, 8'd255, 39'd0};
        else if(q_exp >= 8'd254) q_result={q_sign, 8'd255, 39'd0};
        else if(q_exp < 8'd1) q_result={q_sign, 47'b0};
        else if(tgt_exp_s < 1) q_result={q_sign, 8'b0, q_mant[22:0]};
        else q_result={q_sign, tgt_exp_s[7:0], q_mant[22:0]};
    end
    reg [47:0] result_reg; reg result_ready;
    always @(posedge mclk or posedge rst) begin
