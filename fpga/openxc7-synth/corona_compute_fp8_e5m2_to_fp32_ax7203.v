...
`timescale 1ns / 1ps
// corona_compute_fp8_e5m2_to_fp32_ax7203 — FP8_E5M2 TO_FP32 on AX7203.
module corona_compute_fp8_e5m2_to_fp32_ax7203 (
    ...
    wire sv=a_reg[7]; wire [4:0] ev=a_reg[6:2]; wire [1:0] mv=a_reg[1:0];
    wire iz=(ev==0)&&(mv==0);
    ...
    always @(*) begin
        ...
        else fr={sv,{8'b0,ev}+9'd112,{21{1'b0}},mv};
    end
    always @(posedge mclk or posedge rst) begin
