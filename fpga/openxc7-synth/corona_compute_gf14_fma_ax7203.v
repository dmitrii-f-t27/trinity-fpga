...
`timescale 1ns / 1ps
// corona_compute_gf14_fma_ax7203 — GoldenFloat14 FMA (y = a*b + c) on AX7203.
// GF14: [S:1][E:5][M:8] = 14 bits, BIAS=15, HAS_INF=0.
// Pipelined: gf_mul_param (1 cycle) -> gf_adder_param (1 cycle). Latency: 2 cycles.
// 2 byte(s) per operand, 10-byte frame.
//
// Frame protocol (10 bytes TX):
//   AA 55 fmt a0 a1 b0 b1 c0 c1 trig
// Response (5 bytes RX):
//   A5 r0 r1 00 00

module corona_compute_gf14_fma_ax7203 (
    ...
    always @(posedge mclk or posedge rst) begin
        if(rst) begin frm<=0;fmt_r<=0;a_r<=0;b_r<=0;c_r<=0;frame_valid<=0; end
            ...
            if(rx_new) begin case(frm)
                ...
                4'd4: begin a_r[13:8]<=rx_byte[5:0];frm<=4'd5; end
                4'd5: begin b_r[7:0]<=rx_byte;frm<=4'd6; end
                4'd6: begin b_r[13:8]<=rx_byte[5:0];frm<=4'd7; end
                4'd7: begin c_r[7:0]<=rx_byte;frm<=4'd8; end
                4'd8: begin c_r[13:8]<=rx_byte[5:0];frm<=4'd9; end
