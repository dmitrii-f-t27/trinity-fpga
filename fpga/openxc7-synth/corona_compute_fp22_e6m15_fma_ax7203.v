...
`timescale 1ns / 1ps
// corona_compute_fp22_e6m15_fma_ax7203 — FP22_E6M15 FMA on AX7203.
module corona_compute_fp22_e6m15_fma_ax7203 (
    ...
    reg [3:0] frm; reg [7:0] fmt_r; reg [21:0] a_r,b_r,c_r; reg frame_valid;
    always @(posedge mclk or posedge rst) begin
    ...
    reg [21:0] a_reg,b_reg,c_reg; reg comp_trigger;
    wire [21:0] fmt_a=a_reg, fmt_b=b_reg, fmt_c=c_reg;
    wire f_sign_a = fmt_a[21];
    wire [5:0] f_exp_a = fmt_a[20:15];
    wire [14:0] f_mant_a = fmt_a[14:0];
    ...
    wire [22:0] f_mant32_norm_a = {1'b0, f_mant_a, 7'b0};
    reg [31:0] fp32_a;
    always @(*) begin
    ...
    wire f_sign_b = fmt_b[21];
    wire [5:0] f_exp_b = fmt_b[20:15];
    wire [14:0] f_mant_b = fmt_b[14:0];
    ...
    wire [22:0] f_mant32_norm_b = {1'b0, f_mant_b, 7'b0};
    reg [31:0] fp32_b;
    always @(*) begin
    ...
    wire f_sign_c = fmt_c[21];
    wire [5:0] f_exp_c = fmt_c[20:15];
    wire [14:0] f_mant_c = fmt_c[14:0];
    ...
    wire [22:0] f_mant32_norm_c = {1'b0, f_mant_c, 7'b0};
    reg [31:0] fp32_c;
    always @(*) begin
    ...
    reg [21:0] q_result;
    always @(*) begin
        if(q_nan) q_result=22'd0;
        else if(q_zero) q_result=22'd0;
        else if(q_inf) q_result={q_sign, 6'd63, 15'd0};
        else if(q_exp >= 8'd160) q_result={q_sign, 6'd63, 15'd0};
        else if(q_exp < 8'd97) q_result={q_sign, 21'b0};
        else if(tgt_exp_s < 1) q_result={q_sign, 6'b0, q_mant[22:8]};
        else q_result={q_sign, tgt_exp_s[5:0], q_mant[22:8]};
    end
    reg [31:0] result_reg; reg result_ready;
    always @(posedge mclk or posedge rst) begin
        if(rst) begin result_reg<=0;result_ready<=0; end
        else begin result_ready<=add_ovld;
            if(add_ovld) result_reg<={10'b0,q_result};
        end
    end
