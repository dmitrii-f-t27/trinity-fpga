...
`timescale 1ns / 1ps
// corona_compute_fp115_e8m106_fma_ax7203 — FP115_E8M106 FMA on AX7203.
module corona_compute_fp115_e8m106_fma_ax7203 (
    ...
    reg [5:0] frm; reg [7:0] fmt_r; reg [114:0] a_r,b_r,c_r; reg frame_valid;
    always @(posedge mclk or posedge rst) begin
        if(rst) begin frm<=0;fmt_r<=0;a_r<=0;b_r<=0;c_r<=0;frame_valid<=0; end
            ...
            if(rx_new) begin case(frm)
                ...
                6'd17: begin a_r[114:112]<=rx_byte;frm<=6'd18; end
                ...
                6'd32: begin b_r[114:112]<=rx_byte;frm<=6'd33; end
                ...
                6'd47: begin c_r[114:112]<=rx_byte;frm<=6'd48; end
    ...
    reg [114:0] a_reg,b_reg,c_reg; reg comp_trigger;
    wire [114:0] fmt_a=a_reg, fmt_b=b_reg, fmt_c=c_reg;
    wire f_sign_a = fmt_a[114];
    wire [7:0] f_exp_a = fmt_a[113:106];
    wire [105:0] f_mant_a = fmt_a[105:0];
    ...
    wire f_sign_b = fmt_b[114];
    wire [7:0] f_exp_b = fmt_b[113:106];
    wire [105:0] f_mant_b = fmt_b[105:0];
    ...
    wire f_sign_c = fmt_c[114];
    wire [7:0] f_exp_c = fmt_c[113:106];
    wire [105:0] f_mant_c = fmt_c[105:0];
    ...
    wire [22:0] f_mant32_c = {f_mant_c, -83'b0};
    ...
    reg [114:0] q_result;
    always @(*) begin
        if(q_nan) q_result=115'd0;
        else if(q_zero) q_result=115'd0;
        else if(q_inf) q_result={q_sign, 8'd255, 106'd0};
        else if(q_exp >= 8'd254) q_result={q_sign, 8'd255, 106'd0};
        else if(q_exp < 8'd1) q_result={q_sign, 114'b0};
        else if(tgt_exp_s < 1) q_result={q_sign, 8'b0, q_mant[22:0]};
        else q_result={q_sign, tgt_exp_s[7:0], q_mant[22:0]};
    end
    reg [114:0] result_reg; reg result_ready;
    always @(posedge mclk or posedge rst) begin
