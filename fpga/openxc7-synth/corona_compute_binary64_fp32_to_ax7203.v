...
`timescale 1ns / 1ps
// corona_compute_binary64_fp32_to_ax7203 — BINARY64 FP32_TO on AX7203.
module corona_compute_binary64_fp32_to_ax7203 (
    ...
    wire sign_v=a_reg[31]; wire [7:0] exp_v=a_reg[30:23]; wire [22:0] mant_v=a_reg[22:0];
    wire is_zero=(exp_v==0)&&(mant_v==0);
    wire is_nan=(exp_v==8'hFF)&&(mant_v!=0);
    wire is_inf=(exp_v==8'hFF)&&(mant_v==0);
    reg [63:0] result_v;
    always @(*) begin
        if(is_nan) result_v={1'b0,11'b11111111111,20'b0,1'b1,32'b0};
        else if(is_inf) result_v={sign_v,11'b11111111111,52'b0};
        else if(is_zero) result_v=64'b0;
        else result_v={sign_v,{3'b0,exp_v},mant_v,29'b0};
    end
    always @(posedge mclk or posedge rst) begin
    ...
    always @(posedge mclk or posedge rst) begin
        if(rst) begin result_reg<=0;result_ready<=0; end
            ...
            if(conv_trigger) result_reg<=result_v;
        end
    end
