...
`timescale 1ns / 1ps
// corona_compute_fp87_e8m78_fma_ax7203 — FP87_E8M78 FMA on AX7203.
module corona_compute_fp87_e8m78_fma_ax7203 (
    ...
    reg [5:0] frm; reg [7:0] fmt_r; reg [86:0] a_r,b_r,c_r; reg frame_valid;
    always @(posedge mclk or posedge rst) begin
        if(rst) begin frm<=0;fmt_r<=0;a_r<=0;b_r<=0;c_r<=0;frame_valid<=0; end
            ...
            if(rx_new) begin case(frm)
                ...
                6'd13: begin a_r[86:80]<=rx_byte;frm<=6'd14; end
                ...
                6'd24: begin b_r[86:80]<=rx_byte;frm<=6'd25; end
                ...
                6'd35: begin c_r[86:80]<=rx_byte;frm<=6'd36; end
    ...
    reg [86:0] a_reg,b_reg,c_reg; reg comp_trigger;
    wire [86:0] fmt_a=a_reg, fmt_b=b_reg, fmt_c=c_reg;
    wire f_sign_a = fmt_a_a[86];
    wire [7:0] f_exp_a = fmt_a_a[85:78];
    wire [77:0] f_mant_a = fmt_a_a[77:0];
    ...
    wire f_sign_b = fmt_b_b[86];
    wire [7:0] f_exp_b = fmt_b_b[85:78];
    wire [77:0] f_mant_b = fmt_b_b[77:0];
    ...
    wire f_sign_c = fmt_c_c[86];
    wire [7:0] f_exp_c = fmt_c_c[85:78];
    wire [77:0] f_mant_c = fmt_c_c[77:0];
    ...
    reg [86:0] q_result;
    always @(*) begin
        if(q_nan) q_result=87'd0;
        else if(q_zero) q_result=87'd0;
        else if(q_inf) q_result={q_sign, 8'd255, 78'd0};
        else if(q_exp >= 8'd254) q_result={q_sign, 8'd255, 78'd0};
        else if(q_exp < 8'd1) q_result={q_sign, 86'b0};
        else if(tgt_exp_s < 1) q_result={q_sign, 8'b0, q_mant[22:0]};
        else q_result={q_sign, tgt_exp_s[7:0], q_mant[22:0]};
    end
    reg [86:0] result_reg; reg result_ready;
    always @(posedge mclk or posedge rst) begin
