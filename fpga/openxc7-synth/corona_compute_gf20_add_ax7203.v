...
`timescale 1ns / 1ps
// corona_compute_gf20_add_ax7203 — GoldenFloat20 ADD on AX7203.
// GF20: [S:1][E:7][M:12] = 20 bits, BIAS=63, HAS_INF=0.
// 3 bytes per operand, 9-byte frame.
//
// Frame protocol (9 bytes TX):
//   AA 55 fmt a0 a1 a2 b0 b1 b2 trig
// Response (5 bytes RX):
//   A5 r0 r1 r2 00

module corona_compute_gf20_add_ax7203 (
    ...
    wire [19:0] comp_result;

    gf_adder_param #(.EXP_BITS(7), .MANT_BITS(12), .HAS_INF(0)) u_comp (
        .clk(mclk), .rst(rst),
    ...
    always @(posedge mclk or posedge rst) begin
        if(rst) begin a_reg<=0; b_reg<=0; comp_trigger<=0; end
            ...
            if(frame_valid) begin a_reg[7:0]<=a_r[7:0]; a_reg[15:8]<=a_r[15:8]; a_reg[19:16]<=a_r[19:16]; b_reg[7:0]<=b_r[7:0]; b_reg[15:8]<=b_r[15:8]; b_reg[19:16]<=b_r[19:16]; end
        end
    end
