...
`timescale 1ns / 1ps
// corona_decode_takum16_ax7203 — takum16 (Hunhold logarithmic) decode on AX7203.
// Frame: AA 55 fmt code_lo code_hi trig (6 bytes) -> A5 r0 r1 r2 r3 (32-bit FP32 LE).
// Infrastructure identical to corona_decode_e8m0_ax7203; only the decoder differs
// (takum16_decode: 65536-entry BRAM LUT, takum16 -> FP32).
module corona_decode_takum16_ax7203 (
    ...
    wire [31:0] result;
    takum16_decode u_dec (.clk(mclk), .takum16_in(code_r[15:0]), .fp32_out(result));
    assign led[2] = |result;
