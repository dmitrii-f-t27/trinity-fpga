...
`timescale 1ns / 1ps
// corona_compute_gf20_to_fp32_ax7203 — GF20 to IEEE binary32 converter.
// Input: GF20 [S:1][E:7][M:12] (bias=63, HAS_INF=0)
// Output: FP32 [S:1][E:8][M:23] (bias=127)
//
// Frame protocol (7 bytes TX):
//   AA 55 fmt a0 a1 a2 trig
// Response (5 bytes RX):
//   A5 r0 r1 r2 r3

module corona_compute_gf20_to_fp32_ax7203 (
    ...
    // ---- Frame FSM ----
    reg [2:0] frm; reg [7:0] fmt_r; reg [19:0] a_r; reg frame_valid;
    always @(posedge mclk or posedge rst) begin
        if(rst) begin frm<=0;fmt_r<=0;a_r<=0;frame_valid<=0; end
            ...
            if(rx_new) begin case(frm)
                ...
                3'd5: begin a_r[19:16]<=rx_byte[3:0];frm<=3'd6; end
    ...
    // ---- GF20->FP32 conversion ----
    reg [19:0] a_reg;
    reg conv_trigger;

    wire        gf_sign = a_reg[19];
    wire [6:0] gf_exp  = a_reg[18:12];
    wire [11:0] gf_mant = a_reg[11:0];
    ...
    // FP32 exp = gf_exp + 64 (= gf_exp - 63 + 127)
    wire [9:0] fp32_exp_norm = $unsigned(gf_exp) + 10'd64;
    wire [22:0] fp32_mant_norm = {gf_mant, 11'b0};

    reg [31:0] fp32_result;
    always @(*) begin
        if (gf_zero)
            fp32_result = {gf_sign, 8'b0, 23'b0};
        else if (gf_denorm)
            fp32_result = {gf_sign, 8'b0, {gf_mant, 11'b0}};
        else
            fp32_result = {gf_sign, fp32_exp_norm[7:0], fp32_mant_norm};
    end
    ...
    always @(posedge mclk or posedge rst) begin
        if(rst) begin responding<=0;tx_idx<=0;tcnt<=BAUD_DIV-1;tbi<=0;tsr<=10'h3FF;uart_tx<=1;
            ...
            if(result_ready) begin
                tx_buf0<=8'hA5; tx_buf1<=result_reg[7:0]; tx_buf2<=result_reg[15:8]; tx_buf3<=result_reg[23:16]; tx_buf4<=result_reg[31:24];
