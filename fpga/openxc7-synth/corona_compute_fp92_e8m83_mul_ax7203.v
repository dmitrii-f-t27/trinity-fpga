...
`timescale 1ns / 1ps
// corona_compute_fp92_e8m83_mul_ax7203 — FP92_E8M83 MUL on AX7203.
module corona_compute_fp92_e8m83_mul_ax7203 (
    ...
    reg [4:0] frm; reg [7:0] fmt_r; reg [91:0] a_r,b_r; reg frame_valid;
    always @(posedge mclk or posedge rst) begin
        if(rst) begin frm<=0;fmt_r<=0;a_r<=0;b_r<=0;frame_valid<=0; end
            ...
            if(rx_new) begin case(frm)
                ...
                5'd14: begin a_r[91:88]<=rx_byte;frm<=5'd15; end
                ...
                5'd26: begin b_r[91:88]<=rx_byte;frm<=5'd27; end
    ...
    reg [91:0] a_reg,b_reg; reg comp_trigger;
    wire [91:0] fmt_a=a_reg, fmt_b=b_reg;
    wire f_sign_a = fmt_a_a[91];
    wire [7:0] f_exp_a = fmt_a_a[90:83];
    wire [82:0] f_mant_a = fmt_a_a[82:0];
    ...
    wire [22:0] f_mant32_a = {f_mant_a, -60'b0};
    ...
    wire f_sign_b = fmt_b_b[91];
    wire [7:0] f_exp_b = fmt_b_b[90:83];
    wire [82:0] f_mant_b = fmt_b_b[82:0];
    ...
    wire [22:0] f_mant32_b = {f_mant_b, -60'b0};
    ...
    reg [91:0] q_result;
    always @(*) begin
        if(q_nan) q_result=92'd0;
        else if(q_zero) q_result=92'd0;
        else if(q_inf) q_result={q_sign, 8'd255, 83'd0};
        else if(q_exp >= 8'd254) q_result={q_sign, 8'd255, 83'd0};
        else if(q_exp < 8'd1) q_result={q_sign, 91'b0};
        else if(tgt_exp_s < 1) q_result={q_sign, 8'b0, q_mant[22:0]};
        else q_result={q_sign, tgt_exp_s[7:0], q_mant[22:0]};
    end
    reg [91:0] result_reg; reg result_ready;
    always @(posedge mclk or posedge rst) begin
