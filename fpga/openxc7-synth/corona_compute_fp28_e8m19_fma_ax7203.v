...
`timescale 1ns / 1ps
// corona_compute_fp28_e8m19_fma_ax7203 — FP28_E8M19 FMA on AX7203.
module corona_compute_fp28_e8m19_fma_ax7203 (
    ...
    reg [3:0] frm; reg [7:0] fmt_r; reg [27:0] a_r,b_r,c_r; reg frame_valid;
    always @(posedge mclk or posedge rst) begin
        if(rst) begin frm<=0;fmt_r<=0;a_r<=0;b_r<=0;c_r<=0;frame_valid<=0; end
            ...
            if(rx_new) begin case(frm)
                ...
                4'd6: begin a_r[27:24]<=rx_byte;frm<=4'd7; end
                ...
                4'd10: begin b_r[27:24]<=rx_byte;frm<=4'd11; end
                ...
                4'd14: begin c_r[27:24]<=rx_byte;frm<=4'd15; end
    ...
    reg [27:0] a_reg,b_reg,c_reg; reg comp_trigger;
    wire [27:0] fmt_a=a_reg, fmt_b=b_reg, fmt_c=c_reg;
    wire f_sign_a = fmt_a[27];
    wire [7:0] f_exp_a = fmt_a[26:19];
    wire [18:0] f_mant_a = fmt_a[18:0];
    ...
    wire [22:0] f_mant32_norm_a = {1'b0, f_mant_a, 3'b0};
    reg [31:0] fp32_a;
    always @(*) begin
    ...
    wire f_sign_b = fmt_b[27];
    wire [7:0] f_exp_b = fmt_b[26:19];
    wire [18:0] f_mant_b = fmt_b[18:0];
    ...
    wire [22:0] f_mant32_norm_b = {1'b0, f_mant_b, 3'b0};
    reg [31:0] fp32_b;
    always @(*) begin
    ...
    wire f_sign_c = fmt_c[27];
    wire [7:0] f_exp_c = fmt_c[26:19];
    wire [18:0] f_mant_c = fmt_c[18:0];
    ...
    wire [22:0] f_mant32_norm_c = {1'b0, f_mant_c, 3'b0};
    reg [31:0] fp32_c;
    always @(*) begin
    ...
    reg [27:0] q_result;
    always @(*) begin
        if(q_nan) q_result=28'd0;
        else if(q_zero) q_result=28'd0;
        else if(q_inf) q_result={q_sign, 8'd255, 19'd0};
        else if(q_exp >= 8'd254) q_result={q_sign, 8'd255, 19'd0};
        else if(q_exp < 8'd1) q_result={q_sign, 27'b0};
        else if(tgt_exp_s < 1) q_result={q_sign, 8'b0, q_mant[22:4]};
        else q_result={q_sign, tgt_exp_s[7:0], q_mant[22:4]};
    end
    reg [31:0] result_reg; reg result_ready;
    always @(posedge mclk or posedge rst) begin
        if(rst) begin result_reg<=0;result_ready<=0; end
        else begin result_ready<=add_ovld;
            if(add_ovld) result_reg<={4'b0,q_result};
        end
    end
