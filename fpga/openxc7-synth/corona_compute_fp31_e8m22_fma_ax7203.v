...
`timescale 1ns / 1ps
// corona_compute_fp31_e8m22_fma_ax7203 — FP31_E8M22 FMA on AX7203.
module corona_compute_fp31_e8m22_fma_ax7203 (
    ...
    reg [3:0] frm; reg [7:0] fmt_r; reg [30:0] a_r,b_r,c_r; reg frame_valid;
    always @(posedge mclk or posedge rst) begin
        if(rst) begin frm<=0;fmt_r<=0;a_r<=0;b_r<=0;c_r<=0;frame_valid<=0; end
            ...
            if(rx_new) begin case(frm)
                ...
                4'd6: begin a_r[30:24]<=rx_byte;frm<=4'd7; end
                ...
                4'd10: begin b_r[30:24]<=rx_byte;frm<=4'd11; end
                ...
                4'd14: begin c_r[30:24]<=rx_byte;frm<=4'd15; end
    ...
    reg [30:0] a_reg,b_reg,c_reg; reg comp_trigger;
    wire [30:0] fmt_a=a_reg, fmt_b=b_reg, fmt_c=c_reg;
    wire f_sign_a = fmt_a[30];
    wire [7:0] f_exp_a = fmt_a[29:22];
    ...
    wire f_sign_b = fmt_b[30];
    wire [7:0] f_exp_b = fmt_b[29:22];
    ...
    wire f_sign_c = fmt_c[30];
    wire [7:0] f_exp_c = fmt_c[29:22];
    ...
    reg [30:0] q_result;
    always @(*) begin
        if(q_nan) q_result=31'd0;
        else if(q_zero) q_result=31'd0;
        else if(q_inf) q_result={q_sign, 8'd255, 22'd0};
        else if(q_exp >= 8'd254) q_result={q_sign, 8'd255, 22'd0};
        else if(q_exp < 8'd1) q_result={q_sign, 30'b0};
        else if(tgt_exp_s < 1) q_result={q_sign, 8'b0, q_mant[22:1]};
        else q_result={q_sign, tgt_exp_s[7:0], q_mant[22:1]};
    end
    reg [31:0] result_reg; reg result_ready;
    always @(posedge mclk or posedge rst) begin
        if(rst) begin result_reg<=0;result_ready<=0; end
        else begin result_ready<=add_ovld;
            if(add_ovld) result_reg<={1'b0,q_result};
        end
    end
