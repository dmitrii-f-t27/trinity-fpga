...
`timescale 1ns / 1ps
// corona_compute_int64_to_fp32_ax7203 — INT64 TO_FP32 on AX7203.
module corona_compute_int64_to_fp32_ax7203 (
    ...
    reg [4:0] frm; reg [7:0] fmt_r; reg [63:0] a_r; reg frame_valid;
    always @(posedge mclk or posedge rst) begin
        if(rst) begin frm<=0;fmt_r<=0;a_r<=0;frame_valid<=0; end
            ...
            if(rx_new) begin case(frm)
                ...
                5'd11: begin frame_valid<=1;frm<=0; end
            endcase end
        end
    end
    assign led[1]=frame_valid;
    reg [63:0] a_reg; reg conv_trigger;
    wire signed [63:0] sint = a_reg;
    wire neg = sint[63];
    wire [62:0] absval = neg ? -sint[62:0] : sint[62:0];
    reg [31:0] fp32_result;
    reg [6:0] lz; integer k; reg [62:0] norm;
    always @(*) begin
        if(sint == 0) fp32_result = 32'h00000000;
        else begin
            lz = 0;
            for(k=62; k>=0; k=k-1) begin
                if(lz==0 && absval[k]) lz = k[6:0];
            end
            norm = absval << (62 - lz);
            fp32_result = {neg, {1'b0, lz[6:0]} + 8'd127, norm[62:40]};
        end
    end
    always @(posedge mclk or posedge rst) begin
