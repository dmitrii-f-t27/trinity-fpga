...
`timescale 1ns / 1ps
// corona_compute_fp12_e4m7_fma_ax7203 — FP12_E4M7 FMA on AX7203.
module corona_compute_fp12_e4m7_fma_ax7203 (
    ...
    reg [3:0] frm; reg [7:0] fmt_r; reg [11:0] a_r,b_r,c_r; reg frame_valid;
    always @(posedge mclk or posedge rst) begin
        if(rst) begin frm<=0;fmt_r<=0;a_r<=0;b_r<=0;c_r<=0;frame_valid<=0; end
            ...
            if(rx_new) begin case(frm)
                ...
                4'd4: begin a_r[11:8]<=rx_byte;frm<=4'd5; end
                4'd5: begin b_r[7:0]<=rx_byte;frm<=4'd6; end
                4'd6: begin b_r[11:8]<=rx_byte;frm<=4'd7; end
                4'd7: begin c_r[7:0]<=rx_byte;frm<=4'd8; end
                4'd8: begin c_r[11:8]<=rx_byte;frm<=4'd9; end
    ...
    reg [11:0] a_reg,b_reg,c_reg; reg comp_trigger;
    wire [11:0] fmt_a=a_reg, fmt_b=b_reg, fmt_c=c_reg;
    wire f_sign_a = fmt_a[11];
    wire [3:0] f_exp_a = fmt_a[10:7];
    wire [6:0] f_mant_a = fmt_a[6:0];
    wire f_zero_a = (f_exp_a == 0) && ((f_mant_a == 0));
    wire f_inf_a = (f_exp_a == 15) && ((f_mant_a == 0));
    wire f_nan_a = (f_exp_a == 15) && ((f_mant_a != 0));
    wire f_sub_a = (f_exp_a == 0) && ((f_mant_a != 0));
    wire signed [13:0] f_de_a = $signed({1'b0, f_exp_a}) - 14'sd7 + 14'sd127;
    wire [7:0] f_exp32_a = (f_de_a > 14'sd254) ? 8'd254 : (f_de_a < 0) ? 8'd0 : f_de_a[7:0];
    wire [22:0] f_mant32_a = {f_mant_a, 16'b0};
    wire [22:0] f_mant32_norm_a = {1'b0, f_mant_a, 15'b0};
    reg [31:0] fp32_a;
    always @(*) begin
        ...
        else if(f_sub_a) fp32_a={f_sign_a, 8'd121, f_mant32_norm_a};
        else fp32_a={f_sign_a, f_exp32_a, f_mant32_a};
    end
    wire f_sign_b = fmt_b[11];
    wire [3:0] f_exp_b = fmt_b[10:7];
    wire [6:0] f_mant_b = fmt_b[6:0];
    wire f_zero_b = (f_exp_b == 0) && ((f_mant_b == 0));
    wire f_inf_b = (f_exp_b == 15) && ((f_mant_b == 0));
    wire f_nan_b = (f_exp_b == 15) && ((f_mant_b != 0));
    wire f_sub_b = (f_exp_b == 0) && ((f_mant_b != 0));
    wire signed [13:0] f_de_b = $signed({1'b0, f_exp_b}) - 14'sd7 + 14'sd127;
    wire [7:0] f_exp32_b = (f_de_b > 14'sd254) ? 8'd254 : (f_de_b < 0) ? 8'd0 : f_de_b[7:0];
    wire [22:0] f_mant32_b = {f_mant_b, 16'b0};
    wire [22:0] f_mant32_norm_b = {1'b0, f_mant_b, 15'b0};
    reg [31:0] fp32_b;
    always @(*) begin
        ...
        else if(f_sub_b) fp32_b={f_sign_b, 8'd121, f_mant32_norm_b};
        else fp32_b={f_sign_b, f_exp32_b, f_mant32_b};
    end
    wire f_sign_c = fmt_c[11];
    wire [3:0] f_exp_c = fmt_c[10:7];
    wire [6:0] f_mant_c = fmt_c[6:0];
    wire f_zero_c = (f_exp_c == 0) && ((f_mant_c == 0));
    wire f_inf_c = (f_exp_c == 15) && ((f_mant_c == 0));
    wire f_nan_c = (f_exp_c == 15) && ((f_mant_c != 0));
    wire f_sub_c = (f_exp_c == 0) && ((f_mant_c != 0));
    wire signed [13:0] f_de_c = $signed({1'b0, f_exp_c}) - 14'sd7 + 14'sd127;
    wire [7:0] f_exp32_c = (f_de_c > 14'sd254) ? 8'd254 : (f_de_c < 0) ? 8'd0 : f_de_c[7:0];
    wire [22:0] f_mant32_c = {f_mant_c, 16'b0};
    wire [22:0] f_mant32_norm_c = {1'b0, f_mant_c, 15'b0};
    reg [31:0] fp32_c;
    always @(*) begin
        ...
        else if(f_sub_c) fp32_c={f_sign_c, 8'd121, f_mant32_norm_c};
    ...
    wire signed [13:0] tgt_exp_s = $signed({1'b0, q_exp}) - 14'sd127 + 14'sd7;
    reg [11:0] q_result;
    always @(*) begin
        if(q_nan) q_result=12'd0;
        else if(q_zero) q_result=12'd0;
        else if(q_inf) q_result={q_sign, 4'd15, 7'd0};
        else if(q_exp >= 8'd136) q_result={q_sign, 4'd15, 7'd0};
        else if(q_exp < 8'd121) q_result={q_sign, 11'b0};
        else if(tgt_exp_s < 1) q_result={q_sign, 4'b0, q_mant[22:16]};
        else q_result={q_sign, tgt_exp_s[3:0], q_mant[22:16]};
    end
    reg [31:0] result_reg; reg result_ready;
    always @(posedge mclk or posedge rst) begin
        if(rst) begin result_reg<=0;result_ready<=0; end
        else begin result_ready<=add_ovld;
            if(add_ovld) result_reg<={20'b0,q_result};
        end
    end
