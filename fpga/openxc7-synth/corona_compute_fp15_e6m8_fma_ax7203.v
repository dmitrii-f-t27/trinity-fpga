...
`timescale 1ns / 1ps
// corona_compute_fp15_e6m8_fma_ax7203 — FP15_E6M8 FMA on AX7203.
module corona_compute_fp15_e6m8_fma_ax7203 (
    ...
    reg [3:0] frm; reg [7:0] fmt_r; reg [14:0] a_r,b_r,c_r; reg frame_valid;
    always @(posedge mclk or posedge rst) begin
        if(rst) begin frm<=0;fmt_r<=0;a_r<=0;b_r<=0;c_r<=0;frame_valid<=0; end
            ...
            if(rx_new) begin case(frm)
                ...
                4'd4: begin a_r[14:8]<=rx_byte;frm<=4'd5; end
                4'd5: begin b_r[7:0]<=rx_byte;frm<=4'd6; end
                4'd6: begin b_r[14:8]<=rx_byte;frm<=4'd7; end
                4'd7: begin c_r[7:0]<=rx_byte;frm<=4'd8; end
                4'd8: begin c_r[14:8]<=rx_byte;frm<=4'd9; end
    ...
    reg [14:0] a_reg,b_reg,c_reg; reg comp_trigger;
    wire [14:0] fmt_a=a_reg, fmt_b=b_reg, fmt_c=c_reg;
    wire f_sign_a = fmt_a[14];
    wire [5:0] f_exp_a = fmt_a[13:8];
    ...
    wire f_sign_b = fmt_b[14];
    wire [5:0] f_exp_b = fmt_b[13:8];
    ...
    wire f_sign_c = fmt_c[14];
    wire [5:0] f_exp_c = fmt_c[13:8];
    ...
    reg [14:0] q_result;
    always @(*) begin
        if(q_nan) q_result=15'd0;
        else if(q_zero) q_result=15'd0;
        else if(q_inf) q_result={q_sign, 6'd63, 8'd0};
        else if(q_exp >= 8'd128) q_result={q_sign, 6'd63, 8'd0};
        else if(q_exp < 8'd65) q_result={q_sign, 14'b0};
        else if(tgt_exp_s < 1) q_result={q_sign, 6'b0, q_mant[22:15]};
        else q_result={q_sign, tgt_exp_s[5:0], q_mant[22:15]};
    end
    reg [31:0] result_reg; reg result_ready;
    always @(posedge mclk or posedge rst) begin
        if(rst) begin result_reg<=0;result_ready<=0; end
        else begin result_ready<=add_ovld;
            if(add_ovld) result_reg<={17'b0,q_result};
        end
    end
