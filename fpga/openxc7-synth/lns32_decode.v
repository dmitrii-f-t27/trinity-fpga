...
// lns32_decode — 32-bit LNS (1 sign + 31-bit signed log, scale 128) → FP32.
// Same 128-entry antilog LUT as lns16, wider integer (24-bit vs 8-bit).
`default_nettype none
`timescale 1ns / 1ps
module lns32_decode (
    input  wire [31:0] lns_in,
    output reg  [31:0] fp32_out,
    output wire        is_zero
);
    wire        sign      = lns_in[31];
    wire signed [30:0] signed_log = $signed(lns_in[30:0]);
    assign is_zero = (lns_in == 32'h0000);
    wire signed [23:0] int_part = signed_log >>> 7;
    wire [6:0]        frac_part = lns_in[6:0];
    ...
    wire signed [24:0] exp_calc = int_part + 25'sd127;
    always @(*) begin
        if (is_zero) fp32_out = 32'h00000000;
        else if (exp_calc > 25'sd254) fp32_out = {sign, 8'hFF, 23'd0};
        else if (exp_calc < 25'sd1)   fp32_out = {sign, 31'd0};
        else                          fp32_out = {sign, exp_calc[7:0], frac_mant};
    end
endmodule
