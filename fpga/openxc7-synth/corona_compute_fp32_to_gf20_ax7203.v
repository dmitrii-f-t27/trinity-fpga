...
`timescale 1ns / 1ps
// corona_compute_fp32_to_gf20_ax7203 — IEEE binary32 to GF20 converter.
// Input: FP32 [S:1][E:8][M:23] (bias=127)
// Output: GF20 [S:1][E:7][M:12] (bias=63, HAS_INF=0)
// Mantissa truncation. Overflow -> max, underflow -> zero.
module corona_compute_fp32_to_gf20_ax7203 (
    ...
    wire signed [9:0] gf_exp_calc = $signed({2'b0, fp_exp}) + 10'sd-64;
    wire overflow  = (gf_exp_calc > 10'sd127);
    wire underflow = (gf_exp_calc < 10'sd1);
    wire [11:0] gf_mant_trunc = fp_mant[22:11];
    reg [19:0] gf_result;
    always @(*) begin
        if (fp_nan)
            gf_result = {fp_sign, {7{1'b1}}, {12{1'b0}}};
        else if (fp_inf || overflow)
            gf_result = {fp_sign, gf_exp_calc[6:0] | {7{1'b1}}, {12{1'b0}}};
        else if (fp_zero || fp_denorm || underflow)
            gf_result = {fp_sign, {7{1'b0}}, {12{1'b0}}};
        else
            gf_result = {fp_sign, gf_exp_calc[6:0], gf_mant_trunc};
    end
    always @(posedge mclk or posedge rst) begin
        if(rst) begin a_reg<=0; conv_trigger<=0; end
        else begin conv_trigger <= frame_valid; if(frame_valid) a_reg <= a_r; end
    end
    reg [19:0] result_reg; reg result_ready;
    always @(posedge mclk or posedge rst) begin
    ...
    always @(posedge mclk or posedge rst) begin
        if(rst) begin responding<=0;tx_idx<=0;tcnt<=BAUD_DIV-1;tbi<=0;tsr<=10'h3FF;uart_tx<=1;
            ...
            if(result_ready) begin
                tx_buf0<=8'hA5; tx_buf1<=result_reg[7:0]; tx_buf2<=result_reg[15:8]; tx_buf3<=result_reg[19:16]; tx_buf4<=8'h00;
