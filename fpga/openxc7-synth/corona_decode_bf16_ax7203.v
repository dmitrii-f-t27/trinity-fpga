...
`timescale 1ns / 1ps
// corona_decode_bf16_ax7203 — single-decoder (bf16) decode-conformance on AX7203.
// Same frame/response protocol as corona_decode_top_ax7203 (corona_decode_host
// compatible, fmt=0=bf16), but ONLY bf16 instantiated — routing complexity =
// gf8_clean (1 compute block + UART) to avoid the openXC7/xc7a200t routing
// instability that blocked the 5-parallel-decoder version.
// Frame: AA 55 fmt code_lo code_hi <trig> -> A5 r0 r1 r2 r3 (bf16->fp32, LE).
module corona_decode_bf16_ax7203 (
    input  wire        rst_n,
    input  wire        uart_rx,
    output reg         uart_tx,
    output wire [3:0]  led
    ...
    // ---- single bf16 decoder (routing-friendly: 1 compute block) ----
    wire [31:0] bf16_o;
    bf16_decode u_bf16 (.bf16_in(code_r[15:0]), .fp32_out(bf16_o), .is_zero(), .is_inf(), .is_nan());
    wire [31:0] result = bf16_o;  // always bf16 (fmt ignored for single-decoder build)
    assign led[2] = |result;
