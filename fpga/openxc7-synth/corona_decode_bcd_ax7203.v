...
`timescale 1ns / 1ps
// corona_decode_bcd_ax7203 — 2-digit packed BCD decode on AX7203.
// Single-decoder build: CFGMCLK (~70 MHz, STARTUPE2) clock, BAUD_DIV 434 (~161290).
// Frame: AA 55 fmt code_lo code_hi trig -> A5 r0 r1 r2 r3 (32-bit LE, bin zero-extended).
// Identical infrastructure to corona_decode_e8m0_ax7203; only the decoder differs
// (bcd_decode: {tens,ones} -> tens*10+ones, 7-bit, valid flag; result zero-extended).
module corona_decode_bcd_ax7203 (
    ...
    wire [6:0] bin;
    wire       valid;
    bcd_decode u_dec (.bcd_in(code_r[7:0]), .bin_out(bin), .valid(valid));
    wire [31:0] result = {25'b0, bin};
    assign led[2] = valid;
