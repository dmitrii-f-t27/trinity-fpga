...
`timescale 1ns / 1ps
// corona_compute_tf32_cmp_ax7203 — TF32 CMP on AX7203.
module corona_compute_tf32_cmp_ax7203 (
    ...
    wire sa=a_reg[18]; wire [7:0] ea=a_reg[17:10]; wire [9:0] ma=a_reg[9:0];
    wire sb=b_reg[18]; wire [7:0] eb=b_reg[17:10]; wire [9:0] mb=b_reg[9:0];
    wire az=(ea==0)&&(ma==0); wire bz=(eb==0)&&(mb==0);
    wire an=(1)&&(ea=={8{1'b1}})&&(ma!=0); wire bn=(1)&&(eb=={8{1'b1}})&&(mb!=0);
    wire [17:0] aa={ea,ma}; wire [17:0] ab={eb,mb};
    wire ml=(aa<ab); wire me=(aa==ab);
    wire ce=(az&&bz)||(~(an|bn)&&(sa==sb)&&((az&&bz)||me));
    wire cl=(an|bn)?1'b0:(az&&bz)?1'b0:(sa&&~sb)?1'b1:(~sa&&sb)?1'b0:(sa&&sb)?(aa>ab):ml;
    wire cd=cl|ce;
    always @(posedge mclk or posedge rst) begin
    ...
    wire cr=(op_reg==8'h00)?ce:(op_reg==8'h01)?cl:(op_reg==8'h02)?cd:1'b0;
