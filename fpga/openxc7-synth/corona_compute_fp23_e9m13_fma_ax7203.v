...
`timescale 1ns / 1ps
// corona_compute_fp23_e9m13_fma_ax7203 — FP23_E9M13 FMA on AX7203.
module corona_compute_fp23_e9m13_fma_ax7203 (
    ...
    reg [3:0] frm; reg [7:0] fmt_r; reg [22:0] a_r,b_r,c_r; reg frame_valid;
    always @(posedge mclk or posedge rst) begin
    ...
    reg [22:0] a_reg,b_reg,c_reg; reg comp_trigger;
    wire [22:0] fmt_a=a_reg, fmt_b=b_reg, fmt_c=c_reg;
    wire f_sign_a = fmt_a[22];
    wire [8:0] f_exp_a = fmt_a[21:13];
    wire [12:0] f_mant_a = fmt_a[12:0];
    ...
    wire f_sign_b = fmt_b[22];
    wire [8:0] f_exp_b = fmt_b[21:13];
    wire [12:0] f_mant_b = fmt_b[12:0];
    ...
    wire f_sign_c = fmt_c[22];
    wire [8:0] f_exp_c = fmt_c[21:13];
    wire [12:0] f_mant_c = fmt_c[12:0];
    ...
    reg [22:0] q_result;
    always @(*) begin
        if(q_nan) q_result=23'd0;
        else if(q_zero) q_result=23'd0;
        else if(q_inf) q_result={q_sign, 9'd511, 13'd0};
        else if(q_exp >= 8'd254) q_result={q_sign, 9'd511, 13'd0};
        else if(q_exp < 8'd0) q_result={q_sign, 22'b0};
        else if(tgt_exp_s < 1) q_result={q_sign, 9'b0, q_mant[22:10]};
        else q_result={q_sign, tgt_exp_s[8:0], q_mant[22:10]};
    end
    reg [31:0] result_reg; reg result_ready;
    always @(posedge mclk or posedge rst) begin
        if(rst) begin result_reg<=0;result_ready<=0; end
        else begin result_ready<=add_ovld;
            if(add_ovld) result_reg<={9'b0,q_result};
        end
    end
