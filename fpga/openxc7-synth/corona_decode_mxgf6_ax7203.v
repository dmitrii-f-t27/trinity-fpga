...
`timescale 1ns / 1ps
// corona_decode_mxgf6_ax7203 — IEEE 754 mxgf6 (half) decode on AX7203.
// mxgf6_decode is authored in-repo and EXHAUSTIVELY verified (65536/65536 vs
// the C-library mxgf6 via struct 'e'). Single-decoder build: CFGMCLK clock,
...
module corona_decode_mxgf6_ax7203 (
    ...
    wire [31:0] result;
    gf_decode_param #(.N(6), .E(2), .M(3), .BIAS(1), .HAS_INF(0), .OUT_REG(0)) u_dec (.clk(1'b0), .rst_n(1'b1), .gf_in(code_r[5:0]), .fp32_out(result), .is_nan_o(), .is_inf_o(), .is_zero_o(), .is_subnormal_o());
    assign led[2] = |result;
