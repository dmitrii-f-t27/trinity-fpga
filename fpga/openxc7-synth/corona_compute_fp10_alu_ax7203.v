...
`timescale 1ns / 1ps
// corona_compute_fp10_alu_ax7203 — FP10 ALU on AX7203.
module corona_compute_fp10_alu_ax7203 (
    ...
    reg [3:0] frm; reg [7:0] fmt_r,op_r; reg [9:0] a_r,b_r; reg frame_valid;
    always @(posedge mclk or posedge rst) begin
        if(rst) begin frm<=0;fmt_r<=0;op_r<=0;a_r<=0;b_r<=0;frame_valid<=0; end
            ...
            if(rx_new) begin case(frm)
                ...
                4'd5: begin a_r[9:8]<=rx_byte;frm<=4'd6; end
                4'd6: begin b_r[7:0]<=rx_byte;frm<=4'd7; end
                4'd7: begin b_r[9:8]<=rx_byte;frm<=4'd8; end
    ...
    reg [9:0] a_reg,b_reg; reg [7:0] op_reg; reg comp_trigger;
    wire [9:0] fmt_a=a_reg, fmt_b=b_reg;
    ...
    always @(posedge mclk or posedge rst) begin
        if(rst) begin result_reg<=0;result_ready<=0; end
            ...
            if(ovld) result_reg<={22'b0,q_result};
        end
    end
