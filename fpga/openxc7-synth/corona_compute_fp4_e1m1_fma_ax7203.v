...
`timescale 1ns / 1ps
// corona_compute_fp4_e1m1_fma_ax7203 — FP4_E1M1 FMA on AX7203.
module corona_compute_fp4_e1m1_fma_ax7203 (
    ...
    wire signed [9:0] f_de_a = $signed({1'b0, f_exp_a}) - 10'sd0 + 10'sd127;
    ...
    wire signed [9:0] f_de_b = $signed({1'b0, f_exp_b}) - 10'sd0 + 10'sd127;
    ...
    wire signed [9:0] f_de_c = $signed({1'b0, f_exp_c}) - 10'sd0 + 10'sd127;
    ...
    wire signed [9:0] tgt_exp_s = $signed({1'b0, q_exp}) - 10'sd127 + 10'sd0;
    reg [3:0] q_result;
    always @(*) begin
        if(q_nan) q_result=4'd0;
        else if(q_zero) q_result=4'd0;
        else if(q_inf) q_result={q_sign, 1'd1, 1'd0, 1'b0};
        else if(q_exp >= 8'd129) q_result={q_sign, 1'd1, 1'd0, 1'b0};
        else if(q_exp < 8'd128) q_result={q_sign, 3'b0};
        else if(tgt_exp_s < 1) q_result={q_sign, 2'b0, q_mant[22:22]};
