...
`timescale 1ns / 1ps
// corona_decode_gf96_ax7203 — signed int128 → FP32 decode on AX7203.
// 16-byte code frame (128-bit input).
module corona_decode_gf96_ax7203 (
    ...
    reg [3:0] frm; reg [7:0] fmt_r; reg [95:0] code_r; reg frame_valid;
    always @(posedge mclk or posedge rst) begin
        if(rst) begin frm<=0;fmt_r<=0;code_r<=0;frame_valid<=0; end
            ...
            if(rx_new) begin case(frm)
                ...
                4'd10: begin code_r[63:56]<=rx_byte;frm<=11; end
                4'd11: begin code_r[71:64]<=rx_byte;frm<=12; end
                4'd12: begin code_r[79:72]<=rx_byte;frm<=13; end
                4'd13: begin code_r[87:80]<=rx_byte;frm<=14; end
                4'd14: begin code_r[95:88]<=rx_byte;frm<=15; end
    ...
    gf_decode_param #(.N(96), .E(36), .M(59), .BIAS(34359738367), .OUT_REG(0)) u_dec (.gf_in(code_r), .fp32_out(result), .is_nan_o(), .is_inf_o(), .is_zero_o(), .is_subnormal_o());
    assign led[2] = |result;
