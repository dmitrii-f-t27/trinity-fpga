...
`timescale 1ns / 1ps
// corona_compute_fp32_to_bitnet_ax7203 — BitNet FP32_TO on AX7203.
module corona_compute_fp32_to_bitnet_ax7203 (
    ...
    reg [1:0] code;
    always @(*) begin
        if(fp32_in == 32'h00000000) code = 2'd0;
        else if(fp32_in[31]) begin
            if(fp32_in >= 32'hBF000000) code = 2'd2;  // <= -0.5 → -1
            else code = 2'd0;                     // > -0.5 → 0
        end else begin
            if(fp32_in >= 32'h3F000000) code = 2'd1;   // >= 0.5 → +1
            else code = 2'd0;                      // < 0.5 → 0
        end
    end
    always @(posedge mclk or posedge rst) begin
    ...
    always @(posedge mclk or posedge rst) begin
        if(rst) begin result_reg<=0;result_ready<=0; end
            ...
            if(conv_trigger) result_reg<={30'b0,code};
        end
    end
