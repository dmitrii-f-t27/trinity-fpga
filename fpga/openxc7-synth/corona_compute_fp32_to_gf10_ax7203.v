...
`timescale 1ns / 1ps
// corona_compute_fp32_to_gf10_ax7203 — IEEE binary32 to GF10 converter.
// Input: FP32 [S:1][E:8][M:23] (bias=127)
// Output: GF10 [S:1][E:3][M:6] (bias=3, HAS_INF=0)
// Mantissa truncation. Overflow -> max, underflow -> zero.
module corona_compute_fp32_to_gf10_ax7203 (
    ...
    wire signed [9:0] gf_exp_calc = $signed({2'b0, fp_exp}) + 10'sd-124;
    wire overflow  = (gf_exp_calc > 10'sd7);
    wire underflow = (gf_exp_calc < 10'sd1);
    wire [5:0] gf_mant_trunc = fp_mant[22:17];
    reg [9:0] gf_result;
    always @(*) begin
        if (fp_nan)
            gf_result = {fp_sign, {3{1'b1}}, {6{1'b0}}};
        else if (fp_inf || overflow)
            gf_result = {fp_sign, gf_exp_calc[2:0] | {3{1'b1}}, {6{1'b0}}};
        else if (fp_zero || fp_denorm || underflow)
            gf_result = {fp_sign, {3{1'b0}}, {6{1'b0}}};
        else
            gf_result = {fp_sign, gf_exp_calc[2:0], gf_mant_trunc};
    end
    always @(posedge mclk or posedge rst) begin
        if(rst) begin a_reg<=0; conv_trigger<=0; end
        else begin conv_trigger <= frame_valid; if(frame_valid) a_reg <= a_r; end
    end
    reg [9:0] result_reg; reg result_ready;
    always @(posedge mclk or posedge rst) begin
