...
`timescale 1ns / 1ps
// corona_compute_gf64_add_ax7203 — GoldenFloat64 ADD on AX7203.
// GF64: [S:1][E:24][M:39] = 64 bits, BIAS=8388607, HAS_INF=0.
// 8 bytes per operand, 20-byte frame.
module corona_compute_gf64_add_ax7203 (
    ...
    gf_adder_param #(.EXP_BITS(24), .MANT_BITS(39), .HAS_INF(0), .PIPELINE(0)) u_comp (
        .clk(mclk), .rst(rst),
    ...
    // ---- UART TX (9 bytes: A5 r0..r7) — buffer+mux pattern (no shift-register race) ----
    reg responding; reg [3:0] tx_idx;
    reg [7:0] tx_buf0,tx_buf1,tx_buf2,tx_buf3,tx_buf4,tx_buf5,tx_buf6,tx_buf7,tx_buf8;
    ...
    always @(posedge mclk or posedge rst) begin
        if(rst) begin responding<=0;tx_idx<=0;tcnt<=BAUD_DIV-1;tbi<=0;tsr<=10'h3FF;uart_tx<=1;
            ...
            tx_buf5<=8'hFF;tx_buf6<=8'hFF;tx_buf7<=8'hFF;tx_buf8<=8'hFF; end
            ...
            if(result_ready) begin
                ...
                tx_buf5<=result_reg[39:32];
                tx_buf6<=result_reg[47:40];
                tx_buf7<=result_reg[55:48];
                tx_buf8<=result_reg[63:56];
