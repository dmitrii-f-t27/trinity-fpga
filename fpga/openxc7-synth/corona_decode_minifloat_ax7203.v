...
`timescale 1ns / 1ps
module corona_decode_minifloat_ax7203 (
    ...
    reg [2:0] frm; reg [7:0] fmt_r; reg [7:0] code_r; reg frame_valid;
    always @(posedge mclk or posedge rst) begin
        if(rst) begin frm<=0;fmt_r<=0;code_r<=0;frame_valid<=0; end
            ...
            if(rx_new) begin case(frm)
                ...
                3'd3: begin code_r<=rx_byte;frm<=4; end
    ...
    wire [31:0] result;
    wire nan_f, inf_f, zero_f;
    minifloat_decode u_dec (.mf_in(code_r), .fp32_out(result), .is_nan_o(nan_f), .is_inf_o(inf_f), .is_zero_o(zero_f));
    assign led[2] = |result;
