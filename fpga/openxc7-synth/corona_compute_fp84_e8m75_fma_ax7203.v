...
`timescale 1ns / 1ps
// corona_compute_fp84_e8m75_fma_ax7203 — FP84_E8M75 FMA on AX7203.
module corona_compute_fp84_e8m75_fma_ax7203 (
    ...
    reg [5:0] frm; reg [7:0] fmt_r; reg [83:0] a_r,b_r,c_r; reg frame_valid;
    always @(posedge mclk or posedge rst) begin
        if(rst) begin frm<=0;fmt_r<=0;a_r<=0;b_r<=0;c_r<=0;frame_valid<=0; end
            ...
            if(rx_new) begin case(frm)
                ...
                6'd13: begin a_r[83:80]<=rx_byte;frm<=6'd14; end
                ...
                6'd24: begin b_r[83:80]<=rx_byte;frm<=6'd25; end
                ...
                6'd35: begin c_r[83:80]<=rx_byte;frm<=6'd36; end
    ...
    reg [83:0] a_reg,b_reg,c_reg; reg comp_trigger;
    wire [83:0] fmt_a=a_reg, fmt_b=b_reg, fmt_c=c_reg;
    wire f_sign_a = fmt_a[83];
    wire [7:0] f_exp_a = fmt_a[82:75];
    wire [74:0] f_mant_a = fmt_a[74:0];
    ...
    wire [22:0] f_mant32_a = {f_mant_a, -52'b0};
    ...
    wire f_sign_b = fmt_b[83];
    wire [7:0] f_exp_b = fmt_b[82:75];
    wire [74:0] f_mant_b = fmt_b[74:0];
    ...
    wire [22:0] f_mant32_b = {f_mant_b, -52'b0};
    ...
    wire f_sign_c = fmt_c[83];
    wire [7:0] f_exp_c = fmt_c[82:75];
    wire [74:0] f_mant_c = fmt_c[74:0];
    ...
    wire [22:0] f_mant32_c = {f_mant_c, -52'b0};
    ...
    always @(*) begin
        ...
        else if(q_inf) q_result={q_sign, 8'd255, 75'd0};
        else if(q_exp >= 8'd254) q_result={q_sign, 8'd255, 75'd0};
        else if(q_exp < 8'd1) q_result={q_sign, 83'b0};
