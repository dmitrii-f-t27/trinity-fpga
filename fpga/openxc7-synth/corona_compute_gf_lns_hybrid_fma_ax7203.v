...
`timescale 1ns / 1ps
// corona_compute_gf_lns_hybrid_fma_ax7203 — GF_LNS_HYBRID FMA on AX7203.
module corona_compute_gf_lns_hybrid_fma_ax7203 (
    ...
    wire gl_sign_c = fmt_c[15];
    wire [5:0] gl_exp_c = fmt_c[14:9];
    wire [8:0] gl_mant_c = fmt_c[8:0];
    wire gl_zero_c = (gl_exp_c == 6'd0) && (gl_mant_c == 9'd0);
    wire gl_nan_c = (gl_exp_c == 6'd63) && (gl_mant_c != 0);
    wire gl_inf_c = (gl_exp_c == 6'd63) && (gl_mant_c == 0);
    wire gl_sub_c = (gl_exp_c == 6'd0) && (gl_mant_c != 0);
    wire [7:0] gl_exp32_c = gl_sub_c ? 8'd96 : ({2'b0, gl_exp_c} + 8'd96);
    wire [22:0] gl_mant32_c = {gl_mant_c, 14'b0};
    reg [31:0] fp32_c;
    always @(*) begin
        if(gl_zero_c) fp32_c={gl_sign_c, 31'b0};
        else if(gl_nan_c) fp32_c=32'h7FC00000;
        else if(gl_inf_c) fp32_c={gl_sign_c, 8'hFF, 23'b0};
        else fp32_c={gl_sign_c, gl_exp32_c, gl_mant32_c};
    end
