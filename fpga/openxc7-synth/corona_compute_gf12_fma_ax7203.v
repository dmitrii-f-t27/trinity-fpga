...
`timescale 1ns / 1ps
// corona_compute_gf12_fma_ax7203 — GoldenFloat12 FMA (y = a*b + c) on AX7203.
...
module corona_compute_gf12_fma_ax7203 (
    ...
    always @(posedge mclk or posedge rst) begin
        if(rst) begin frm<=0;fmt_r<=0;a_r<=0;b_r<=0;c_r<=0;frame_valid<=0; end
            ...
            if(rx_new) begin case(frm)
                ...
                4'd4: begin a_r[11:8]<=rx_byte[3:0];frm<=4'd5; end
                4'd5: begin b_r[7:0]<=rx_byte;frm<=4'd6; end
                4'd6: begin b_r[11:8]<=rx_byte[3:0];frm<=4'd7; end
                4'd7: begin c_r[7:0]<=rx_byte;frm<=4'd8; end
                4'd8: begin c_r[11:8]<=rx_byte[3:0];frm<=4'd9; end
