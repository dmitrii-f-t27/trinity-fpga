...
`timescale 1ns / 1ps
// corona_compute_gf14_alu_ax7203 — GoldenFloat14 ALU (ADD+MUL) on AX7203.
// Instantiates both gf_adder_param and gf_mul_param in parallel, selects via op byte.
// GF14: [S:1][E:5][M:8] = 14 bits, BIAS=15, HAS_INF=0.
//
// Frame protocol (9 bytes TX):
//   AA 55 fmt op a0 a1 b0 b1 trig
//   op: 0x00=ADD, 0x01=MUL
// Response (5 bytes RX):
//   A5 r0 r1 00 00

module corona_compute_gf14_alu_ax7203 (
    ...
    // ---- Frame FSM ----
    reg [3:0] frm; reg [7:0] fmt_r; reg [7:0] op_r; reg [13:0] a_r, b_r; reg frame_valid;
    always @(posedge mclk or posedge rst) begin
        if(rst) begin frm<=0;fmt_r<=0;op_r<=0;a_r<=0;b_r<=0;frame_valid<=0; end
            ...
            if(rx_new) begin case(frm)
                ...
                4'd5: begin a_r[13:8]<=rx_byte[5:0];frm<=4'd6; end
                4'd6: begin b_r[7:0]<=rx_byte;frm<=4'd7; end
                4'd7: begin b_r[13:8]<=rx_byte[5:0];frm<=4'd8; end
    ...
    // ---- GF14 ADD + MUL (both compute in parallel, mux selects) ----
    reg [13:0] a_reg, b_reg;
    reg comp_trigger;
    wire add_in_ready, add_out_valid;
    wire mul_in_ready, mul_out_valid;
    wire [13:0] add_result, mul_result;

    gf_adder_param #(.EXP_BITS(5), .MANT_BITS(8), .HAS_INF(0)) u_add (
        .clk(mclk), .rst(rst),
        .in_valid(comp_trigger), .in_a(a_reg), .in_b(b_reg), .in_ready(add_in_ready),
        .out_valid(add_out_valid), .out_y(add_result), .out_ready(1'b1)
    );

    gf_mul_param #(.EXP_BITS(5), .MANT_BITS(8), .HAS_INF(0)) u_mul (
        .clk(mclk), .rst(rst),
    ...
    // ---- Result capture (mux ADD vs MUL output) ----
    reg [13:0] result_reg;
    reg result_ready;
    wire comp_out_valid = is_mul ? mul_out_valid : add_out_valid;
    wire [13:0] comp_result = is_mul ? mul_result : add_result;
    ...
    always @(posedge mclk or posedge rst) begin
        if(rst) begin responding<=0;tx_idx<=0;tcnt<=BAUD_DIV-1;tbi<=0;tsr<=10'h3FF;uart_tx<=1;
            ...
            if(result_ready) begin
                tx_buf0<=8'hA5; tx_buf1<=result_reg[7:0]; tx_buf2<=result_reg[13:8]; tx_buf3<=8'h00; tx_buf4<=8'h00;
