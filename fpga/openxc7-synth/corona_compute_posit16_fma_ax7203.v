...
`timescale 1ns / 1ps
// corona_compute_posit16_fma_ax7203 — POSIT16 FMA on AX7203.
module corona_compute_posit16_fma_ax7203 (
    ...
    wire [15:0] pos_val_c = fmt_c;
    wire pos_zero_c = (pos_val_c==16'h0000);
    wire pos_nar_c = (pos_val_c==16'h8000);
    wire pos_sign_c = pos_val_c[15];
    wire [14:0] pos_abs_c = pos_sign_c ? (~pos_val_c[14:0]+15'd1) : pos_val_c[14:0];
    wire pos_rs_c = pos_abs_c[14];
    wire [14:0] pos_rb_c = pos_rs_c ? ~pos_abs_c : pos_abs_c;
    reg [3:0] pos_lzc_c;
    always @(*) begin
        casez(pos_rb_c)
            15'b1??????????????: pos_lzc_c=4'd0;
            15'b01?????????????: pos_lzc_c=4'd1;
            15'b001????????????: pos_lzc_c=4'd2;
            15'b0001???????????: pos_lzc_c=4'd3;
            15'b00001??????????: pos_lzc_c=4'd4;
            15'b000001?????????: pos_lzc_c=4'd5;
            15'b0000001????????: pos_lzc_c=4'd6;
            15'b00000001???????: pos_lzc_c=4'd7;
            15'b000000001??????: pos_lzc_c=4'd8;
            15'b0000000001?????: pos_lzc_c=4'd9;
            15'b00000000001????: pos_lzc_c=4'd10;
            15'b000000000001???: pos_lzc_c=4'd11;
            15'b0000000000001??: pos_lzc_c=4'd12;
            15'b00000000000001?: pos_lzc_c=4'd13;
            default: pos_lzc_c=4'd14;
        endcase
    end
    wire signed [4:0] pos_k_c = pos_rs_c ? $signed({1'b0,pos_lzc_c})-5'sd1 : -$signed({1'b0,pos_lzc_c});
    wire [3:0] pos_rt_c = (pos_lzc_c<4'd14) ? pos_lzc_c+4'd1 : pos_lzc_c;
    wire [14:0] pos_shifted_c = pos_abs_c << pos_rt_c;
    wire [1:0] pos_es_c = pos_shifted_c[14:13];
    wire [12:0] pos_frac_c = pos_shifted_c[12:0];
    wire signed [6:0] pos_exp2_c = {{2{pos_k_c[4]}},pos_k_c} + {5'd0,pos_es_c};
    wire [8:0] pos_fp32_exp_c = {1'b0,8'd127} + {{1{pos_exp2_c[6]}},pos_exp2_c};
    reg [31:0] fp32_c;
    always @(*) begin
        if(pos_zero_c) fp32_c=32'h00000000;
        else if(pos_nar_c) fp32_c=32'h7FC00000;
        else fp32_c={pos_sign_c,pos_fp32_exp_c[7:0],pos_frac_c,10'b0};
    end
