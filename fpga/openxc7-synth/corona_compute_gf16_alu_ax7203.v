...
`timescale 1ns / 1ps
// corona_compute_gf16_alu_ax7203 — GoldenFloat16 ALU (ADD+MUL) on AX7203.
// Instantiates both gf_adder_param and gf_mul_param in parallel, selects via op byte.
// GF16 format: [S:1][E:6][M:9] = 16 bits, BIAS=31, HAS_INF=1.
//
...
module corona_compute_gf16_alu_ax7203 (
    ...
    reg [3:0] frm; reg [7:0] fmt_r; reg [7:0] op_r; reg [15:0] a_r, b_r; reg frame_valid;
    always @(posedge mclk or posedge rst) begin
    ...
    // ---- GF16 ADD + MUL (both compute in parallel, mux selects) ----
    ...
    gf_mul_param #(.EXP_BITS(6), .MANT_BITS(9), .HAS_INF(1)) u_mul (
        .clk(mclk), .rst(rst),
    ...
    wire [15:0] comp_result = is_mul ? mul_result : add_result;
