...
`timescale 1ns / 1ps
// corona_compute_binary64_to_fp32_ax7203 — BINARY64 TO_FP32 on AX7203.
module corona_compute_binary64_to_fp32_ax7203 (
    ...
    wire sign_v=a_reg[63]; wire [10:0] exp_v=a_reg[62:52]; wire [51:0] mant_v=a_reg[51:0];
    wire is_zero=(exp_v==0)&&(mant_v==0);
    wire is_inf=(exp_v==11'b11111111111)&&(mant_v==0);
    wire is_nan=(exp_v==11'b11111111111)&&(mant_v!=0);
    reg [31:0] fp32_r;
    always @(*) begin
        if(is_nan) fp32_r=32'h7FC00000;
        else if(is_inf) fp32_r={sign_v,8'hFF,23'h0};
        else if(is_zero) fp32_r={sign_v,8'h00,23'h0};
        else fp32_r={sign_v,exp_v[7:0],mant_v[51:29]};
    end
    always @(posedge mclk or posedge rst) begin
    ...
    always @(posedge mclk or posedge rst) begin
        if(rst) begin result_reg<=0;result_ready<=0; end
            ...
            if(conv_trigger) result_reg<=fp32_r;
        end
    end
