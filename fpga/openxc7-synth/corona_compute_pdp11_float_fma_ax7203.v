...
`timescale 1ns / 1ps
// corona_compute_pdp11_float_fma_ax7203 — PDP11_FLOAT FMA on AX7203.
module corona_compute_pdp11_float_fma_ax7203 (
    ...
    wire [31:0] pdp_val_a = fmt_a;
    wire pdp_sign_a = pdp_val_a[31];
    wire [7:0] pdp_exp_a = pdp_val_a[30:23];
    wire [22:0] pdp_mant_a = pdp_val_a[22:0];
    wire pdp_zero_a = (pdp_val_a==32'h0);
    wire pdp_nan_a = (pdp_exp_a==8'hFF)&&(pdp_mant_a!=0);
    ...
    always @(*) begin
        ...
        else fp32_a={pdp_sign_a,pdp_exp_a-8'd1,pdp_mant_a};
    end
    wire [31:0] pdp_val_b = fmt_b;
    wire pdp_sign_b = pdp_val_b[31];
    wire [7:0] pdp_exp_b = pdp_val_b[30:23];
    wire [22:0] pdp_mant_b = pdp_val_b[22:0];
    wire pdp_zero_b = (pdp_val_b==32'h0);
    wire pdp_nan_b = (pdp_exp_b==8'hFF)&&(pdp_mant_b!=0);
    ...
    always @(*) begin
        ...
        else fp32_b={pdp_sign_b,pdp_exp_b-8'd1,pdp_mant_b};
    end
    wire [31:0] pdp_val_c = fmt_c;
    wire pdp_sign_c = pdp_val_c[31];
    wire [7:0] pdp_exp_c = pdp_val_c[30:23];
    wire [22:0] pdp_mant_c = pdp_val_c[22:0];
    wire pdp_zero_c = (pdp_val_c==32'h0);
    wire pdp_nan_c = (pdp_exp_c==8'hFF)&&(pdp_mant_c!=0);
    wire pdp_inf_c = (pdp_exp_c==8'hFF)&&(pdp_mant_c==0);
    reg [31:0] fp32_c;
    always @(*) begin
        if(pdp_zero_c) fp32_c={pdp_sign_c, 31'b0};
        else if(pdp_nan_c) fp32_c=32'h7FC00000;
        else if(pdp_inf_c) fp32_c={pdp_sign_c,8'hFF,23'b0};
        else if(pdp_exp_c<=8'd1) fp32_c={pdp_sign_c,31'b0};
        else fp32_c={pdp_sign_c,pdp_exp_c-8'd1,pdp_mant_c};
    end
    ...
    always @(*) begin
        ...
        else if(q_inf) q_result={q_sign,8'hFF,23'b0};
        else if(q_exp==0) q_result={q_sign,8'd2,23'b0};
        else if(q_exp>=8'd254) q_result={q_sign,8'hFE,q_mant};
        else q_result={q_sign,q_exp+8'd1,q_mant};
    end
