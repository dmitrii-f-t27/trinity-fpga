...
`timescale 1ns / 1ps
// corona_compute_fp36_e8m27_fma_ax7203 — FP36_E8M27 FMA on AX7203.
module corona_compute_fp36_e8m27_fma_ax7203 (
    ...
    reg [4:0] frm; reg [7:0] fmt_r; reg [35:0] a_r,b_r,c_r; reg frame_valid;
    always @(posedge mclk or posedge rst) begin
        if(rst) begin frm<=0;fmt_r<=0;a_r<=0;b_r<=0;c_r<=0;frame_valid<=0; end
            ...
            if(rx_new) begin case(frm)
                ...
                5'd7: begin a_r[35:32]<=rx_byte;frm<=5'd8; end
                ...
                5'd12: begin b_r[35:32]<=rx_byte;frm<=5'd13; end
                ...
                5'd17: begin c_r[35:32]<=rx_byte;frm<=5'd18; end
    ...
    reg [35:0] a_reg,b_reg,c_reg; reg comp_trigger;
    wire [35:0] fmt_a=a_reg, fmt_b=b_reg, fmt_c=c_reg;
    wire f_sign_a = fmt_a[35];
    wire [7:0] f_exp_a = fmt_a[34:27];
    ...
    wire [22:0] f_mant32_a = {f_mant_a, -4'b0};
    ...
    wire f_sign_b = fmt_b[35];
    wire [7:0] f_exp_b = fmt_b[34:27];
    ...
    wire [22:0] f_mant32_b = {f_mant_b, -4'b0};
    ...
    wire f_sign_c = fmt_c[35];
    wire [7:0] f_exp_c = fmt_c[34:27];
    wire [26:0] f_mant_c = fmt_c[26:0];
    ...
    wire [22:0] f_mant32_c = {f_mant_c, -4'b0};
    ...
    reg [35:0] q_result;
    always @(*) begin
        if(q_nan) q_result=36'd0;
        else if(q_zero) q_result=36'd0;
        else if(q_inf) q_result={q_sign, 8'd255, 27'd0};
        else if(q_exp >= 8'd254) q_result={q_sign, 8'd255, 27'd0};
        else if(q_exp < 8'd1) q_result={q_sign, 35'b0};
        else if(tgt_exp_s < 1) q_result={q_sign, 8'b0, q_mant[22:0]};
        else q_result={q_sign, tgt_exp_s[7:0], q_mant[22:0]};
    end
    reg [35:0] result_reg; reg result_ready;
    always @(posedge mclk or posedge rst) begin
