...
`timescale 1ns / 1ps
// corona_compute_fp32_to_minifloat_ax7203 — MINIFLOAT FP32_TO on AX7203.
module corona_compute_fp32_to_minifloat_ax7203 (
    ...
    reg [1:0] frm; reg [7:0] fmt_r; reg [31:0] a_r; reg frame_valid;
    always @(posedge mclk or posedge rst) begin
        if(rst) begin frm<=0;fmt_r<=0;a_r<=0;frame_valid<=0; end
            ...
            if(rx_new) begin case(frm)
                ...
                2'd2: begin fmt_r<=rx_byte;frm<=2'd3; end
                2'd3: begin frame_valid<=1;frm<=0; end
            endcase end
        end
    end
    ...
    wire sign=a_reg[31]; wire [7:0] exp_in=a_reg[30:23]; wire [22:0] mant=a_reg[22:0];
    wire is_zero=(exp_in==0)&&(mant==0); wire is_nan=(exp_in==8'hFF)&&(mant!=0);
    wire [8:0] mf_e={1'b0,exp_in}-9'd124;
    reg [7:0] mf_result;
    always @(*) begin
        if(is_zero) mf_result=8'h00;
        else if(is_nan) mf_result=8'h7F;
        else if(exp_in>=8'd131) mf_result={sign,3'd6,4'd8};
        else if(mf_e[8]) mf_result=8'h00;
        else mf_result={sign,mf_e[2:0],mant[22:19]};
    end
    always @(posedge mclk or posedge rst) begin
    ...
    always @(posedge mclk or posedge rst) begin
        if(rst) begin result_reg<=0;result_ready<=0; end
            ...
            if(conv_trigger) result_reg<={24'b0,mf_result};
        end
    end
