...
`timescale 1ns / 1ps
// corona_compute_fp32_to_gf12_ax7203 — IEEE binary32 to GF12 converter.
// Input: FP32 [S:1][E:8][M:23] (bias=127)
// Output: GF12 [S:1][E:4][M:7] (bias=7, HAS_INF=0)
// Mantissa truncation. Overflow -> max, underflow -> zero.
module corona_compute_fp32_to_gf12_ax7203 (
    ...
    always @(*) begin
        if (fp_nan)
            gf_result = {fp_sign, {4{1'b1}}, {7{1'b0}}};
        else if (fp_inf || overflow)
            gf_result = {fp_sign, gf_exp_calc[3:0] | {4{1'b1}}, {7{1'b0}}};
        else if (fp_zero || fp_denorm || underflow)
            gf_result = {fp_sign, {4{1'b0}}, {7{1'b0}}};
        else
            gf_result = {fp_sign, gf_exp_calc[3:0], gf_mant_trunc};
    end
    always @(posedge mclk or posedge rst) begin
        if(rst) begin a_reg<=0; conv_trigger<=0; end
        else begin conv_trigger <= frame_valid; if(frame_valid) a_reg <= a_r; end
    end
    reg [11:0] result_reg; reg result_ready;
    always @(posedge mclk or posedge rst) begin
