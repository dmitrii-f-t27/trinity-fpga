...
`timescale 1ns / 1ps
// corona_compute_fp102_e8m93_fma_ax7203 — FP102_E8M93 FMA on AX7203.
module corona_compute_fp102_e8m93_fma_ax7203 (
    ...
    reg [5:0] frm; reg [7:0] fmt_r; reg [101:0] a_r,b_r,c_r; reg frame_valid;
    always @(posedge mclk or posedge rst) begin
        if(rst) begin frm<=0;fmt_r<=0;a_r<=0;b_r<=0;c_r<=0;frame_valid<=0; end
            ...
            if(rx_new) begin case(frm)
                ...
                6'd15: begin a_r[101:96]<=rx_byte;frm<=6'd16; end
                6'd16: begin b_r[7:0]<=rx_byte;frm<=6'd17; end
                6'd17: begin b_r[15:8]<=rx_byte;frm<=6'd18; end
                6'd18: begin b_r[23:16]<=rx_byte;frm<=6'd19; end
                6'd19: begin b_r[31:24]<=rx_byte;frm<=6'd20; end
                6'd20: begin b_r[39:32]<=rx_byte;frm<=6'd21; end
                6'd21: begin b_r[47:40]<=rx_byte;frm<=6'd22; end
                6'd22: begin b_r[55:48]<=rx_byte;frm<=6'd23; end
                6'd23: begin b_r[63:56]<=rx_byte;frm<=6'd24; end
                6'd24: begin b_r[71:64]<=rx_byte;frm<=6'd25; end
                6'd25: begin b_r[79:72]<=rx_byte;frm<=6'd26; end
                6'd26: begin b_r[87:80]<=rx_byte;frm<=6'd27; end
                6'd27: begin b_r[95:88]<=rx_byte;frm<=6'd28; end
                6'd28: begin b_r[101:96]<=rx_byte;frm<=6'd29; end
                6'd29: begin c_r[7:0]<=rx_byte;frm<=6'd30; end
                6'd30: begin c_r[15:8]<=rx_byte;frm<=6'd31; end
                6'd31: begin c_r[23:16]<=rx_byte;frm<=6'd32; end
                6'd32: begin c_r[31:24]<=rx_byte;frm<=6'd33; end
                6'd33: begin c_r[39:32]<=rx_byte;frm<=6'd34; end
                6'd34: begin c_r[47:40]<=rx_byte;frm<=6'd35; end
                6'd35: begin c_r[55:48]<=rx_byte;frm<=6'd36; end
                6'd36: begin c_r[63:56]<=rx_byte;frm<=6'd37; end
                6'd37: begin c_r[71:64]<=rx_byte;frm<=6'd38; end
                6'd38: begin c_r[79:72]<=rx_byte;frm<=6'd39; end
                6'd39: begin c_r[87:80]<=rx_byte;frm<=6'd40; end
                6'd40: begin c_r[95:88]<=rx_byte;frm<=6'd41; end
                6'd41: begin c_r[101:96]<=rx_byte;frm<=6'd42; end
                6'd42: begin frame_valid<=1;frm<=0; end
            endcase end
        end
    end
    assign led[1]=frame_valid;

    reg [101:0] a_reg,b_reg,c_reg; reg comp_trigger;
    wire [101:0] fmt_a=a_reg, fmt_b=b_reg, fmt_c=c_reg;
    wire f_sign_a = fmt_a[101];
    wire [7:0] f_exp_a = fmt_a[100:93];
    ...
    wire f_sign_b = fmt_b[101];
    wire [7:0] f_exp_b = fmt_b[100:93];
    ...
    wire f_sign_c = fmt_c[101];
    wire [7:0] f_exp_c = fmt_c[100:93];
    ...
    reg [101:0] q_result;
    always @(*) begin
        if(q_nan) q_result=102'd0;
        else if(q_zero) q_result=102'd0;
        else if(q_inf) q_result={q_sign, 8'd255, 93'd0};
        else if(q_exp >= 8'd254) q_result={q_sign, 8'd255, 93'd0};
        else if(q_exp < 8'd1) q_result={q_sign, 101'b0};
        else if(tgt_exp_s < 1) q_result={q_sign, 8'b0, q_mant[22:0]};
        else q_result={q_sign, tgt_exp_s[7:0], q_mant[22:0]};
    end
    reg [101:0] result_reg; reg result_ready;
    always @(posedge mclk or posedge rst) begin
    ...
    localparam [3:0] TX_LEN = 14;
    // TX: buffer+mux (no conflicting NBA — fixes tx race). 14 bytes sliced from tx_load[111:0].
    wire [111:0] tx_load = {result_reg, 8'hA5};
    reg responding; reg [3:0] tx_idx; reg [7:0] tx_buf0, tx_buf1, tx_buf2, tx_buf3, tx_buf4, tx_buf5, tx_buf6, tx_buf7, tx_buf8, tx_buf9, tx_buf10, tx_buf11, tx_buf12, tx_buf13;
    reg [8:0] tcnt; reg [3:0] tbi; reg [9:0] tsr;
    always @(posedge mclk or posedge rst) begin
        if(rst) begin responding<=0;tx_idx<=0;tcnt<=BAUD_DIV-1;tbi<=0;tsr<=10'h3FF;uart_tx<=1;
            tx_buf0<=8'hFF; tx_buf1<=8'hFF; tx_buf2<=8'hFF; tx_buf3<=8'hFF; tx_buf4<=8'hFF; tx_buf5<=8'hFF; tx_buf6<=8'hFF; tx_buf7<=8'hFF; tx_buf8<=8'hFF; tx_buf9<=8'hFF; tx_buf10<=8'hFF; tx_buf11<=8'hFF; tx_buf12<=8'hFF; tx_buf13<=8'hFF; end
        else begin uart_tx<=tsr[0];
            if(result_ready) begin
                tx_buf0<=tx_load[7:0]; tx_buf1<=tx_load[15:8]; tx_buf2<=tx_load[23:16]; tx_buf3<=tx_load[31:24]; tx_buf4<=tx_load[39:32]; tx_buf5<=tx_load[47:40]; tx_buf6<=tx_load[55:48]; tx_buf7<=tx_load[63:56]; tx_buf8<=tx_load[71:64]; tx_buf9<=tx_load[79:72]; tx_buf10<=tx_load[87:80]; tx_buf11<=tx_load[95:88]; tx_buf12<=tx_load[103:96]; tx_buf13<=tx_load[111:104]; responding<=1; tx_idx<=0;
            end
            if(tcnt==0) begin tcnt<=BAUD_DIV-1;
                if(tbi==9) begin tbi<=0;
                    if(responding) begin
                        ...
                        endcase
                        if(tx_idx==13) responding<=0; else tx_idx<=tx_idx+1;
