...
`timescale 1ns / 1ps
// corona_compute_fp5_e3m1_alu_ax7203 — FP5_E3M1 ALU on AX7203.
module corona_compute_fp5_e3m1_alu_ax7203 (
    ...
    reg [2:0] frm; reg [7:0] fmt_r,op_r; reg [4:0] a_r,b_r; reg frame_valid;
    always @(posedge mclk or posedge rst) begin
        if(rst) begin frm<=0;fmt_r<=0;op_r<=0;a_r<=0;b_r<=0;frame_valid<=0; end
            ...
            if(rx_new) begin case(frm)
                ...
                3'd4: begin a_r[4:0]<=rx_byte;frm<=3'd5; end
                3'd5: begin b_r[4:0]<=rx_byte;frm<=3'd6; end
    ...
    reg [4:0] a_reg,b_reg; reg [7:0] op_reg; reg comp_trigger;
    wire [4:0] fmt_a=a_reg, fmt_b=b_reg;
    ...
    always @(posedge mclk or posedge rst) begin
        if(rst) begin result_reg<=0;result_ready<=0; end
            ...
            if(ovld) result_reg<={27'b0,q_result};
        end
    end
