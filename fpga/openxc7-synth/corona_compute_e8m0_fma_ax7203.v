...
`timescale 1ns / 1ps
// corona_compute_e8m0_fma_ax7203 — E8M0 FMA on AX7203.
module corona_compute_e8m0_fma_ax7203 (
    ...
    wire [7:0] e_val_a = fmt_a;
    wire e_nan_a = (e_val_a==8'hFF);
    reg [31:0] fp32_a;
    always @(*) begin
        if(e_val_a==8'h00) fp32_a=32'h00400000;
        else if(e_nan_a) fp32_a=32'h7FC00000;
        else fp32_a={1'b0,e_val_a,23'b0};
    end
    wire [7:0] e_val_b = fmt_b;
    wire e_nan_b = (e_val_b==8'hFF);
    reg [31:0] fp32_b;
    always @(*) begin
        if(e_val_b==8'h00) fp32_b=32'h00400000;
        else if(e_nan_b) fp32_b=32'h7FC00000;
        else fp32_b={1'b0,e_val_b,23'b0};
    end
    wire [7:0] e_val_c = fmt_c;
    wire e_nan_c = (e_val_c==8'hFF);
    reg [31:0] fp32_c;
    always @(*) begin
        if(e_val_c==8'h00) fp32_c=32'h00400000;
        else if(e_nan_c) fp32_c=32'h7FC00000;
        else fp32_c={1'b0,e_val_c,23'b0};
    end
    ...
    wire q_nan=(q_in==32'h7FC00000);
    wire q_zero=(q_in==32'h00000000);
    wire [7:0] q_exp=q_in[30:23];
    reg [7:0] q_result;
    always @(*) begin
        if(q_nan) q_result=8'hFF;
        else if(q_zero||q_in[31]||q_exp<8'd1) q_result=8'h00;
        else if(q_exp>=8'd254) q_result=8'hFE;
        else if(|q_in[22:0]) q_result=q_exp+8'h01;
        else q_result=q_exp;
    end
