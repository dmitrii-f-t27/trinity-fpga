...
`timescale 1ns / 1ps
// corona_decode_posit16_ax7203 — Posit16 (n=16, es=2) decode on AX7203.
// Single-decoder build: CFGMCLK (~70 MHz, STARTUPE2) clock, BAUD_DIV 434 (~161290).
// Frame: AA 55 fmt code_lo code_hi trig -> A5 r0 r1 r2 r3 (32-bit LE FP32).
// Identical infrastructure to corona_decode_int16_ax7203; only the decoder differs
// (posit16_decode: Posit16 -> FP32, exact decode, uses code_r[15:0]).
module corona_decode_posit16_ax7203 (
    ...
    wire [31:0] result;
    wire        is_zero, is_nar;
    posit16_decode u_dec (.posit_in(code_r[15:0]), .fp32_out(result), .is_zero(is_zero), .is_nar(is_nar));
    assign led[2] = |result;
