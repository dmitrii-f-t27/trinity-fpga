...
`timescale 1ns / 1ps
// corona_compute_ibm_hfp128_fma_ax7203 — IBM_HFP128 FMA on AX7203.
module corona_compute_ibm_hfp128_fma_ax7203 (
    ...
    wire ih_sign_a = fmt_a[127];
    wire [6:0] ih_exp_a = fmt_a[126:120];
    wire [119:0] ih_frac_a = fmt_a[119:0];
    wire ih_zero_a = (ih_exp_a == 7'd0) && (ih_frac_a == 120'd0);
    reg [2:0] ih_lhd_a;
    always @(*) begin
        casez(ih_frac_a[119:116])
            4'b1???: ih_lhd_a=3'd0;
            4'b01??: ih_lhd_a=3'd1;
            4'b001?: ih_lhd_a=3'd2;
            4'b0001: ih_lhd_a=3'd3;
            default: ih_lhd_a=3'd4;
        endcase
    end
    wire [119:0] ih_shifted_a = ih_frac_a << ({ih_lhd_a, 2'b00});
    wire signed [10:0] ih_exp_s_a = $signed({1'b0, ih_exp_a, 2'b00}) - 11'sd185
        + $signed({8'b0, ih_lhd_a, 2'b00});
    wire [7:0] ih_exp32_a = ih_exp_s_a[7:0];
    ...
    wire [6:0] ih_exp_b = fmt_b[126:120];
    wire [119:0] ih_frac_b = fmt_b[119:0];
    wire ih_zero_b = (ih_exp_b == 7'd0) && (ih_frac_b == 120'd0);
    reg [2:0] ih_lhd_b;
    always @(*) begin
        casez(ih_frac_b[119:116])
            4'b1???: ih_lhd_b=3'd0;
            4'b01??: ih_lhd_b=3'd1;
            4'b001?: ih_lhd_b=3'd2;
            4'b0001: ih_lhd_b=3'd3;
            default: ih_lhd_b=3'd4;
        endcase
    end
    wire [119:0] ih_shifted_b = ih_frac_b << ({ih_lhd_b, 2'b00});
    wire signed [10:0] ih_exp_s_b = $signed({1'b0, ih_exp_b, 2'b00}) - 11'sd185
        + $signed({8'b0, ih_lhd_b, 2'b00});
    wire [7:0] ih_exp32_b = ih_exp_s_b[7:0];
    ...
    wire [6:0] ih_exp_c = fmt_c[126:120];
    wire [119:0] ih_frac_c = fmt_c[119:0];
    wire ih_zero_c = (ih_exp_c == 7'd0) && (ih_frac_c == 120'd0);
    reg [2:0] ih_lhd_c;
    always @(*) begin
        casez(ih_frac_c[119:116])
            4'b1???: ih_lhd_c=3'd0;
            4'b01??: ih_lhd_c=3'd1;
            4'b001?: ih_lhd_c=3'd2;
            4'b0001: ih_lhd_c=3'd3;
            default: ih_lhd_c=3'd4;
        endcase
    end
    wire [119:0] ih_shifted_c = ih_frac_c << ({ih_lhd_c, 2'b00});
    wire signed [10:0] ih_exp_s_c = $signed({1'b0, ih_exp_c, 2'b00}) - 11'sd185
        + $signed({8'b0, ih_lhd_c, 2'b00});
    wire [7:0] ih_exp32_c = ih_exp_s_c[7:0];
    ...
    wire signed [9:0] ih_raw = $signed({1'b0, q_exp}) - 10'sd126;
    wire [7:0] ih_exp = ih_raw[9] ? 8'd0 : {ih_raw[8:1] + 8'd64};
    reg [127:0] q_result;
    always @(*) begin
        if(q_nan) q_result=128'h0;
        else if(q_zero) q_result=128'h0;
        else q_result={q_sign, ih_exp[6:0], 4'h8, q_mant, 93'b0};
    end
