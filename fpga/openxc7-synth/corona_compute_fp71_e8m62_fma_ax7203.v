...
`timescale 1ns / 1ps
// corona_compute_fp71_e8m62_fma_ax7203 — FP71_E8M62 FMA on AX7203.
module corona_compute_fp71_e8m62_fma_ax7203 (
    ...
    reg [4:0] frm; reg [7:0] fmt_r; reg [70:0] a_r,b_r,c_r; reg frame_valid;
    always @(posedge mclk or posedge rst) begin
        if(rst) begin frm<=0;fmt_r<=0;a_r<=0;b_r<=0;c_r<=0;frame_valid<=0; end
            ...
            if(rx_new) begin case(frm)
                ...
                5'd29: begin c_r[70:64]<=rx_byte;frm<=5'd30; end
    ...
    reg [70:0] a_reg,b_reg,c_reg; reg comp_trigger;
    wire [70:0] fmt_a=a_reg, fmt_b=b_reg, fmt_c=c_reg;
    wire f_sign_a = fmt_a[70];
    wire [7:0] f_exp_a = fmt_a[69:62];
    wire [61:0] f_mant_a = fmt_a[61:0];
    ...
    wire [22:0] f_mant32_a = {f_mant_a, -39'b0};
    ...
    wire f_sign_b = fmt_b[70];
    wire [7:0] f_exp_b = fmt_b[69:62];
    wire [61:0] f_mant_b = fmt_b[61:0];
    ...
    wire [22:0] f_mant32_b = {f_mant_b, -39'b0};
    ...
    wire f_sign_c = fmt_c[70];
    wire [7:0] f_exp_c = fmt_c[69:62];
    wire [61:0] f_mant_c = fmt_c[61:0];
    ...
    wire [22:0] f_mant32_c = {f_mant_c, -39'b0};
    ...
    always @(*) begin
        ...
        else if(q_inf) q_result={q_sign, 8'd255, 62'd0};
        else if(q_exp >= 8'd254) q_result={q_sign, 8'd255, 62'd0};
        else if(q_exp < 8'd1) q_result={q_sign, 70'b0};
