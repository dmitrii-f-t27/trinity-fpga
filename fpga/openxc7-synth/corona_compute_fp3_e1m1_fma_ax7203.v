...
`timescale 1ns / 1ps
// corona_compute_fp3_e1m1_fma_ax7203 — FP3_E1M1 FMA on AX7203.
module corona_compute_fp3_e1m1_fma_ax7203 (
    ...
    reg [2:0] frm; reg [7:0] fmt_r; reg [2:0] a_r,b_r,c_r; reg frame_valid;
    always @(posedge mclk or posedge rst) begin
        if(rst) begin frm<=0;fmt_r<=0;a_r<=0;b_r<=0;c_r<=0;frame_valid<=0; end
            ...
            if(rx_new) begin case(frm)
                ...
                3'd3: begin a_r[2:0]<=rx_byte;frm<=3'd4; end
                3'd4: begin b_r[2:0]<=rx_byte;frm<=3'd5; end
                3'd5: begin c_r[2:0]<=rx_byte;frm<=3'd6; end
    ...
    reg [2:0] a_reg,b_reg,c_reg; reg comp_trigger;
    wire [2:0] fmt_a=a_reg, fmt_b=b_reg, fmt_c=c_reg;
    wire f_sign_a = fmt_a[2];
    wire [0:0] f_exp_a = fmt_a[1:1];
    wire [0:0] f_mant_a = fmt_a[0:0];
    wire f_zero_a = (f_exp_a == 0) && ((f_mant_a == 0));
    wire f_inf_a = (f_exp_a == 1) && ((f_mant_a == 0));
    wire f_nan_a = (f_exp_a == 1) && ((f_mant_a != 0));
    wire f_sub_a = (f_exp_a == 0) && ((f_mant_a != 0));
    wire signed [10:0] f_de_a = $signed({1'b0, f_exp_a}) - 11'sd0 + 11'sd127;
    wire [7:0] f_exp32_a = (f_de_a > 11'sd254) ? 8'd254 : (f_de_a < 0) ? 8'd0 : f_de_a[7:0];
    wire [22:0] f_mant32_a = {f_mant_a, 22'b0};
    wire [22:0] f_mant32_norm_a = {1'b0, f_mant_a, 21'b0};
    reg [31:0] fp32_a;
    always @(*) begin
        ...
        else if(f_sub_a) fp32_a={f_sign_a, 8'd128, f_mant32_norm_a};
        else fp32_a={f_sign_a, f_exp32_a, f_mant32_a};
    end
    wire f_sign_b = fmt_b[2];
    wire [0:0] f_exp_b = fmt_b[1:1];
    wire [0:0] f_mant_b = fmt_b[0:0];
    wire f_zero_b = (f_exp_b == 0) && ((f_mant_b == 0));
    wire f_inf_b = (f_exp_b == 1) && ((f_mant_b == 0));
    wire f_nan_b = (f_exp_b == 1) && ((f_mant_b != 0));
    wire f_sub_b = (f_exp_b == 0) && ((f_mant_b != 0));
    wire signed [10:0] f_de_b = $signed({1'b0, f_exp_b}) - 11'sd0 + 11'sd127;
    wire [7:0] f_exp32_b = (f_de_b > 11'sd254) ? 8'd254 : (f_de_b < 0) ? 8'd0 : f_de_b[7:0];
    wire [22:0] f_mant32_b = {f_mant_b, 22'b0};
    wire [22:0] f_mant32_norm_b = {1'b0, f_mant_b, 21'b0};
    reg [31:0] fp32_b;
    always @(*) begin
        ...
        else if(f_sub_b) fp32_b={f_sign_b, 8'd128, f_mant32_norm_b};
        else fp32_b={f_sign_b, f_exp32_b, f_mant32_b};
    end
    wire f_sign_c = fmt_c[2];
    wire [0:0] f_exp_c = fmt_c[1:1];
    wire [0:0] f_mant_c = fmt_c[0:0];
    wire f_zero_c = (f_exp_c == 0) && ((f_mant_c == 0));
    wire f_inf_c = (f_exp_c == 1) && ((f_mant_c == 0));
    wire f_nan_c = (f_exp_c == 1) && ((f_mant_c != 0));
    wire f_sub_c = (f_exp_c == 0) && ((f_mant_c != 0));
    wire signed [10:0] f_de_c = $signed({1'b0, f_exp_c}) - 11'sd0 + 11'sd127;
    wire [7:0] f_exp32_c = (f_de_c > 11'sd254) ? 8'd254 : (f_de_c < 0) ? 8'd0 : f_de_c[7:0];
    wire [22:0] f_mant32_c = {f_mant_c, 22'b0};
    wire [22:0] f_mant32_norm_c = {1'b0, f_mant_c, 21'b0};
    reg [31:0] fp32_c;
    always @(*) begin
        ...
        else if(f_sub_c) fp32_c={f_sign_c, 8'd128, f_mant32_norm_c};
    ...
    wire signed [10:0] tgt_exp_s = $signed({1'b0, q_exp}) - 11'sd127 + 11'sd0;
    reg [2:0] q_result;
    always @(*) begin
        if(q_nan) q_result=3'd0;
        else if(q_zero) q_result=3'd0;
        else if(q_inf) q_result={q_sign, 1'd1, 1'd0};
        else if(q_exp >= 8'd129) q_result={q_sign, 1'd1, 1'd0};
        else if(q_exp < 8'd128) q_result={q_sign, 2'b0};
        else if(tgt_exp_s < 1) q_result={q_sign, 1'b0, q_mant[22:22]};
        else q_result={q_sign, tgt_exp_s[0:0], q_mant[22:22]};
    end
    reg [31:0] result_reg; reg result_ready;
    always @(posedge mclk or posedge rst) begin
        if(rst) begin result_reg<=0;result_ready<=0; end
        else begin result_ready<=add_ovld;
            if(add_ovld) result_reg<={29'b0,q_result};
        end
    end
