...
`timescale 1ns / 1ps
// corona_compute_fp48_fma_ax7203 — FP48 FMA on AX7203.
module corona_compute_fp48_fma_ax7203 (
    ...
    wire signed [9:0] f_de_a = $signed({1'b0, f_exp_a}) - 10'sd63 + 10'sd127;
    ...
    always @(*) begin
        ...
        else if(f_sub_a) fp32_a={f_sign_a, 8'd65, f_mant32_norm_a};
        else fp32_a={f_sign_a, f_exp32_a, f_mant32_a};
    end
    wire f_sign_b = fmt_b[47];
    wire [6:0] f_exp_b = fmt_b[46:40];
    ...
    wire signed [9:0] f_de_b = $signed({1'b0, f_exp_b}) - 10'sd63 + 10'sd127;
    ...
    always @(*) begin
        ...
        else if(f_sub_b) fp32_b={f_sign_b, 8'd65, f_mant32_norm_b};
        else fp32_b={f_sign_b, f_exp32_b, f_mant32_b};
    end
    wire f_sign_c = fmt_c[47];
    wire [6:0] f_exp_c = fmt_c[46:40];
    wire [39:0] f_mant_c = fmt_c[39:0];
    wire f_zero_c = (f_exp_c == 0) && (f_mant_c == 0);
    wire f_inf_c = (f_exp_c == 127) && (f_mant_c == 0);
    wire f_nan_c = (f_exp_c == 127) && (f_mant_c != 0);
    wire f_sub_c = (f_exp_c == 0) && (f_mant_c != 0);
    wire signed [9:0] f_de_c = $signed({1'b0, f_exp_c}) - 10'sd63 + 10'sd127;
    wire [7:0] f_exp32_c = f_de_c[7:0];
    wire [22:0] f_mant32_c = {f_mant_c, -17'b0};
    wire [22:0] f_mant32_norm_c = {1'b0, f_mant_c, -18'b0};
    reg [31:0] fp32_c;
    always @(*) begin
        ...
        else if(f_sub_c) fp32_c={f_sign_c, 8'd65, f_mant32_norm_c};
    ...
    wire signed [9:0] tgt_exp_s = $signed({1'b0, q_exp}) - 10'sd127 + 10'sd63;
    reg [47:0] q_result;
    always @(*) begin
        if(q_nan) q_result=48'd0;
        else if(q_zero) q_result=48'd0;
        else if(q_inf) q_result={q_sign, 7'd127, 40'd0};
        else if(q_exp >= 8'd192) q_result={q_sign, 7'd127, 40'd0};
        else if(q_exp < 8'd65) q_result={q_sign, 47'b0};
        else if(tgt_exp_s < 1) q_result={q_sign, 7'b0, q_mant};
        else q_result={q_sign, tgt_exp_s[6:0], q_mant};
    end
