...
`timescale 1ns / 1ps
// corona_compute_fp96_e8m87_fma_ax7203 — FP96_E8M87 FMA on AX7203.
module corona_compute_fp96_e8m87_fma_ax7203 (
    ...
    reg [5:0] frm; reg [7:0] fmt_r; reg [95:0] a_r,b_r,c_r; reg frame_valid;
    always @(posedge mclk or posedge rst) begin
        if(rst) begin frm<=0;fmt_r<=0;a_r<=0;b_r<=0;c_r<=0;frame_valid<=0; end
            ...
            if(rx_new) begin case(frm)
                ...
                6'd26: begin b_r[95:88]<=rx_byte;frm<=6'd27; end
                ...
                6'd38: begin c_r[95:88]<=rx_byte;frm<=6'd39; end
    ...
    reg [95:0] a_reg,b_reg,c_reg; reg comp_trigger;
    wire [95:0] fmt_a=a_reg, fmt_b=b_reg, fmt_c=c_reg;
    wire f_sign_a = fmt_a[95];
    wire [7:0] f_exp_a = fmt_a[94:87];
    wire [86:0] f_mant_a = fmt_a[86:0];
    ...
    wire [22:0] f_mant32_a = {f_mant_a, -64'b0};
    ...
    wire f_sign_b = fmt_b[95];
    wire [7:0] f_exp_b = fmt_b[94:87];
    wire [86:0] f_mant_b = fmt_b[86:0];
    ...
    wire [22:0] f_mant32_b = {f_mant_b, -64'b0};
    ...
    wire f_sign_c = fmt_c[95];
    wire [7:0] f_exp_c = fmt_c[94:87];
    wire [86:0] f_mant_c = fmt_c[86:0];
    ...
    wire [22:0] f_mant32_c = {f_mant_c, -64'b0};
    ...
    reg [95:0] q_result;
    always @(*) begin
        if(q_nan) q_result=96'd0;
        else if(q_zero) q_result=96'd0;
        else if(q_inf) q_result={q_sign, 8'd255, 87'd0};
        else if(q_exp >= 8'd254) q_result={q_sign, 8'd255, 87'd0};
        else if(q_exp < 8'd1) q_result={q_sign, 95'b0};
