...
`timescale 1ns / 1ps
// corona_decode_gf64_ax7203 — signed int128 → FP32 decode on AX7203.
// 16-byte code frame (128-bit input).
module corona_decode_gf64_ax7203 (
    ...
    gf_decode_param #(.N(64), .E(24), .M(39), .BIAS(8388607), .OUT_REG(0)) u_dec (.gf_in(code_r), .fp32_out(result), .is_nan_o(), .is_inf_o(), .is_zero_o(), .is_subnormal_o());
    assign led[2] = |result;
