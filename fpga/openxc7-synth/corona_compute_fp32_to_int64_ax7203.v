...
`timescale 1ns / 1ps
// corona_compute_fp32_to_int64_ax7203 — INT64 FP32_TO on AX7203.
module corona_compute_fp32_to_int64_ax7203 (
    ...
    reg signed [63:0] int_result;
    reg [30:0] tmp_mant;
    always @(*) begin
        if(is_zero||is_nan) int_result=0;
        else begin
            tmp_mant = {1'b1, mant_v} >> (127 - exp_v);
            int_result = sign_v ? -$signed({{33{1'b0}},tmp_mant}) : $signed({{33{1'b0}},tmp_mant});
        end
    end
    always @(posedge mclk or posedge rst) begin
    ...
    always @(posedge mclk or posedge rst) begin
        if(rst) begin result_reg<=0;result_ready<=0; end
            ...
            if(conv_trigger) result_reg<=int_result;
        end
    end
