...
`timescale 1ns / 1ps
// corona_compute_fp32_to_int4_ax7203 — INT4 FP32_TO on AX7203.
module corona_compute_fp32_to_int4_ax7203 (
    ...
    wire sign_v = a_reg[31];
    wire [7:0] exp_v = a_reg[30:23];
    wire [22:0] mant_v = a_reg[22:0];
    wire is_zero = (exp_v==0)&&(mant_v==0);
    wire is_nan = (exp_v==8'hFF)&&(mant_v!=0);
    wire is_inf = (exp_v==8'hFF)&&(mant_v==0);
    reg signed [3:0] int_result;
    reg [31:0] tmp_mant; reg [7:0] tmp_exp; integer j;
    always @(*) begin
        if(is_zero||is_nan||is_inf) int_result=0;
        else begin
            tmp_exp = exp_v - 8'd127;
            tmp_mant = {1'b1, mant_v, 9'b0};
            tmp_mant = tmp_mant >> (23 - tmp_exp);
            int_result = sign_v ? -$signed(tmp_mant[31:0]) : $signed(tmp_mant[3:0]);
        end
    end
    always @(posedge mclk or posedge rst) begin
    ...
    always @(posedge mclk or posedge rst) begin
        if(rst) begin result_reg<=0;result_ready<=0; end
            ...
            if(conv_trigger) result_reg<={{28{1'b0}},int_result};
        end
    end
