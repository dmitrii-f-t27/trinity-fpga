...
`timescale 1ns / 1ps
// corona_compute_gf4_alu_ax7203 — GoldenFloat4 ALU (ADD+MUL) on AX7203.
// Instantiates both gf_adder_param and gf_mul_param in parallel, selects via op byte.
// GF4: [S:1][E:1][M:2] = 4 bits, BIAS=0, HAS_INF=0.
//
...
module corona_compute_gf4_alu_ax7203 (
    ...
    // ---- Frame FSM ----
    reg [2:0] frm; reg [7:0] fmt_r; reg [7:0] op_r; reg [3:0] a_r, b_r; reg frame_valid;
    always @(posedge mclk or posedge rst) begin
        if(rst) begin frm<=0;fmt_r<=0;op_r<=0;a_r<=0;b_r<=0;frame_valid<=0; end
            ...
            if(rx_new) begin case(frm)
                ...
                3'd4: begin a_r[3:0]<=rx_byte[3:0];frm<=3'd5; end
                3'd5: begin b_r[3:0]<=rx_byte[3:0];frm<=3'd6; end
    ...
    // ---- GF4 ADD + MUL (both compute in parallel, mux selects) ----
    reg [3:0] a_reg, b_reg;
    reg comp_trigger;
    wire add_in_ready, add_out_valid;
    wire mul_in_ready, mul_out_valid;
    wire [3:0] add_result, mul_result;

    gf_adder_param #(.EXP_BITS(1), .MANT_BITS(2), .HAS_INF(0)) u_add (
        .clk(mclk), .rst(rst),
        .in_valid(comp_trigger), .in_a(a_reg), .in_b(b_reg), .in_ready(add_in_ready),
        .out_valid(add_out_valid), .out_y(add_result), .out_ready(1'b1)
    );

    gf_mul_param #(.EXP_BITS(1), .MANT_BITS(2), .HAS_INF(0)) u_mul (
        .clk(mclk), .rst(rst),
    ...
    wire [3:0] comp_result = is_mul ? mul_result : add_result;
    ...
    always @(posedge mclk or posedge rst) begin
        if(rst) begin responding<=0;tx_idx<=0;tcnt<=BAUD_DIV-1;tbi<=0;tsr<=10'h3FF;uart_tx<=1;
            ...
            if(result_ready) begin
                tx_buf0<=8'hA5; tx_buf1<={4'b0, result_reg[3:0]}; tx_buf2<=8'h00; tx_buf3<=8'h00; tx_buf4<=8'h00;
