...
`timescale 1ns / 1ps
// corona_compute_mxfp8_e4m3_fma_ax7203 — MXFP8 FMA on AX7203.
module corona_compute_mxfp8_e4m3_fma_ax7203 (
    ...
    wire m8_sign_a = fmt_a[7];
    wire [3:0] m8_exp_a = fmt_a[6:3];
    wire [2:0] m8_mant_a = fmt_a[2:0];
    wire m8_zero_a = (fmt_a == 8'd0);
    wire m8_sub_a = (m8_exp_a == 4'd0) && (m8_mant_a != 3'd0);
    wire [7:0] m8_exp32_a = m8_sub_a ? 8'd120 : ({4'd0, m8_exp_a} + 8'd120);
    ...
    wire [3:0] m8_exp_b = fmt_b[6:3];
    wire [2:0] m8_mant_b = fmt_b[2:0];
    wire m8_zero_b = (fmt_b == 8'd0);
    wire m8_sub_b = (m8_exp_b == 4'd0) && (m8_mant_b != 3'd0);
    wire [7:0] m8_exp32_b = m8_sub_b ? 8'd120 : ({4'd0, m8_exp_b} + 8'd120);
    ...
    wire [3:0] m8_exp_c = fmt_c[6:3];
    wire [2:0] m8_mant_c = fmt_c[2:0];
    wire m8_zero_c = (fmt_c == 8'd0);
    wire m8_sub_c = (m8_exp_c == 4'd0) && (m8_mant_c != 3'd0);
    wire [7:0] m8_exp32_c = m8_sub_c ? 8'd120 : ({4'd0, m8_exp_c} + 8'd120);
    ...
    wire [8:0] q_m8_e = {1'b0, q_exp} - 9'd120;
    reg [7:0] q_result;
    always @(*) begin
        if(q_nan) q_result=8'h00;
        else if(q_zero||q_exp<8'd121) q_result=8'h00;
        else if(q_exp>=8'd135) q_result={q_sign,4'd14,3'b0};
        else q_result={q_sign,q_m8_e[3:0],q_mant[22:20]};
    end
