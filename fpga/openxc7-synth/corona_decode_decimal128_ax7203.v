...
`timescale 1ns / 1ps
// corona_decode_decimal128_ax7203 — Bailey/Hida double-double (2x FP64) decode on AX7203.
// 128-bit decode frame: AA 55 fmt code[0..15] trig (19 bytes) -> A5 FP32 LE.
// Infrastructure mirrors corona_decode_binary64 (extended to a 16-byte code field).
module corona_decode_decimal128_ax7203 (
    ...
    // 128-bit decode frame: AA 55 fmt code[0..15] trig (19 bytes); counter collects 16 code bytes
    reg [4:0] frm; reg [7:0] fmt_r; reg [127:0] code_r; reg [3:0] bcnt; reg frame_valid;
    always @(posedge mclk or posedge rst) begin
        if(rst) begin frm<=0;fmt_r<=0;code_r<=0;bcnt<=0;frame_valid<=0; end
        else begin frame_valid<=0;
            if(rx_new) begin case(frm)
                5'd0: frm<=(rx_byte==8'hAA)?5'd1:5'd0;
                5'd1: frm<=(rx_byte==8'h55)?5'd2:5'd0;
                5'd2: begin fmt_r<=rx_byte; frm<=5'd3; bcnt<=0; end
                5'd3: begin code_r[bcnt*8 +: 8]<=rx_byte;
                       if(bcnt==4'd15) frm<=5'd4; else bcnt<=bcnt+4'd1; end
                5'd4: begin frame_valid<=1; frm<=5'd0; end
                default: frm<=0;
            endcase end
        end
    end
    ...
    wire [31:0] result;
    decimal128_wide_decode u_dec (.bid128(code_r[127:0]), .fp32_out(result));
    assign led[2] = |result;
