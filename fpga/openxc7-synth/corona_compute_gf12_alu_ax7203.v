...
`timescale 1ns / 1ps
// corona_compute_gf12_alu_ax7203 — GoldenFloat12 ALU (ADD+MUL) on AX7203.
...
module corona_compute_gf12_alu_ax7203 (
    ...
    // ---- Frame FSM ----
    reg [3:0] frm; reg [7:0] fmt_r; reg [7:0] op_r; reg [11:0] a_r, b_r; reg frame_valid;
    always @(posedge mclk or posedge rst) begin
        if(rst) begin frm<=0;fmt_r<=0;op_r<=0;a_r<=0;b_r<=0;frame_valid<=0; end
            ...
            if(rx_new) begin case(frm)
                ...
                4'd5: begin a_r[11:8]<=rx_byte[3:0];frm<=4'd6; end
                4'd6: begin b_r[7:0]<=rx_byte;frm<=4'd7; end
                4'd7: begin b_r[11:8]<=rx_byte[3:0];frm<=4'd8; end
    ...
    wire [11:0] comp_result = is_mul ? mul_result : add_result;
    ...
    always @(posedge mclk or posedge rst) begin
        if(rst) begin responding<=0;tx_idx<=0;tcnt<=BAUD_DIV-1;tbi<=0;tsr<=10'h3FF;uart_tx<=1;
            ...
            if(result_ready) begin
                tx_buf0<=8'hA5; tx_buf1<=result_reg[7:0]; tx_buf2<=result_reg[11:8]; tx_buf3<=8'h00; tx_buf4<=8'h00;
