...
`timescale 1ns / 1ps
// corona_compute_gf4_to_fp32_ax7203 — GF4 to IEEE binary32 converter.
// Input: GF4 [S:1][E:1][M:2] (bias=0, HAS_INF=0)
...
module corona_compute_gf4_to_fp32_ax7203 (
    ...
    // ---- Frame FSM ----
    reg [2:0] frm; reg [7:0] fmt_r; reg [3:0] a_r; reg frame_valid;
    always @(posedge mclk or posedge rst) begin
        if(rst) begin frm<=0;fmt_r<=0;a_r<=0;frame_valid<=0; end
            ...
            if(rx_new) begin case(frm)
                ...
                3'd3: begin a_r[3:0]<=rx_byte[3:0];frm<=3'd4; end
    ...
    wire        gf_sign = a_reg[3];
    wire [0:0] gf_exp  = a_reg[2:2];
    wire [1:0] gf_mant = a_reg[1:0];
    ...
    // FP32 exp = gf_exp + 127 (= gf_exp - 0 + 127)
    wire [9:0] fp32_exp_norm = $unsigned(gf_exp) + 10'd127;
    wire [22:0] fp32_mant_norm = {gf_mant, 21'b0};
    ...
    always @(*) begin
        if (gf_zero)
            ...
            fp32_result = {gf_sign, 8'b0, {gf_mant, 21'b0}};
        else
