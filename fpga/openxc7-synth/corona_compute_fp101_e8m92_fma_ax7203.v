...
`timescale 1ns / 1ps
// corona_compute_fp101_e8m92_fma_ax7203 — FP101_E8M92 FMA on AX7203.
module corona_compute_fp101_e8m92_fma_ax7203 (
    ...
    reg [5:0] frm; reg [7:0] fmt_r; reg [100:0] a_r,b_r,c_r; reg frame_valid;
    always @(posedge mclk or posedge rst) begin
        if(rst) begin frm<=0;fmt_r<=0;a_r<=0;b_r<=0;c_r<=0;frame_valid<=0; end
            ...
            if(rx_new) begin case(frm)
                ...
                6'd15: begin a_r[100:96]<=rx_byte;frm<=6'd16; end
                ...
                6'd28: begin b_r[100:96]<=rx_byte;frm<=6'd29; end
                ...
                6'd41: begin c_r[100:96]<=rx_byte;frm<=6'd42; end
    ...
    reg [100:0] a_reg,b_reg,c_reg; reg comp_trigger;
    wire [100:0] fmt_a=a_reg, fmt_b=b_reg, fmt_c=c_reg;
    wire f_sign_a = fmt_a[100];
    wire [7:0] f_exp_a = fmt_a[99:92];
    ...
    wire f_sign_b = fmt_b[100];
    wire [7:0] f_exp_b = fmt_b[99:92];
    ...
    wire f_sign_c = fmt_c[100];
    wire [7:0] f_exp_c = fmt_c[99:92];
    ...
    reg [100:0] q_result;
    always @(*) begin
        if(q_nan) q_result=101'd0;
        else if(q_zero) q_result=101'd0;
        else if(q_inf) q_result={q_sign, 8'd255, 92'd0};
        else if(q_exp >= 8'd254) q_result={q_sign, 8'd255, 92'd0};
        else if(q_exp < 8'd1) q_result={q_sign, 100'b0};
        else if(tgt_exp_s < 1) q_result={q_sign, 8'b0, q_mant[22:0]};
        else q_result={q_sign, tgt_exp_s[7:0], q_mant[22:0]};
    end
    reg [100:0] result_reg; reg result_ready;
    always @(posedge mclk or posedge rst) begin
