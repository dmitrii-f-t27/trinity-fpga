...
`timescale 1ns / 1ps
// corona_compute_fp73_e8m64_alu_ax7203 — FP73_E8M64 ALU on AX7203.
module corona_compute_fp73_e8m64_alu_ax7203 (
    ...
    reg [4:0] frm; reg [7:0] fmt_r,op_r; reg [72:0] a_r,b_r; reg frame_valid;
    always @(posedge mclk or posedge rst) begin
        if(rst) begin frm<=0;fmt_r<=0;op_r<=0;a_r<=0;b_r<=0;frame_valid<=0; end
            ...
            if(rx_new) begin case(frm)
                ...
                5'd13: begin a_r[72:72]<=rx_byte;frm<=5'd14; end
                ...
                5'd23: begin b_r[72:72]<=rx_byte;frm<=5'd24; end
    ...
    reg [72:0] a_reg,b_reg; reg [7:0] op_reg; reg comp_trigger;
    wire [72:0] fmt_a=a_reg, fmt_b=b_reg;
    wire f_sign_a = fmt_a_a[72];
    wire [7:0] f_exp_a = fmt_a_a[71:64];
    ...
    wire f_sign_b = fmt_b_b[72];
    wire [7:0] f_exp_b = fmt_b_b[71:64];
    ...
    reg [72:0] q_result;
    always @(*) begin
        if(q_nan) q_result=73'd0;
        else if(q_zero) q_result=73'd0;
        else if(q_inf) q_result={q_sign, 8'd255, 64'd0};
        else if(q_exp >= 8'd254) q_result={q_sign, 8'd255, 64'd0};
        else if(q_exp < 8'd1) q_result={q_sign, 72'b0};
        else if(tgt_exp_s < 1) q_result={q_sign, 8'b0, q_mant[22:0]};
        else q_result={q_sign, tgt_exp_s[7:0], q_mant[22:0]};
    end
    reg [72:0] result_reg; reg result_ready;
    always @(posedge mclk or posedge rst) begin
