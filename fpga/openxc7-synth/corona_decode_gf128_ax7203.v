...
`timescale 1ns / 1ps
// corona_decode_gf128_ax7203 — signed int128 → FP32 decode on AX7203.
// 16-byte code frame (128-bit input).
module corona_decode_gf128_ax7203 (
    ...
    gf_decode_param #(.N(128), .E(49), .M(78), .BIAS(281474976710655), .OUT_REG(0)) u_dec (.gf_in(code_r), .fp32_out(result), .is_nan_o(), .is_inf_o(), .is_zero_o(), .is_subnormal_o());
    assign led[2] = |result;
