...
`timescale 1ns / 1ps
// corona_compute_fp32_to_gf48_ax7203 — IEEE binary32 to GF48 converter.
// Output: GF48 [S:1][E:18][M:29]
module corona_compute_fp32_to_gf48_ax7203 (
    ...
    wire signed [19:0] gf_exp_calc = $signed({2'b0, fp_exp}) + 20'sd130944;
    wire overflow = (gf_exp_calc > 20'sd262142);
    wire underflow = (gf_exp_calc < 20'sd1);
    wire [28:0] gf_mant_trunc = fp_mant[22:-6];
    reg [47:0] gf_result;
    always @(*) begin
        if(fp_nan) gf_result = {1'b0, {18{1'b1}}, {28{1'b0}}, 1'b1};
        else if(fp_inf || overflow) gf_result = {fp_sign, {18{1'b1}}, {29{1'b0}}};
        else if(fp_zero || fp_denorm || underflow) gf_result = {fp_sign, {18{1'b0}}, {29{1'b0}}};
        else gf_result = {fp_sign, gf_exp_calc[17:0], gf_mant_trunc};
    end
    always @(posedge mclk or posedge rst) begin
