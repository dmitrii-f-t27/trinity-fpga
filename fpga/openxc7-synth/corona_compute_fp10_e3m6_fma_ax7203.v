...
`timescale 1ns / 1ps
// corona_compute_fp10_e3m6_fma_ax7203 — FP10_E3M6 FMA on AX7203.
module corona_compute_fp10_e3m6_fma_ax7203 (
    ...
    wire [2:0] f_exp_b = fmt_b[8:6];
    wire [5:0] f_mant_b = fmt_b[5:0];
    wire f_zero_b = (f_exp_b == 0) && ((f_mant_b == 0));
    wire f_inf_b = (f_exp_b == 7) && ((f_mant_b == 0));
    wire f_nan_b = (f_exp_b == 7) && ((f_mant_b != 0));
    ...
    wire [2:0] f_exp_c = fmt_c[8:6];
    wire [5:0] f_mant_c = fmt_c[5:0];
    wire f_zero_c = (f_exp_c == 0) && ((f_mant_c == 0));
    wire f_inf_c = (f_exp_c == 7) && ((f_mant_c == 0));
    wire f_nan_c = (f_exp_c == 7) && ((f_mant_c != 0));
    wire f_sub_c = (f_exp_c == 0) && ((f_mant_c != 0));
    wire signed [12:0] f_de_c = $signed({1'b0, f_exp_c}) - 13'sd3 + 13'sd127;
    wire [7:0] f_exp32_c = (f_de_c > 13'sd254) ? 8'd254 : (f_de_c < 0) ? 8'd0 : f_de_c[7:0];
    ...
    always @(*) begin
        ...
        else if(f_sub_c) fp32_c={f_sign_c, 8'd125, f_mant32_norm_c};
    ...
    always @(*) begin
        ...
        else if(q_inf) q_result={q_sign, 3'd7, 6'd0};
        else if(q_exp >= 8'd132) q_result={q_sign, 3'd7, 6'd0};
        else if(q_exp < 8'd125) q_result={q_sign, 9'b0};
        else if(tgt_exp_s < 1) q_result={q_sign, 3'b0, q_mant[22:17]};
        else q_result={q_sign, tgt_exp_s[2:0], q_mant[22:17]};
    end
