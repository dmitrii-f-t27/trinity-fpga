...
`timescale 1ns / 1ps
// corona_compute_fp4_e2m1_alu_ax7203 — FP4_E2M1 ALU on AX7203.
module corona_compute_fp4_e2m1_alu_ax7203 (
    ...
    wire add_irdy,add_ovld; wire [3:0] add_res;
    wire mul_irdy,mul_ovld; wire [3:0] mul_res;
    gf_adder_param #(.EXP_BITS(2),.MANT_BITS(1),.HAS_INF(0)) u_add (
    ...
    gf_mul_param #(.EXP_BITS(2),.MANT_BITS(1),.HAS_INF(0)) u_mul (
        .clk(mclk),.rst(rst),.in_valid(comp_trigger),.in_a(a_reg),.in_b(b_reg),
