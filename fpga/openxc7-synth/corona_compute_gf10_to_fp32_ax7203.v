...
`timescale 1ns / 1ps
// corona_compute_gf10_to_fp32_ax7203 — GF10 to IEEE binary32 converter.
// Input: GF10 [S:1][E:3][M:6] (bias=3, HAS_INF=0)
...
module corona_compute_gf10_to_fp32_ax7203 (
    ...
    // ---- Frame FSM ----
    reg [2:0] frm; reg [7:0] fmt_r; reg [9:0] a_r; reg frame_valid;
    always @(posedge mclk or posedge rst) begin
        if(rst) begin frm<=0;fmt_r<=0;a_r<=0;frame_valid<=0; end
            ...
            if(rx_new) begin case(frm)
                ...
                3'd4: begin a_r[9:8]<=rx_byte[1:0];frm<=3'd5; end
    ...
    // ---- GF10->FP32 conversion ----
    reg [9:0] a_reg;
    reg conv_trigger;

    wire        gf_sign = a_reg[9];
    wire [2:0] gf_exp  = a_reg[8:6];
    wire [5:0] gf_mant = a_reg[5:0];
    ...
    // FP32 exp = gf_exp + 124 (= gf_exp - 3 + 127)
    wire [9:0] fp32_exp_norm = $unsigned(gf_exp) + 10'd124;
    wire [22:0] fp32_mant_norm = {gf_mant, 17'b0};
    ...
    always @(*) begin
        if (gf_zero)
            ...
            fp32_result = {gf_sign, 8'b0, {gf_mant, 17'b0}};
        else
