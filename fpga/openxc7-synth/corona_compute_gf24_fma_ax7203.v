...
`timescale 1ns / 1ps
// corona_compute_gf24_fma_ax7203 — GoldenFloat24 FMA (y = a*b + c) on AX7203.
// GF24: [S:1][E:9][M:14] = 24 bits, BIAS=255, HAS_INF=0.
// Pipelined: gf_mul_param (1 cycle) -> gf_adder_param (1 cycle). Latency: 2 cycles.
// 3 byte(s) per operand, 13-byte frame.
//
// Frame protocol (13 bytes TX):
//   AA 55 fmt a0 a1 a2 b0 b1 b2 c0 c1 c2 trig
// Response (5 bytes RX):
//   A5 r0 r1 r2 00

module corona_compute_gf24_fma_ax7203 (
    ...
    always @(posedge mclk or posedge rst) begin
        if(rst) begin
            ...
            mul_trigger<=0; add_trigger<=0;
        end else begin
            mul_trigger <= frame_valid;
            if(frame_valid) begin a_reg<=a_r; b_reg<=b_r; c_reg<=c_r; end
            add_trigger <= mul_out_valid;
        end
    end
    ...
    always @(posedge mclk or posedge rst) begin
        if(rst) begin result_reg<=0; result_ready<=0; end
            ...
            result_ready <= add_out_valid;
            if(add_out_valid) result_reg <= add_result;
        end
    end
    ...
    always @(posedge mclk or posedge rst) begin
        if(rst) begin responding<=0;tx_idx<=0;tcnt<=BAUD_DIV-1;tbi<=0;tsr<=10'h3FF;uart_tx<=1;
            ...
            if(result_ready) begin
                tx_buf0<=8'hA5; tx_buf1<=result_reg[7:0]; tx_buf2<=result_reg[15:8]; tx_buf3<=result_reg[23:16]; tx_buf4<=8'h00;
