...
`timescale 1ns / 1ps
// corona_compute_fp32_to_gf8_ax7203 — IEEE binary32 to GF8 converter.
// Input: FP32 [S:1][E:8][M:23] (bias=127)
// Output: GF8 [S:1][E:3][M:4] (bias=3, HAS_INF=0)
// Mantissa truncation. Overflow -> max, underflow -> zero.
module corona_compute_fp32_to_gf8_ax7203 (
    ...
    wire [3:0] gf_mant_trunc = fp_mant[22:19];
    reg [7:0] gf_result;
    always @(*) begin
        if (fp_nan)
            gf_result = {fp_sign, {3{1'b1}}, {4{1'b0}}};
        else if (fp_inf || overflow)
            gf_result = {fp_sign, gf_exp_calc[2:0] | {3{1'b1}}, {4{1'b0}}};
        else if (fp_zero || fp_denorm || underflow)
            gf_result = {fp_sign, {3{1'b0}}, {4{1'b0}}};
        else
    ...
    always @(posedge mclk or posedge rst) begin
        if(rst) begin responding<=0;tx_idx<=0;tcnt<=BAUD_DIV-1;tbi<=0;tsr<=10'h3FF;uart_tx<=1;
            ...
            if(result_ready) begin
                tx_buf0<=8'hA5; tx_buf1<=result_reg[7:0]; tx_buf2<=8'h00; tx_buf3<=8'h00; tx_buf4<=8'h00;
