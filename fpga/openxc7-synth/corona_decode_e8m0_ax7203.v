...
`timescale 1ns / 1ps
// corona_decode_e8m0_ax7203 — E8M0 (OCP MX shared scale factor) decode on AX7203.
// Single-decoder build: CFGMCLK (~70 MHz, STARTUPE2) clock, BAUD_DIV 434 (~161290).
// Frame: AA 55 fmt code_lo code_hi trig -> A5 r0 r1 r2 r3 (32-bit decoded FP32 LE).
// Identical infrastructure to corona_decode_fp8_e5m2_ax7203; only the decoder differs
// (e8m0_decode: 8-bit exponent-only, value = 2^(e-127), 0xFF=NaN, 0x00=2^-127).
module corona_decode_e8m0_ax7203 (
    ...
    wire [31:0] result;
    wire is_nan;
    e8m0_decode u_dec (.e8m0_in(code_r[7:0]), .fp32_out(result), .is_nan(is_nan));
    assign led[2] = |result;
