...
`timescale 1ns / 1ps
// corona_compute_fp10_fma_ax7203 — FP10 FMA on AX7203.
module corona_compute_fp10_fma_ax7203 (
    ...
    reg [3:0] frm; reg [7:0] fmt_r; reg [9:0] a_r,b_r,c_r; reg frame_valid;
    always @(posedge mclk or posedge rst) begin
        if(rst) begin frm<=0;fmt_r<=0;a_r<=0;b_r<=0;c_r<=0;frame_valid<=0; end
            ...
            if(rx_new) begin case(frm)
                ...
                4'd4: begin a_r[9:8]<=rx_byte;frm<=4'd5; end
                4'd5: begin b_r[7:0]<=rx_byte;frm<=4'd6; end
                4'd6: begin b_r[9:8]<=rx_byte;frm<=4'd7; end
                4'd7: begin c_r[7:0]<=rx_byte;frm<=4'd8; end
                4'd8: begin c_r[9:8]<=rx_byte;frm<=4'd9; end
    ...
    reg [9:0] a_reg,b_reg,c_reg; reg comp_trigger;
    wire [9:0] fmt_a=a_reg, fmt_b=b_reg, fmt_c=c_reg;
    wire f_sign_a = fmt_a[9];
    wire [3:0] f_exp_a = fmt_a[8:5];
    ...
    wire signed [9:0] f_de_a = $signed({1'b0, f_exp_a}) - 10'sd7 + 10'sd127;
    ...
    wire f_sign_b = fmt_b[9];
    wire [3:0] f_exp_b = fmt_b[8:5];
    ...
    wire signed [9:0] f_de_b = $signed({1'b0, f_exp_b}) - 10'sd7 + 10'sd127;
    ...
    wire f_sign_c = fmt_c[9];
    wire [3:0] f_exp_c = fmt_c[8:5];
    ...
    wire signed [9:0] f_de_c = $signed({1'b0, f_exp_c}) - 10'sd7 + 10'sd127;
    ...
    wire signed [9:0] tgt_exp_s = $signed({1'b0, q_exp}) - 10'sd127 + 10'sd7;
    reg [9:0] q_result;
    always @(*) begin
        if(q_nan) q_result=10'd0;
        else if(q_zero) q_result=10'd0;
        else if(q_inf) q_result={q_sign, 4'd15, 5'd0};
        else if(q_exp >= 8'd136) q_result={q_sign, 4'd15, 5'd0};
        else if(q_exp < 8'd121) q_result={q_sign, 9'b0};
        else if(tgt_exp_s < 1) q_result={q_sign, 4'b0, q_mant[22:18]};
        else q_result={q_sign, tgt_exp_s[3:0], q_mant[22:18]};
    end
    reg [31:0] result_reg; reg result_ready;
    always @(posedge mclk or posedge rst) begin
        if(rst) begin result_reg<=0;result_ready<=0; end
        else begin result_ready<=add_ovld;
            if(add_ovld) result_reg<={22'b0,q_result};
        end
    end
