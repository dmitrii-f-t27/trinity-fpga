...
module corona_decode_gf6_ax7203 (
    ...
    wire [31:0] result;
    gf_decode_param #(.N(6), .E(2), .HAS_INF(0), .M(3), .BIAS(1), .OUT_REG(0)) u_dec (.clk(1'b0), .rst_n(1'b1), .gf_in(code_r[5:0]), .fp32_out(result), .is_nan_o(), .is_inf_o(), .is_zero_o(), .is_subnormal_o());
    assign led[2] = |result;
