...
`timescale 1ns / 1ps
// corona_compute_fp32_to_gf4_ax7203 — IEEE binary32 to GF4 converter.
// Input: FP32 [S:1][E:8][M:23] (bias=127)
// Output: GF4 [S:1][E:1][M:2] (bias=0, HAS_INF=0)
// Mantissa truncation. Overflow -> max, underflow -> zero.
module corona_compute_fp32_to_gf4_ax7203 (
    ...
    wire signed [9:0] gf_exp_calc = $signed({2'b0, fp_exp}) + 10'sd-127;
    wire overflow  = (gf_exp_calc > 10'sd1);
    wire underflow = (gf_exp_calc < 10'sd1);
    wire [1:0] gf_mant_trunc = fp_mant[22:21];
    reg [3:0] gf_result;
    always @(*) begin
        if (fp_nan)
            gf_result = {fp_sign, {1{1'b1}}, {2{1'b0}}};
        else if (fp_inf || overflow)
            gf_result = {fp_sign, gf_exp_calc[0:0] | {1{1'b1}}, {2{1'b0}}};
        else if (fp_zero || fp_denorm || underflow)
            gf_result = {fp_sign, {1{1'b0}}, {2{1'b0}}};
        else
            gf_result = {fp_sign, gf_exp_calc[0:0], gf_mant_trunc};
    end
    always @(posedge mclk or posedge rst) begin
        if(rst) begin a_reg<=0; conv_trigger<=0; end
        else begin conv_trigger <= frame_valid; if(frame_valid) a_reg <= a_r; end
    end
    reg [3:0] result_reg; reg result_ready;
    always @(posedge mclk or posedge rst) begin
