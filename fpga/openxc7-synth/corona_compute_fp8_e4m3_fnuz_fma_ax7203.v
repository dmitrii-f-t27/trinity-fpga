...
`timescale 1ns / 1ps
// corona_compute_fp8_e4m3_fnuz_fma_ax7203 — FP8_E4M3_FNUZ FMA on AX7203.
module corona_compute_fp8_e4m3_fnuz_fma_ax7203 (
    ...
    wire [7:0] fnuz_val_a = fmt_a;
    wire fnuz_sign_a = fnuz_val_a[7];
    wire [3:0] fnuz_exp_a = fnuz_val_a[6:3];
    wire [2:0] fnuz_mant_a = fnuz_val_a[2:0];
    wire fnuz_zero_a = (fnuz_exp_a==0);
    wire fnuz_nan_a = (fnuz_val_a==8'hFF);
    // bias=8, FP32 exp = fnuz_exp + 127 - 8 = fnuz_exp + 119
    ...
    wire [7:0] fnuz_val_b = fmt_b;
    wire fnuz_sign_b = fnuz_val_b[7];
    wire [3:0] fnuz_exp_b = fnuz_val_b[6:3];
    wire [2:0] fnuz_mant_b = fnuz_val_b[2:0];
    wire fnuz_zero_b = (fnuz_exp_b==0);
    wire fnuz_nan_b = (fnuz_val_b==8'hFF);
    // bias=8, FP32 exp = fnuz_exp + 127 - 8 = fnuz_exp + 119
    wire [7:0] fnuz_e32_b = {4'd0,fnuz_exp_b} + 8'd119;
    reg [31:0] fp32_b;
    always @(*) begin
        if(fnuz_zero_b) fp32_b={fnuz_sign_b, 31'b0};
        else if(fnuz_nan_b) fp32_b=32'h7FC00000;
        else fp32_b={fnuz_sign_b,fnuz_e32_b,{fnuz_mant_b,20'b0}};
    end
    wire [7:0] fnuz_val_c = fmt_c;
    wire fnuz_sign_c = fnuz_val_c[7];
    wire [3:0] fnuz_exp_c = fnuz_val_c[6:3];
    wire [2:0] fnuz_mant_c = fnuz_val_c[2:0];
    wire fnuz_zero_c = (fnuz_exp_c==0);
    wire fnuz_nan_c = (fnuz_val_c==8'hFF);
    // bias=8, FP32 exp = fnuz_exp + 127 - 8 = fnuz_exp + 119
    wire [7:0] fnuz_e32_c = {4'd0,fnuz_exp_c} + 8'd119;
    reg [31:0] fp32_c;
    always @(*) begin
        if(fnuz_zero_c) fp32_c={fnuz_sign_c, 31'b0};
        else if(fnuz_nan_c) fp32_c=32'h7FC00000;
        else fp32_c={fnuz_sign_c,fnuz_e32_c,{fnuz_mant_c,20'b0}};
    end
    ...
    // FP32 exp = fnuz_exp + 119, so fnuz_exp = q_exp - 119
    wire [8:0] q_fe = {1'b0,q_exp} - 9'd119;
    reg [7:0] q_result;
    always @(*) begin
        if(q_nan) q_result=8'hFF;
        else if(q_zero||q_exp<8'd120) q_result=8'h00; // flush subnormals
        else if(q_exp>=8'd134) q_result=q_sign?8'h80:8'h7E; // saturate
        else q_result={q_sign,q_fe[3:0],q_mant[22:20]};
    end
