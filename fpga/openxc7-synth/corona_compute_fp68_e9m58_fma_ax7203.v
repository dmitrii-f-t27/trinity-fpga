...
`timescale 1ns / 1ps
// corona_compute_fp68_e9m58_fma_ax7203 — FP68_E9M58 FMA on AX7203.
module corona_compute_fp68_e9m58_fma_ax7203 (
    ...
    reg [4:0] frm; reg [7:0] fmt_r; reg [67:0] a_r,b_r,c_r; reg frame_valid;
    always @(posedge mclk or posedge rst) begin
        if(rst) begin frm<=0;fmt_r<=0;a_r<=0;b_r<=0;c_r<=0;frame_valid<=0; end
            ...
            if(rx_new) begin case(frm)
                ...
                5'd11: begin a_r[67:64]<=rx_byte;frm<=5'd12; end
                ...
                5'd20: begin b_r[67:64]<=rx_byte;frm<=5'd21; end
                5'd21: begin c_r[7:0]<=rx_byte;frm<=5'd22; end
                5'd22: begin c_r[15:8]<=rx_byte;frm<=5'd23; end
                5'd23: begin c_r[23:16]<=rx_byte;frm<=5'd24; end
                5'd24: begin c_r[31:24]<=rx_byte;frm<=5'd25; end
                5'd25: begin c_r[39:32]<=rx_byte;frm<=5'd26; end
                5'd26: begin c_r[47:40]<=rx_byte;frm<=5'd27; end
                5'd27: begin c_r[55:48]<=rx_byte;frm<=5'd28; end
                5'd28: begin c_r[63:56]<=rx_byte;frm<=5'd29; end
                5'd29: begin c_r[67:64]<=rx_byte;frm<=5'd30; end
                5'd30: begin frame_valid<=1;frm<=0; end
            endcase end
        end
    end
    assign led[1]=frame_valid;

    reg [67:0] a_reg,b_reg,c_reg; reg comp_trigger;
    wire [67:0] fmt_a=a_reg, fmt_b=b_reg, fmt_c=c_reg;
    wire f_sign_a = fmt_a[67];
    wire [8:0] f_exp_a = fmt_a[66:58];
    wire [57:0] f_mant_a = fmt_a[57:0];
    ...
    wire [22:0] f_mant32_a = {f_mant_a, -35'b0};
    ...
    wire f_sign_b = fmt_b[67];
    wire [8:0] f_exp_b = fmt_b[66:58];
    wire [57:0] f_mant_b = fmt_b[57:0];
    ...
    wire [22:0] f_mant32_b = {f_mant_b, -35'b0};
    ...
    wire f_sign_c = fmt_c[67];
    wire [8:0] f_exp_c = fmt_c[66:58];
    wire [57:0] f_mant_c = fmt_c[57:0];
    ...
    wire [22:0] f_mant32_c = {f_mant_c, -35'b0};
    ...
    reg [67:0] q_result;
    always @(*) begin
        if(q_nan) q_result=68'd0;
        else if(q_zero) q_result=68'd0;
        else if(q_inf) q_result={q_sign, 9'd511, 58'd0};
        else if(q_exp >= 8'd254) q_result={q_sign, 9'd511, 58'd0};
        else if(q_exp < 8'd0) q_result={q_sign, 67'b0};
        else if(tgt_exp_s < 1) q_result={q_sign, 9'b0, q_mant[22:0]};
        else q_result={q_sign, tgt_exp_s[8:0], q_mant[22:0]};
    end
    reg [67:0] result_reg; reg result_ready;
    always @(posedge mclk or posedge rst) begin
    ...
    // TX: buffer+mux (no conflicting NBA — fixes tx race). 10 bytes sliced from tx_load[79:0].
    wire [79:0] tx_load = {result_reg, 8'hA5};
    reg responding; reg [3:0] tx_idx; reg [7:0] tx_buf0, tx_buf1, tx_buf2, tx_buf3, tx_buf4, tx_buf5, tx_buf6, tx_buf7, tx_buf8, tx_buf9;
    reg [8:0] tcnt; reg [3:0] tbi; reg [9:0] tsr;
    always @(posedge mclk or posedge rst) begin
        if(rst) begin responding<=0;tx_idx<=0;tcnt<=BAUD_DIV-1;tbi<=0;tsr<=10'h3FF;uart_tx<=1;
            tx_buf0<=8'hFF; tx_buf1<=8'hFF; tx_buf2<=8'hFF; tx_buf3<=8'hFF; tx_buf4<=8'hFF; tx_buf5<=8'hFF; tx_buf6<=8'hFF; tx_buf7<=8'hFF; tx_buf8<=8'hFF; tx_buf9<=8'hFF; end
        else begin uart_tx<=tsr[0];
            if(result_ready) begin
                tx_buf0<=tx_load[7:0]; tx_buf1<=tx_load[15:8]; tx_buf2<=tx_load[23:16]; tx_buf3<=tx_load[31:24]; tx_buf4<=tx_load[39:32]; tx_buf5<=tx_load[47:40]; tx_buf6<=tx_load[55:48]; tx_buf7<=tx_load[63:56]; tx_buf8<=tx_load[71:64]; tx_buf9<=tx_load[79:72]; responding<=1; tx_idx<=0;
            end
            if(tcnt==0) begin tcnt<=BAUD_DIV-1;
                if(tbi==9) begin tbi<=0;
                    if(responding) begin
                        ...
                        endcase
                        if(tx_idx==9) responding<=0; else tx_idx<=tx_idx+1;
