...
`timescale 1ns / 1ps
// corona_compute_fp20_e8m11_fma_ax7203 — FP20_E8M11 FMA on AX7203.
module corona_compute_fp20_e8m11_fma_ax7203 (
    ...
    reg [3:0] frm; reg [7:0] fmt_r; reg [19:0] a_r,b_r,c_r; reg frame_valid;
    always @(posedge mclk or posedge rst) begin
    ...
    reg [19:0] a_reg,b_reg,c_reg; reg comp_trigger;
    wire [19:0] fmt_a=a_reg, fmt_b=b_reg, fmt_c=c_reg;
    wire f_sign_a = fmt_a[19];
    wire [7:0] f_exp_a = fmt_a[18:11];
    ...
    wire f_sign_b = fmt_b[19];
    wire [7:0] f_exp_b = fmt_b[18:11];
    ...
    wire f_sign_c = fmt_c[19];
    wire [7:0] f_exp_c = fmt_c[18:11];
    ...
    reg [19:0] q_result;
    always @(*) begin
        if(q_nan) q_result=20'd0;
        else if(q_zero) q_result=20'd0;
        else if(q_inf) q_result={q_sign, 8'd255, 11'd0};
        else if(q_exp >= 8'd254) q_result={q_sign, 8'd255, 11'd0};
        else if(q_exp < 8'd1) q_result={q_sign, 19'b0};
        else if(tgt_exp_s < 1) q_result={q_sign, 8'b0, q_mant[22:12]};
        else q_result={q_sign, tgt_exp_s[7:0], q_mant[22:12]};
    end
    reg [31:0] result_reg; reg result_ready;
    always @(posedge mclk or posedge rst) begin
        if(rst) begin result_reg<=0;result_ready<=0; end
        else begin result_ready<=add_ovld;
            if(add_ovld) result_reg<={12'b0,q_result};
        end
    end
