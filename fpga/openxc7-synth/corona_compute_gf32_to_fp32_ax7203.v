...
`timescale 1ns / 1ps
// corona_compute_gf32_to_fp32_ax7203 — GF32 to IEEE binary32 converter.
// Input: GF32 [S:1][E:12][M:19] (bias=2047, HAS_INF=0)
...
module corona_compute_gf32_to_fp32_ax7203 (
    ...
    // ---- GF32->FP32 conversion ----
    reg [31:0] a_reg;
    reg conv_trigger;

    wire        gf_sign = a_reg[31];
    wire [11:0] gf_exp  = a_reg[30:19];
    wire [18:0] gf_mant = a_reg[18:0];
    ...
    // FP32 exp = gf_exp + -1920 (= gf_exp - 2047 + 127)
    wire signed [13:0] fp32_exp_norm = $signed(gf_exp) + 14'sd-1920;
    wire [22:0] fp32_mant_norm = {gf_mant, 4'b0};
    ...
    always @(*) begin
        if (gf_zero)
            ...
            fp32_result = {gf_sign, 8'b0, {gf_mant, 4'b0}};
        else
