...
`timescale 1ns / 1ps
// corona_compute_uint4_fma_ax7203 — UINT4 FMA on AX7203.
module corona_compute_uint4_fma_ax7203 (
    ...
    wire [3:0] uval_c = fmt_c_c[3:0];
    wire u_zero_c = (uval_c == 4'd0);
    wire [2:0] u_lzd_c;
    always @(*) begin
        casez(uval_c)
            4'b1???: u_lzd_c=3'd0;
            4'b01??: u_lzd_c=3'd1;
            4'b001?: u_lzd_c=3'd2;
            default: u_lzd_c=3'd3;
        endcase
    end
    wire [7:0] u_fp32_exp_c = 8'd127 + 8'd3 - {5'b0, u_lzd_c};
    wire [3:0] u_shifted_c = uval_c << {u_lzd_c, 1'b0};
    wire [22:0] u_fp32_mant_c = {u_shifted_c[2:0], 20'b0};
    reg [31:0] fp32_c;
    always @(*) begin
