...
`timescale 1ns / 1ps
// corona_compute_int2_fma_ax7203 — INT2 FMA on AX7203.
module corona_compute_int2_fma_ax7203 (
    ...
    always @(*) begin
        case(ival_a)
            ...
            2'b01: fp32_a=32'h3F800000;
            2'b10: fp32_a=32'hC0000000;
    ...
    always @(*) begin
        case(ival_b)
            ...
            2'b01: fp32_b=32'h3F800000;
            2'b10: fp32_b=32'hC0000000;
            2'b11: fp32_b=32'hBF800000;
        endcase
    end
    wire [1:0] ival_c = fmt_c_c[1:0];
    wire sign_c = ival_c[1];
    wire [1:0] abs_val_c = sign_c ? (~ival_c + 2'd1) : ival_c;
    reg [31:0] fp32_c;
    always @(*) begin
        case(ival_c)
            2'b00: fp32_c=32'h00000000;
            2'b01: fp32_c=32'h3F800000;
            2'b10: fp32_c=32'hC0000000;
            2'b11: fp32_c=32'hBF800000;
        endcase
    end
    ...
    reg [1:0] q_result;
    always @(*) begin
        if(q_nan) q_result=2'b00;
        else if(q_zero) q_result=2'b00;
        else begin
            if(q_sign) begin
                if(q_exp >= 8'd128) q_result=2'b10;
                else q_result=2'b11;
            end else begin
                if(q_exp >= 8'd128) q_result=2'b01;
                else q_result=2'b00;
            end
        end
    end
