...
`timescale 1ns / 1ps
// corona_compute_gf16_mul_ax7203 — GoldenFloat16 ADD on AX7203.
// Accepts two GF16 operands via UART, returns GF16 product.
//
// Frame protocol (8 bytes TX):
//   AA 55 fmt a_lo a_hi b_lo b_hi trig
// Response (5 bytes RX):
//   A5 result[7:0] result[15:8] 00 00

module corona_compute_gf16_mul_ax7203 (
    ...
    // ---- Frame FSM: AA 55 fmt a_lo a_hi b_lo b_hi trig ----
    ...
    always @(posedge mclk or posedge rst) begin
        if(rst) begin frm<=0;fmt_r<=0;a_r<=0;b_r<=0;frame_valid<=0; end
            ...
            if(rx_new) begin case(frm)
                ...
                3'd5: begin b_r[7:0]<=rx_byte;frm<=6; end
                3'd6: begin b_r[15:8]<=rx_byte;frm<=7; end
    ...
    // ---- GF16 MUL compute (registered, 1-cycle latency) ----
    reg [15:0] a_reg, b_reg;
    reg comp_trigger;
    wire comp_in_ready, comp_out_valid;
    wire [15:0] comp_result;

    gf_mul_param #(.EXP_BITS(6), .MANT_BITS(9), .HAS_INF(1)) u_comp (
        .clk(mclk), .rst(rst),
