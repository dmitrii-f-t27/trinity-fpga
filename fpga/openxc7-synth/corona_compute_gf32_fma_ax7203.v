...
`timescale 1ns / 1ps
// corona_compute_gf32_fma_ax7203 — GoldenFloat32 FMA (y = a*b + c) on AX7203.
// GF32: [S:1][E:8][M:23] = 32 bits, BIAS=127, HAS_INF=1. Uses -nodsp.
//
// Frame protocol (15 bytes TX):
//   AA 55 fmt a0 a1 a2 a3 b0 b1 b2 b3 c0 c1 c2 c3 trig
// Response (5 bytes RX):
//   A5 r0 r1 r2 r3

module corona_compute_gf32_fma_ax7203 (
    ...
    // ---- Frame FSM: AA 55 fmt a[0..3] b[0..3] c[0..3] trig (15 bytes) ----
    ...
    gf_mul_param #(.EXP_BITS(12), .MANT_BITS(19), .HAS_INF(0)) u_mul (
        .clk(mclk), .rst(rst),
    ...
    gf_adder_param #(.EXP_BITS(12), .MANT_BITS(19), .HAS_INF(0)) u_add (
        .clk(mclk), .rst(rst),
