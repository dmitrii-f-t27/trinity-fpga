...
`timescale 1ns / 1ps
// corona_compute_gf6_add_ax7203 — GoldenFloat6 ADD on AX7203.
// GF6: [S:1][E:2][M:3] = 6 bits, BIAS=1, HAS_INF=0.
// 1 bytes per operand, 5-byte frame.
//
// Frame protocol (5 bytes TX):
//   AA 55 fmt a0 b0 trig
// Response (5 bytes RX):
//   A5 r0 00 00 00

module corona_compute_gf6_add_ax7203 (
    ...
    gf_adder_param #(.EXP_BITS(2), .MANT_BITS(3), .HAS_INF(0)) u_comp (
        .clk(mclk), .rst(rst),
    ...
    always @(posedge mclk or posedge rst) begin
        if(rst) begin a_reg<=0; b_reg<=0; comp_trigger<=0; end
            ...
            if(frame_valid) begin a_reg[5:0]<=a_r[5:0]; b_reg[5:0]<=b_r[5:0]; end
        end
    end
