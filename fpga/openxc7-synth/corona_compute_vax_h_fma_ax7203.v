...
`timescale 1ns / 1ps
// corona_compute_vax_h_fma_ax7203 — VAX_H FMA on AX7203.
module corona_compute_vax_h_fma_ax7203 (
    ...
    wire vh_sign_a = fmt_a[127];
    wire [14:0] vh_exp_a = fmt_a[126:112];
    wire [111:0] vh_mant_a = fmt_a[111:0];
    wire vh_zero_a = (vh_exp_a == 15'd0);
    wire signed [15:0] vh_exp32_s_a = $signed({1'b0, vh_exp_a}) - 16'sd16257;
    wire [7:0] vh_exp32_a = vh_exp32_s_a[7:0];
    ...
    wire [14:0] vh_exp_b = fmt_b[126:112];
    wire [111:0] vh_mant_b = fmt_b[111:0];
    wire vh_zero_b = (vh_exp_b == 15'd0);
    wire signed [15:0] vh_exp32_s_b = $signed({1'b0, vh_exp_b}) - 16'sd16257;
    wire [7:0] vh_exp32_b = vh_exp32_s_b[7:0];
    ...
    wire vh_sign_c = fmt_c[127];
    wire [14:0] vh_exp_c = fmt_c[126:112];
    wire [111:0] vh_mant_c = fmt_c[111:0];
    wire vh_zero_c = (vh_exp_c == 15'd0);
    wire signed [15:0] vh_exp32_s_c = $signed({1'b0, vh_exp_c}) - 16'sd16257;
    wire [7:0] vh_exp32_c = vh_exp32_s_c[7:0];
    ...
    wire signed [15:0] vh_exp = $signed({1'b0, q_exp}) - 16'sd127 + 16'sd16257;
    reg [127:0] q_result;
    always @(*) begin
        if(q_nan) q_result=128'h0;
        else if(q_zero) q_result=128'h0;
        else q_result={q_sign, vh_exp[14:0], q_mant, 89'b0};
    end
