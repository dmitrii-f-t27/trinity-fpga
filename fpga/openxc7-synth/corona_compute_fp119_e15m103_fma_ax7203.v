...
`timescale 1ns / 1ps
// corona_compute_fp119_e15m103_fma_ax7203 — FP119_E15M103 FMA on AX7203.
module corona_compute_fp119_e15m103_fma_ax7203 (
    ...
    wire [14:0] f_exp_a = fmt_a[117:103];
    ...
    wire signed [24:0] f_de_a = $signed({1'b0, f_exp_a}) - 25'sd16383 + 25'sd127;
    wire [7:0] f_exp32_a = (f_de_a > 25'sd254) ? 8'd254 : (f_de_a < 0) ? 8'd0 : f_de_a[7:0];
    ...
    wire [14:0] f_exp_b = fmt_b[117:103];
    wire [102:0] f_mant_b = fmt_b[102:0];
    wire f_zero_b = (f_exp_b == 0) && ((f_mant_b == 0));
    wire f_inf_b = (f_exp_b == 32767) && ((f_mant_b == 0));
    wire f_nan_b = (f_exp_b == 32767) && ((f_mant_b != 0));
    wire f_sub_b = (f_exp_b == 0) && ((f_mant_b != 0));
    wire signed [24:0] f_de_b = $signed({1'b0, f_exp_b}) - 25'sd16383 + 25'sd127;
    wire [7:0] f_exp32_b = (f_de_b > 25'sd254) ? 8'd254 : (f_de_b < 0) ? 8'd0 : f_de_b[7:0];
    ...
    wire [14:0] f_exp_c = fmt_c[117:103];
    wire [102:0] f_mant_c = fmt_c[102:0];
    wire f_zero_c = (f_exp_c == 0) && ((f_mant_c == 0));
    wire f_inf_c = (f_exp_c == 32767) && ((f_mant_c == 0));
    wire f_nan_c = (f_exp_c == 32767) && ((f_mant_c != 0));
    wire f_sub_c = (f_exp_c == 0) && ((f_mant_c != 0));
    wire signed [24:0] f_de_c = $signed({1'b0, f_exp_c}) - 25'sd16383 + 25'sd127;
    wire [7:0] f_exp32_c = (f_de_c > 25'sd254) ? 8'd254 : (f_de_c < 0) ? 8'd0 : f_de_c[7:0];
    ...
    wire signed [24:0] tgt_exp_s = $signed({1'b0, q_exp}) - 25'sd127 + 25'sd16383;
    reg [118:0] q_result;
    always @(*) begin
        if(q_nan) q_result=119'd0;
        else if(q_zero) q_result=119'd0;
        else if(q_inf) q_result={q_sign, 15'd32767, 103'd0};
        else if(q_exp >= 8'd254) q_result={q_sign, 15'd32767, 103'd0};
        else if(q_exp < 8'd0) q_result={q_sign, 118'b0};
        else if(tgt_exp_s < 1) q_result={q_sign, 15'b0, q_mant[22:0]};
        else q_result={q_sign, tgt_exp_s[14:0], q_mant[22:0]};
    end
