...
`timescale 1ns / 1ps
// corona_compute_fp46_e8m37_fma_ax7203 — FP46_E8M37 FMA on AX7203.
module corona_compute_fp46_e8m37_fma_ax7203 (
    ...
    reg [4:0] frm; reg [7:0] fmt_r; reg [45:0] a_r,b_r,c_r; reg frame_valid;
    always @(posedge mclk or posedge rst) begin
        if(rst) begin frm<=0;fmt_r<=0;a_r<=0;b_r<=0;c_r<=0;frame_valid<=0; end
            ...
            if(rx_new) begin case(frm)
                ...
                5'd8: begin a_r[45:40]<=rx_byte;frm<=5'd9; end
                5'd9: begin b_r[7:0]<=rx_byte;frm<=5'd10; end
                5'd10: begin b_r[15:8]<=rx_byte;frm<=5'd11; end
                5'd11: begin b_r[23:16]<=rx_byte;frm<=5'd12; end
                5'd12: begin b_r[31:24]<=rx_byte;frm<=5'd13; end
                5'd13: begin b_r[39:32]<=rx_byte;frm<=5'd14; end
                5'd14: begin b_r[45:40]<=rx_byte;frm<=5'd15; end
                5'd15: begin c_r[7:0]<=rx_byte;frm<=5'd16; end
                5'd16: begin c_r[15:8]<=rx_byte;frm<=5'd17; end
                5'd17: begin c_r[23:16]<=rx_byte;frm<=5'd18; end
                5'd18: begin c_r[31:24]<=rx_byte;frm<=5'd19; end
                5'd19: begin c_r[39:32]<=rx_byte;frm<=5'd20; end
                5'd20: begin c_r[45:40]<=rx_byte;frm<=5'd21; end
    ...
    reg [45:0] a_reg,b_reg,c_reg; reg comp_trigger;
    wire [45:0] fmt_a=a_reg, fmt_b=b_reg, fmt_c=c_reg;
    wire f_sign_a = fmt_a[45];
    wire [7:0] f_exp_a = fmt_a[44:37];
    wire [36:0] f_mant_a = fmt_a[36:0];
    ...
    wire [22:0] f_mant32_a = {f_mant_a, -14'b0};
    ...
    wire f_sign_b = fmt_b[45];
    wire [7:0] f_exp_b = fmt_b[44:37];
    wire [36:0] f_mant_b = fmt_b[36:0];
    ...
    wire [22:0] f_mant32_b = {f_mant_b, -14'b0};
    ...
    wire f_sign_c = fmt_c[45];
    wire [7:0] f_exp_c = fmt_c[44:37];
    wire [36:0] f_mant_c = fmt_c[36:0];
    ...
    wire [22:0] f_mant32_c = {f_mant_c, -14'b0};
    ...
    reg [45:0] q_result;
    always @(*) begin
        if(q_nan) q_result=46'd0;
        else if(q_zero) q_result=46'd0;
        else if(q_inf) q_result={q_sign, 8'd255, 37'd0};
        else if(q_exp >= 8'd254) q_result={q_sign, 8'd255, 37'd0};
        else if(q_exp < 8'd1) q_result={q_sign, 45'b0};
        else if(tgt_exp_s < 1) q_result={q_sign, 8'b0, q_mant[22:0]};
        else q_result={q_sign, tgt_exp_s[7:0], q_mant[22:0]};
    end
    reg [45:0] result_reg; reg result_ready;
    always @(posedge mclk or posedge rst) begin
    ...
    localparam [3:0] TX_LEN = 7;
    // TX: buffer+mux (no conflicting NBA — fixes tx race). 7 bytes sliced from tx_load[55:0].
    wire [55:0] tx_load = {result_reg, 8'hA5};
    reg responding; reg [2:0] tx_idx; reg [7:0] tx_buf0, tx_buf1, tx_buf2, tx_buf3, tx_buf4, tx_buf5, tx_buf6;
    reg [8:0] tcnt; reg [3:0] tbi; reg [9:0] tsr;
    always @(posedge mclk or posedge rst) begin
        if(rst) begin responding<=0;tx_idx<=0;tcnt<=BAUD_DIV-1;tbi<=0;tsr<=10'h3FF;uart_tx<=1;
            tx_buf0<=8'hFF; tx_buf1<=8'hFF; tx_buf2<=8'hFF; tx_buf3<=8'hFF; tx_buf4<=8'hFF; tx_buf5<=8'hFF; tx_buf6<=8'hFF; end
        else begin uart_tx<=tsr[0];
            if(result_ready) begin
                tx_buf0<=tx_load[7:0]; tx_buf1<=tx_load[15:8]; tx_buf2<=tx_load[23:16]; tx_buf3<=tx_load[31:24]; tx_buf4<=tx_load[39:32]; tx_buf5<=tx_load[47:40]; tx_buf6<=tx_load[55:48]; responding<=1; tx_idx<=0;
            end
            if(tcnt==0) begin tcnt<=BAUD_DIV-1;
                if(tbi==9) begin tbi<=0;
                    if(responding) begin
                        ...
                        endcase
                        if(tx_idx==6) responding<=0; else tx_idx<=tx_idx+1;
