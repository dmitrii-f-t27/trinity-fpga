...
`timescale 1ns / 1ps
// corona_compute_takum8_mul_ax7203 — TAKUM8 MUL on AX7203.
module corona_compute_takum8_mul_ax7203 (
    ...
    wire [7:0] tk_val_a = fmt_a;
    wire tk_zero_a = (tk_val_a <= 8'd7) || (tk_val_a >= 8'd248);
    wire tk_nar_a = (tk_val_a == 8'd80);
    reg [31:0] fp32_a;
    always @(*) begin
        case(tk_val_a)
8'd0: fp32_a=32'h00000000;
    ...
    wire [7:0] tk_val_b = fmt_b;
    wire tk_zero_b = (tk_val_b <= 8'd7) || (tk_val_b >= 8'd248);
    wire tk_nar_b = (tk_val_b == 8'd80);
    reg [31:0] fp32_b;
    always @(*) begin
        case(tk_val_b)
8'd0: fp32_b=32'h00000000;
    ...
    reg [7:0] q_result;
    always @(*) begin
        if(q_nan) q_result=8'd80;
        else if(q_zero) q_result=8'd0;
        else if(q_inf) q_result=q_sign?8'd129:8'd120;
        else if(q_sign) begin
            if(q_exp<=8'd40) q_result=8'd247;
            else if(q_exp>=8'd212) q_result=8'd136;
            else q_result=8'd248-(q_exp-8'd40);
        end else begin
            if(q_exp<=8'd40) q_result=8'd0;
            else if(q_exp>=8'd250) q_result=8'd120;
            else q_result=(q_exp-8'd40)+8'd8;
        end
    end
