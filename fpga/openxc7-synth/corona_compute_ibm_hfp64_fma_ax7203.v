...
`timescale 1ns / 1ps
// corona_compute_ibm_hfp64_fma_ax7203 — IBM_HFP64 FMA on AX7203.
module corona_compute_ibm_hfp64_fma_ax7203 (
    ...
    wire ib_sign_c = fmt_c[63];
    wire [6:0] ib_exp_c = fmt_c[62:56];
    wire [55:0] ib_frac_c = fmt_c[55:0];
    wire ib_zero_c = (ib_exp_c == 7'd0) && (ib_frac_c == 56'd0);
    reg [2:0] ib_lhd_c;
    always @(*) begin
        casez(ib_frac_c[55:52])
            4'b1???: ib_lhd_c=3'd0;
            4'b01??: ib_lhd_c=3'd1;
            4'b001?: ib_lhd_c=3'd2;
            4'b0001: ib_lhd_c=3'd3;
            default: ib_lhd_c=3'd4;
        endcase
    end
    wire [55:0] ib_shifted_c = ib_frac_c << ({ib_lhd_c, 2'b00});
    wire signed [10:0] ib_exp_s_c = $signed({1'b0, ib_exp_c, 2'b00}) - 11'sd185
        + $signed({8'b0, ib_lhd_c, 2'b00});
    wire [7:0] ib_exp32_c = ib_exp_s_c[7:0];
    wire [22:0] ib_mant32_c = ib_shifted_c[54:32];
    reg [31:0] fp32_c;
    always @(*) begin
        if(ib_zero_c) fp32_c={ib_sign_c, 31'b0};
        else fp32_c={ib_sign_c, ib_exp32_c, ib_mant32_c};
    end
