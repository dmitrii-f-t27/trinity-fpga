...
module corona_decode_gf4_ax7203 (
    ...
    wire [31:0] result;
    gf_decode_param #(.N(4), .E(1, .HAS_INF(0)), .M(2), .BIAS(0), .OUT_REG(0)) u_dec (.clk(1'b0), .rst_n(1'b1), .gf_in(code_r[3:0]), .fp32_out(result), .is_nan_o(), .is_inf_o(), .is_zero_o(), .is_subnormal_o());
    assign led[2] = |result;
