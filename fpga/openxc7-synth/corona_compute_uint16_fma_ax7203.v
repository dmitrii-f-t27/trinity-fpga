...
`timescale 1ns / 1ps
// corona_compute_uint16_fma_ax7203 — UINT16 FMA on AX7203.
module corona_compute_uint16_fma_ax7203 (
    ...
    always @(*) begin
        casez(uval_a)
            16'b1???????????????: u_lzd_a=5'd0;
            16'b01??????????????: u_lzd_a=5'd1;
            16'b001?????????????: u_lzd_a=5'd2;
            16'b0001????????????: u_lzd_a=5'd3;
            16'b00001???????????: u_lzd_a=5'd4;
            16'b000001??????????: u_lzd_a=5'd5;
            16'b0000001?????????: u_lzd_a=5'd6;
            16'b00000001????????: u_lzd_a=5'd7;
            16'b000000001???????: u_lzd_a=5'd8;
            16'b0000000001??????: u_lzd_a=5'd9;
            16'b00000000001?????: u_lzd_a=5'd10;
            16'b000000000001????: u_lzd_a=5'd11;
            16'b0000000000001???: u_lzd_a=5'd12;
            16'b00000000000001??: u_lzd_a=5'd13;
            16'b000000000000001?: u_lzd_a=5'd14;
            default: u_lzd_a=5'd15;
        endcase
    end
    wire [7:0] u_fp32_exp_a = 8'd127 + 8'd15 - {3'b0, u_lzd_a};
    wire [15:0] u_shifted_a = uval_a << {u_lzd_a, 1'b0};
    wire [22:0] u_fp32_mant_a = {u_shifted_a[14:0], 8'b0};
    reg [31:0] fp32_a;
    always @(*) begin
    ...
    always @(*) begin
        casez(uval_b)
            16'b1???????????????: u_lzd_b=5'd0;
            16'b01??????????????: u_lzd_b=5'd1;
            16'b001?????????????: u_lzd_b=5'd2;
            16'b0001????????????: u_lzd_b=5'd3;
            16'b00001???????????: u_lzd_b=5'd4;
            16'b000001??????????: u_lzd_b=5'd5;
            16'b0000001?????????: u_lzd_b=5'd6;
            16'b00000001????????: u_lzd_b=5'd7;
            16'b000000001???????: u_lzd_b=5'd8;
            16'b0000000001??????: u_lzd_b=5'd9;
            16'b00000000001?????: u_lzd_b=5'd10;
            16'b000000000001????: u_lzd_b=5'd11;
            16'b0000000000001???: u_lzd_b=5'd12;
            16'b00000000000001??: u_lzd_b=5'd13;
            16'b000000000000001?: u_lzd_b=5'd14;
            default: u_lzd_b=5'd15;
        endcase
    end
    wire [7:0] u_fp32_exp_b = 8'd127 + 8'd15 - {3'b0, u_lzd_b};
    wire [15:0] u_shifted_b = uval_b << {u_lzd_b, 1'b0};
    wire [22:0] u_fp32_mant_b = {u_shifted_b[14:0], 8'b0};
    reg [31:0] fp32_b;
    always @(*) begin
        if(u_zero_b) fp32_b=32'h00000000;
        else fp32_b={1'b0, u_fp32_exp_b, u_fp32_mant_b};
    end
    wire [15:0] uval_c = fmt_c_c[15:0];
    wire u_zero_c = (uval_c == 16'd0);
    wire [4:0] u_lzd_c;
    always @(*) begin
        casez(uval_c)
            16'b1???????????????: u_lzd_c=5'd0;
            16'b01??????????????: u_lzd_c=5'd1;
            16'b001?????????????: u_lzd_c=5'd2;
            16'b0001????????????: u_lzd_c=5'd3;
            16'b00001???????????: u_lzd_c=5'd4;
            16'b000001??????????: u_lzd_c=5'd5;
            16'b0000001?????????: u_lzd_c=5'd6;
            16'b00000001????????: u_lzd_c=5'd7;
            16'b000000001???????: u_lzd_c=5'd8;
            16'b0000000001??????: u_lzd_c=5'd9;
            16'b00000000001?????: u_lzd_c=5'd10;
            16'b000000000001????: u_lzd_c=5'd11;
            16'b0000000000001???: u_lzd_c=5'd12;
            16'b00000000000001??: u_lzd_c=5'd13;
            16'b000000000000001?: u_lzd_c=5'd14;
            default: u_lzd_c=5'd15;
        endcase
    end
    wire [7:0] u_fp32_exp_c = 8'd127 + 8'd15 - {3'b0, u_lzd_c};
    wire [15:0] u_shifted_c = uval_c << {u_lzd_c, 1'b0};
    wire [22:0] u_fp32_mant_c = {u_shifted_c[14:0], 8'b0};
    reg [31:0] fp32_c;
    always @(*) begin
        if(u_zero_c) fp32_c=32'h00000000;
        else fp32_c={1'b0, u_fp32_exp_c, u_fp32_mant_c};
    end
    ...
    wire [31:0] q_shifted = (q_real_exp >= 0) ? (q_abs_mant << q_real_exp[4:0]) : (q_abs_mant >> (-q_real_exp));
    reg [15:0] q_result;
    always @(*) begin
        if(q_nan) q_result=16'd0;
        else if(q_zero) q_result=16'd0;
        else if(q_sign) q_result=16'd0;
        else if(q_shifted > 32'd65535) q_result=16'hFFFF;
        else q_result=q_shifted[15:0];
    end
