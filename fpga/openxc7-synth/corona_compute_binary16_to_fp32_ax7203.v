...
`timescale 1ns / 1ps
// corona_compute_binary16_to_fp32_ax7203 — BINARY16 TO_FP32 on AX7203.
module corona_compute_binary16_to_fp32_ax7203 (
    ...
    wire sv=a_reg[15]; wire [4:0] ev=a_reg[14:10]; wire [9:0] mv=a_reg[9:0];
    wire iz=(ev==0)&&(mv==0);
    wire ii=(1)&&(ev=={5{1'b1}})&&(mv==0);
    wire in=(1)&&(ev=={5{1'b1}})&&(mv!=0);
    reg [31:0] fr;
    always @(*) begin
        if(in) fr=32'h7FC00000;
        else if(ii) fr={sv,8'hFF,23'h0};
        else if(iz) fr={sv,8'h00,23'h0};
        else fr={sv,{8'b0,ev}+9'd112,{13{1'b0}},mv};
    end
    always @(posedge mclk or posedge rst) begin
