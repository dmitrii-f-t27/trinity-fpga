...
`timescale 1ns / 1ps
// corona_compute_fp7_e2m4_fma_ax7203 — FP7_E2M4 FMA on AX7203.
module corona_compute_fp7_e2m4_fma_ax7203 (
    ...
    wire [1:0] f_exp_b = fmt_b_b[5:4];
    wire [3:0] f_mant_b = fmt_b_b[3:0];
    wire f_zero_b = (f_exp_b == 0) && ((f_mant_b == 0));
    wire f_inf_b = (f_exp_b == 3) && ((f_mant_b == 0));
    wire f_nan_b = (f_exp_b == 3) && ((f_mant_b != 0));
    ...
    wire [1:0] f_exp_c = fmt_c_c[5:4];
    wire [3:0] f_mant_c = fmt_c_c[3:0];
    wire f_zero_c = (f_exp_c == 0) && ((f_mant_c == 0));
    wire f_inf_c = (f_exp_c == 3) && ((f_mant_c == 0));
    wire f_nan_c = (f_exp_c == 3) && ((f_mant_c != 0));
    ...
    always @(*) begin
        ...
        else if(q_inf) q_result={q_sign, 2'd3, 4'd0};
        else if(q_exp >= 8'd130) q_result={q_sign, 2'd3, 4'd0};
        else if(q_exp < 8'd127) q_result={q_sign, 6'b0};
        else if(tgt_exp_s < 1) q_result={q_sign, 2'b0, q_mant[22:19]};
        else q_result={q_sign, tgt_exp_s[1:0], q_mant[22:19]};
    end
