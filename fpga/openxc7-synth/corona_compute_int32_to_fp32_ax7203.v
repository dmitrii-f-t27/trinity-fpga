...
`timescale 1ns / 1ps
// corona_compute_int32_to_fp32_ax7203 — INT32 TO_FP32 on AX7203.
module corona_compute_int32_to_fp32_ax7203 (
    ...
    wire signed [31:0] sint = a_reg;
    wire neg = sint[31];
    wire [30:0] absval = neg ? -sint : sint;
    reg [31:0] fp32_result;
    reg [5:0] lz; integer k;
    reg [31:0] norm;
    always @(*) begin
        if(sint == 0) fp32_result = 32'h00000000;
            ...
            for(k=30; k>=0; k=k-1) begin
                if(lz==0 && absval[k]) lz = k[5:0];
            end
            norm = absval << (30 - lz);
            fp32_result = {neg, {1'b0, lz} + 9'd127, norm[30:7]};
        end
    end
    always @(posedge mclk or posedge rst) begin
