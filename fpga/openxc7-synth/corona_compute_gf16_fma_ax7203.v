...
`timescale 1ns / 1ps
// corona_compute_gf16_fma_ax7203 — GoldenFloat16 FMA (y = a*b + c) on AX7203.
// Chains gf_mul_param (1 cycle) → gf_adder_param (1 cycle). Total latency: 2 cycles.
// GF16: [S:1][E:6][M:9] = 16 bits, BIAS=31, HAS_INF=1.
//
// Frame protocol (11 bytes TX):
//   AA 55 fmt a_lo a_hi b_lo b_hi c_lo c_hi trig
// Response (5 bytes RX):
//   A5 r_lo r_hi 00 00

module corona_compute_gf16_fma_ax7203 (
    ...
    // ---- Frame FSM: AA 55 fmt a[lo,hi] b[lo,hi] c[lo,hi] trig (11 bytes) ----
    ...
    // Pipeline: frame_valid triggers MUL, MUL output triggers ADD
    always @(posedge mclk or posedge rst) begin
        if(rst) begin
            ...
            // When MUL output is valid, feed product to ADD
