...
`timescale 1ns / 1ps
// corona_compute_gf256_to_fp32_ax7203 — GF256 to IEEE binary32 converter.
// Input: GF256 [S:1][E:97][M:158]
// Output: FP32 [S:1][E:8][M:23]
module corona_compute_gf256_to_fp32_ax7203 (
    ...
    wire gf_sign = a_reg[255];
    wire [96:0] gf_exp = a_reg[254:158];
    wire [157:0] gf_mant = a_reg[157:0];
    ...
    always @(*) begin
        ...
        else if(gf_denorm) fp32_result = {gf_sign, 8'b0, {gf_mant, -135'b0}};
    ...
    // TX: buffer+mux (no conflicting NBA — fixes tx race). 5 bytes sliced from tx_load[263:0].
