...
`timescale 1ns / 1ps
// corona_compute_gf12_add_ax7203 — GoldenFloat12 ADD on AX7203.
// GF12: [S:1][E:5][M:6] = 12 bits, BIAS=15, HAS_INF=0.
// 2 bytes per operand, 7-byte frame.
//
// Frame protocol (7 bytes TX):
//   AA 55 fmt a0 a1 b0 b1 trig
// Response (5 bytes RX):
//   A5 r0 r1 00 00

module corona_compute_gf12_add_ax7203 (
    ...
    // ---- Frame FSM ----
    reg [3:0] frm; reg [7:0] fmt_r; reg [11:0] a_r, b_r; reg frame_valid;
    always @(posedge mclk or posedge rst) begin
    ...
    gf_adder_param #(.EXP_BITS(4), .MANT_BITS(7), .HAS_INF(0)) u_comp (
        .clk(mclk), .rst(rst),
    ...
    always @(posedge mclk or posedge rst) begin
        if(rst) begin a_reg<=0; b_reg<=0; comp_trigger<=0; end
            ...
            if(frame_valid) begin a_reg[7:0]<=a_r[7:0]; a_reg[11:8]<=a_r[11:8]; b_reg[7:0]<=b_r[7:0]; b_reg[11:8]<=b_r[11:8]; end
        end
    end
