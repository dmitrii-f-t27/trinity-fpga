...
`timescale 1ns / 1ps
// corona_compute_int32_fma_ax7203 — INT32 FMA on AX7203.
module corona_compute_int32_fma_ax7203 (
    ...
    always @(posedge mclk or posedge rst) begin
        if(rst) begin frm<=0;fmt_r<=0;c_r<=0;a_r<=0;b_r<=0;frame_valid<=0; end
    ...
    wire signed [31:0] sr = $signed(a_reg) * $signed(b_reg) + $signed(c_reg);
    always @(posedge mclk or posedge rst) begin
