...
`timescale 1ns / 1ps
// corona_compute_fp108_e12m95_fma_ax7203 — FP108_E12M95 FMA on AX7203.
module corona_compute_fp108_e12m95_fma_ax7203 (
    ...
    wire [11:0] f_exp_a = fmt_a[106:95];
    wire [94:0] f_mant_a = fmt_a[94:0];
    ...
    wire signed [21:0] f_de_a = $signed({1'b0, f_exp_a}) - 22'sd2047 + 22'sd127;
    wire [7:0] f_exp32_a = (f_de_a > 22'sd254) ? 8'd254 : (f_de_a < 0) ? 8'd0 : f_de_a[7:0];
    wire [22:0] f_mant32_a = {f_mant_a, -72'b0};
    ...
    wire [11:0] f_exp_b = fmt_b[106:95];
    wire [94:0] f_mant_b = fmt_b[94:0];
    wire f_zero_b = (f_exp_b == 0) && ((f_mant_b == 0));
    wire f_inf_b = (f_exp_b == 4095) && ((f_mant_b == 0));
    wire f_nan_b = (f_exp_b == 4095) && ((f_mant_b != 0));
    wire f_sub_b = (f_exp_b == 0) && ((f_mant_b != 0));
    wire signed [21:0] f_de_b = $signed({1'b0, f_exp_b}) - 22'sd2047 + 22'sd127;
    wire [7:0] f_exp32_b = (f_de_b > 22'sd254) ? 8'd254 : (f_de_b < 0) ? 8'd0 : f_de_b[7:0];
    wire [22:0] f_mant32_b = {f_mant_b, -72'b0};
    ...
    wire [11:0] f_exp_c = fmt_c[106:95];
    wire [94:0] f_mant_c = fmt_c[94:0];
    wire f_zero_c = (f_exp_c == 0) && ((f_mant_c == 0));
    wire f_inf_c = (f_exp_c == 4095) && ((f_mant_c == 0));
    wire f_nan_c = (f_exp_c == 4095) && ((f_mant_c != 0));
    wire f_sub_c = (f_exp_c == 0) && ((f_mant_c != 0));
    wire signed [21:0] f_de_c = $signed({1'b0, f_exp_c}) - 22'sd2047 + 22'sd127;
    wire [7:0] f_exp32_c = (f_de_c > 22'sd254) ? 8'd254 : (f_de_c < 0) ? 8'd0 : f_de_c[7:0];
    wire [22:0] f_mant32_c = {f_mant_c, -72'b0};
    ...
    wire signed [21:0] tgt_exp_s = $signed({1'b0, q_exp}) - 22'sd127 + 22'sd2047;
    reg [107:0] q_result;
    always @(*) begin
        if(q_nan) q_result=108'd0;
        else if(q_zero) q_result=108'd0;
        else if(q_inf) q_result={q_sign, 12'd4095, 95'd0};
        else if(q_exp >= 8'd254) q_result={q_sign, 12'd4095, 95'd0};
        else if(q_exp < 8'd0) q_result={q_sign, 107'b0};
        else if(tgt_exp_s < 1) q_result={q_sign, 12'b0, q_mant[22:0]};
        else q_result={q_sign, tgt_exp_s[11:0], q_mant[22:0]};
    end
