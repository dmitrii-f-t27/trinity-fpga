...
`timescale 1ns / 1ps
// corona_compute_fp7_e4m2_fma_ax7203 — FP7_E4M2 FMA on AX7203.
module corona_compute_fp7_e4m2_fma_ax7203 (
    ...
    reg [2:0] frm; reg [7:0] fmt_r; reg [6:0] a_r,b_r,c_r; reg frame_valid;
    always @(posedge mclk or posedge rst) begin
        if(rst) begin frm<=0;fmt_r<=0;a_r<=0;b_r<=0;c_r<=0;frame_valid<=0; end
            ...
            if(rx_new) begin case(frm)
                ...
                3'd3: begin a_r[6:0]<=rx_byte;frm<=3'd4; end
                3'd4: begin b_r[6:0]<=rx_byte;frm<=3'd5; end
                3'd5: begin c_r[6:0]<=rx_byte;frm<=3'd6; end
    ...
    reg [6:0] a_reg,b_reg,c_reg; reg comp_trigger;
    wire [6:0] fmt_a=a_reg, fmt_b=b_reg, fmt_c=c_reg;
    wire f_sign_a = fmt_a_a[6];
    wire [3:0] f_exp_a = fmt_a_a[5:2];
    ...
    wire f_sign_b = fmt_b_b[6];
    wire [3:0] f_exp_b = fmt_b_b[5:2];
    wire [1:0] f_mant_b = fmt_b_b[1:0];
    ...
    wire f_sign_c = fmt_c_c[6];
    wire [3:0] f_exp_c = fmt_c_c[5:2];
    wire [1:0] f_mant_c = fmt_c_c[1:0];
    ...
    wire [22:0] f_mant32_norm_c = {1'b0, f_mant_c, 20'b0};
    reg [31:0] fp32_c;
    always @(*) begin
    ...
    reg [6:0] q_result;
    always @(*) begin
        if(q_nan) q_result=7'd0;
        else if(q_zero) q_result=7'd0;
        else if(q_inf) q_result={q_sign, 4'd15, 2'd0};
        else if(q_exp >= 8'd136) q_result={q_sign, 4'd15, 2'd0};
        else if(q_exp < 8'd121) q_result={q_sign, 6'b0};
        else if(tgt_exp_s < 1) q_result={q_sign, 4'b0, q_mant[22:21]};
        else q_result={q_sign, tgt_exp_s[3:0], q_mant[22:21]};
    end
    reg [31:0] result_reg; reg result_ready;
    always @(posedge mclk or posedge rst) begin
        if(rst) begin result_reg<=0;result_ready<=0; end
        else begin result_ready<=add_ovld;
            if(add_ovld) result_reg<={25'b0,q_result};
        end
    end
