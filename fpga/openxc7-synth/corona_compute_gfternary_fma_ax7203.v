...
`timescale 1ns / 1ps
// corona_compute_gfternary_fma_ax7203 — GFTERNARY FMA on AX7203.
module corona_compute_gfternary_fma_ax7203 (
    ...
    reg [2:0] frm; reg [7:0] fmt_r; reg [1:0] a_r,b_r,c_r; reg frame_valid;
    always @(posedge mclk or posedge rst) begin
        if(rst) begin frm<=0;fmt_r<=0;a_r<=0;b_r<=0;c_r<=0;frame_valid<=0; end
            ...
            if(rx_new) begin case(frm)
                ...
                3'd3: begin a_r[1:0]<=rx_byte;frm<=3'd4; end
                3'd4: begin b_r[1:0]<=rx_byte;frm<=3'd5; end
                3'd5: begin c_r[1:0]<=rx_byte;frm<=3'd6; end
    ...
    reg [1:0] a_reg,b_reg,c_reg; reg comp_trigger;
    wire [1:0] fmt_a=a_reg, fmt_b=b_reg, fmt_c=c_reg;
    wire [1:0] gft_val_a = fmt_a;
    reg [31:0] fp32_a;
    always @(*) begin
        case(gft_val_a)
            2'b00: fp32_a=32'h00000000;
            2'b01: fp32_a=32'h3FCF1BBD;
            2'b10: fp32_a=32'hBFCF1BBD;
            default: fp32_a=32'h3FCF1BBD;
        endcase
    end
    wire [1:0] gft_val_b = fmt_b;
    reg [31:0] fp32_b;
    always @(*) begin
        case(gft_val_b)
            2'b00: fp32_b=32'h00000000;
            2'b01: fp32_b=32'h3FCF1BBD;
            2'b10: fp32_b=32'hBFCF1BBD;
            default: fp32_b=32'h3FCF1BBD;
        endcase
    end
    wire [1:0] gft_val_c = fmt_c;
    reg [31:0] fp32_c;
    always @(*) begin
        case(gft_val_c)
            2'b00: fp32_c=32'h00000000;
            2'b01: fp32_c=32'h3FCF1BBD;
            2'b10: fp32_c=32'hBFCF1BBD;
            default: fp32_c=32'h3FCF1BBD;
        endcase
    end
    ...
    reg [1:0] q_result;
    always @(*) begin
        if(q_in==32'h00000000) q_result=2'd0;
        else if(q_in[31]) begin
            if(q_in>=32'hBF000000) q_result=2'd2;
            else q_result=2'd0;
        end else begin
            if(q_in>=32'h3F800000) q_result=2'd1;
            else if(q_in>=32'h3E800000) q_result=2'd1;
            else q_result=2'd0;
        end
    end
    ...
    always @(posedge mclk or posedge rst) begin
        if(rst) begin result_reg<=0;result_ready<=0; end
            ...
            if(add_ovld) result_reg<={30'b0,q_result};
        end
    end
