...
`timescale 1ns / 1ps
// corona_compute_int16_to_fp32_ax7203 — INT16 TO_FP32 on AX7203.
module corona_compute_int16_to_fp32_ax7203 (
    ...
    wire signed [15:0] sint = a_reg;
    wire neg = sint[15];
    wire [14:0] absval = neg ? -sint : sint;
    ...
    always @(*) begin
        if(sint == 0) fp32_result = 32'h00000000;
            ...
            norm = absval << (14 - lz);
            fp32_result = {neg, {1'b0, lz} + 9'd127, norm[14:0]};
        end
    end
    always @(posedge mclk or posedge rst) begin
