...
`timescale 1ns / 1ps
// corona_compute_fp32_to_nf4_ax7203 — NF4 FP32_TO on AX7203.
module corona_compute_fp32_to_nf4_ax7203 (
    ...
    wire [31:0] fp32_in = a_reg;
    reg [3:0] code;
    always @(*) begin
        if(fp32_in == 32'h00000000) code = 4'd7;
        else if(fp32_in[31]) begin
            // Negative
            if(fp32_in >= 32'hBF593800) code = 4'd0;
            else if(fp32_in >= 32'hBF1C6800) code = 4'd1;
            else if(fp32_in >= 32'hBEE8B200) code = 4'd2;
            else if(fp32_in >= 32'hBEAC2000) code = 4'd3;
            else if(fp32_in >= 32'hBE6C9000) code = 4'd4;
            else if(fp32_in >= 32'hBDCBE800) code = 4'd5;
            else if(fp32_in >= 32'hBD4B4000) code = 4'd6;
            else code = 4'd7;
        end else begin
            // Positive
            if(fp32_in >= 32'h3F593800) code = 4'd15;
            else if(fp32_in >= 32'h3F279800) code = 4'd14;
            else if(fp32_in >= 32'h3EF19800) code = 4'd13;
            else if(fp32_in >= 32'h3ECA6000) code = 4'd12;
            else if(fp32_in >= 32'h3E969800) code = 4'd11;
            else if(fp32_in >= 32'h3E537800) code = 4'd10;
            else if(fp32_in >= 32'h3E029000) code = 4'd9;
            else if(fp32_in >= 32'h3D818000) code = 4'd8;
            else code = 4'd7;
        end
    end
    always @(posedge mclk or posedge rst) begin
    ...
    always @(posedge mclk or posedge rst) begin
        if(rst) begin result_reg<=0;result_ready<=0; end
            ...
            if(conv_trigger) result_reg<={28'b0,code};
        end
    end
