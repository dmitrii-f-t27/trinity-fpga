...
`timescale 1ns / 1ps
// corona_compute_fp33_e7m25_fma_ax7203 — FP33_E7M25 FMA on AX7203.
module corona_compute_fp33_e7m25_fma_ax7203 (
    ...
    wire signed [16:0] f_de_c = $signed({1'b0, f_exp_c}) - 17'sd63 + 17'sd127;
    wire [7:0] f_exp32_c = (f_de_c > 17'sd254) ? 8'd254 : (f_de_c < 0) ? 8'd0 : f_de_c[7:0];
    ...
    wire signed [16:0] tgt_exp_s = $signed({1'b0, q_exp}) - 17'sd127 + 17'sd63;
    reg [32:0] q_result;
    always @(*) begin
        if(q_nan) q_result=33'd0;
        else if(q_zero) q_result=33'd0;
        else if(q_inf) q_result={q_sign, 7'd127, 25'd0};
        else if(q_exp >= 8'd192) q_result={q_sign, 7'd127, 25'd0};
        else if(q_exp < 8'd65) q_result={q_sign, 32'b0};
        else if(tgt_exp_s < 1) q_result={q_sign, 7'b0, q_mant[22:0]};
        else q_result={q_sign, tgt_exp_s[6:0], q_mant[22:0]};
    end
