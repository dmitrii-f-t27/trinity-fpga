...
`timescale 1ns / 1ps
// corona_compute_fp32_to_gf32_ax7203 — IEEE binary32 to GF32 converter.
// Input: FP32 [S:1][E:8][M:23] (bias=127)
// Output: GF32 [S:1][E:12][M:19] (bias=2047, HAS_INF=0)
// Mantissa truncation. Overflow -> max, underflow -> zero.
module corona_compute_fp32_to_gf32_ax7203 (
    ...
    wire signed [13:0] gf_exp_calc = $signed({2'b0, fp_exp}) + 14'sd1920;
    wire overflow  = (gf_exp_calc > 14'sd4095);
    wire underflow = (gf_exp_calc < 14'sd1);
    wire [18:0] gf_mant_trunc = fp_mant[22:4];
    reg [31:0] gf_result;
    always @(*) begin
        if (fp_nan)
            gf_result = {fp_sign, {12{1'b1}}, {19{1'b0}}};
        else if (fp_inf || overflow)
            gf_result = {fp_sign, gf_exp_calc[11:0] | {12{1'b1}}, {19{1'b0}}};
        else if (fp_zero || fp_denorm || underflow)
            gf_result = {fp_sign, {12{1'b0}}, {19{1'b0}}};
        else
            gf_result = {fp_sign, gf_exp_calc[11:0], gf_mant_trunc};
    end
    always @(posedge mclk or posedge rst) begin
