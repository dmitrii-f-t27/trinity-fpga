...
`timescale 1ns / 1ps
// corona_compute_fp14_e6m7_fma_ax7203 — FP14_E6M7 FMA on AX7203.
module corona_compute_fp14_e6m7_fma_ax7203 (
    ...
    reg [3:0] frm; reg [7:0] fmt_r; reg [13:0] a_r,b_r,c_r; reg frame_valid;
    always @(posedge mclk or posedge rst) begin
        if(rst) begin frm<=0;fmt_r<=0;a_r<=0;b_r<=0;c_r<=0;frame_valid<=0; end
            ...
            if(rx_new) begin case(frm)
                ...
                4'd4: begin a_r[13:8]<=rx_byte;frm<=4'd5; end
                4'd5: begin b_r[7:0]<=rx_byte;frm<=4'd6; end
                4'd6: begin b_r[13:8]<=rx_byte;frm<=4'd7; end
                4'd7: begin c_r[7:0]<=rx_byte;frm<=4'd8; end
                4'd8: begin c_r[13:8]<=rx_byte;frm<=4'd9; end
    ...
    reg [13:0] a_reg,b_reg,c_reg; reg comp_trigger;
    wire [13:0] fmt_a=a_reg, fmt_b=b_reg, fmt_c=c_reg;
    wire f_sign_a = fmt_a[13];
    wire [5:0] f_exp_a = fmt_a[12:7];
    ...
    wire f_sign_b = fmt_b[13];
    wire [5:0] f_exp_b = fmt_b[12:7];
    ...
    wire f_sign_c = fmt_c[13];
    wire [5:0] f_exp_c = fmt_c[12:7];
    ...
    reg [13:0] q_result;
    always @(*) begin
        if(q_nan) q_result=14'd0;
        else if(q_zero) q_result=14'd0;
        else if(q_inf) q_result={q_sign, 6'd63, 7'd0};
        else if(q_exp >= 8'd160) q_result={q_sign, 6'd63, 7'd0};
        else if(q_exp < 8'd97) q_result={q_sign, 13'b0};
        else if(tgt_exp_s < 1) q_result={q_sign, 6'b0, q_mant[22:16]};
        else q_result={q_sign, tgt_exp_s[5:0], q_mant[22:16]};
    end
    reg [31:0] result_reg; reg result_ready;
    always @(posedge mclk or posedge rst) begin
        if(rst) begin result_reg<=0;result_ready<=0; end
        else begin result_ready<=add_ovld;
            if(add_ovld) result_reg<={18'b0,q_result};
        end
    end
