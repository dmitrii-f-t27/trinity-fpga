...
`timescale 1ns / 1ps
// corona_compute_gf24_to_fp32_ax7203 — GF24 to IEEE binary32 converter.
// Input: GF24 [S:1][E:9][M:14] (bias=255, HAS_INF=0)
...
module corona_compute_gf24_to_fp32_ax7203 (
    ...
    // ---- Frame FSM ----
    reg [2:0] frm; reg [7:0] fmt_r; reg [23:0] a_r; reg frame_valid;
    always @(posedge mclk or posedge rst) begin
    ...
    // ---- GF24->FP32 conversion ----
    reg [23:0] a_reg;
    reg conv_trigger;

    wire        gf_sign = a_reg[23];
    wire [8:0] gf_exp  = a_reg[22:14];
    wire [13:0] gf_mant = a_reg[13:0];
    ...
    always @(*) begin
        if (gf_zero)
            ...
            fp32_result = {gf_sign, 8'b0, {gf_mant, 9'b0}};
        else
