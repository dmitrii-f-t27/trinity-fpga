...
`timescale 1ns / 1ps
// corona_compute_fp95_e8m86_mul_ax7203 — FP95_E8M86 MUL on AX7203.
module corona_compute_fp95_e8m86_mul_ax7203 (
    ...
    reg [4:0] frm; reg [7:0] fmt_r; reg [94:0] a_r,b_r; reg frame_valid;
    always @(posedge mclk or posedge rst) begin
        if(rst) begin frm<=0;fmt_r<=0;a_r<=0;b_r<=0;frame_valid<=0; end
            ...
            if(rx_new) begin case(frm)
                ...
                5'd14: begin a_r[94:88]<=rx_byte;frm<=5'd15; end
                ...
                5'd26: begin b_r[94:88]<=rx_byte;frm<=5'd27; end
    ...
    reg [94:0] a_reg,b_reg; reg comp_trigger;
    wire [94:0] fmt_a=a_reg, fmt_b=b_reg;
    wire f_sign_a = fmt_a_a[94];
    wire [7:0] f_exp_a = fmt_a_a[93:86];
    ...
    wire f_sign_b = fmt_b_b[94];
    wire [7:0] f_exp_b = fmt_b_b[93:86];
    ...
    reg [94:0] q_result;
    always @(*) begin
        if(q_nan) q_result=95'd0;
        else if(q_zero) q_result=95'd0;
        else if(q_inf) q_result={q_sign, 8'd255, 86'd0};
        else if(q_exp >= 8'd254) q_result={q_sign, 8'd255, 86'd0};
        else if(q_exp < 8'd1) q_result={q_sign, 94'b0};
        else if(tgt_exp_s < 1) q_result={q_sign, 8'b0, q_mant[22:0]};
        else q_result={q_sign, tgt_exp_s[7:0], q_mant[22:0]};
    end
    reg [94:0] result_reg; reg result_ready;
    always @(posedge mclk or posedge rst) begin
